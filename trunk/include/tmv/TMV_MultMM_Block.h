...
// This file contains the implementation of the MultMM algorithms 63 and 64
// which copy the matrix in blocks to temporary storage that is guaranteed
// to be in a form for which we can use the optimized kernels (see 
// TMV_MultMM_Kernel.h).
...
#ifdef TMV_CompilingBlockMultMM
#define TMV_STATIC
#else
#define TMV_STATIC static
  ...
  struct MyCopy<true,cs,rs,M1> // complex
  {
    enum { C1 = M1::mconj };
    typedef typename M1::real_type RT;
    typedef typename M1::const_realview_type M1r;
...
#endif
      CopyM_Helper<-1,cs,rs,M1r,M2r>::call(m1r,m2r); 
      AddMM_Helper<-1,cs,rs,false,ix1,RT,M1r,M2r>::call(one,m1i,m2i); 
#ifdef PRINTALGO_MM_BLOCK
      //std::cout<<"m2r => "<<m2r<<std::endl;
      //std::cout<<"m2i => "<<m2i<<std::endl;
#endif
  ...
  struct MyProd<true,true,cs,rs,xs,RT> // both complex
  {
        ...
        RT* m3p, const int si3, const int sj3)
    {
      ...
      select_multmm<cs,rs,xs,1,RT>::call(M,N,K,one,m1p,m2p,m3p);
      select_multmm<cs,rs,xs,-1,RT>::call(M,N,K,mone,m1p+xn1,m2p+xn2,m3p);
      select_multmm<cs,rs,xs,1,RT>::call(M,N,K,one,m1p+xn1,m2p,m3p+xn3);
      select_multmm<cs,rs,xs,1,RT>::call(M,N,K,one,m1p,m2p+xn2,m3p+xn3);
#ifdef PRINTALGO_MM_BLOCK
      std::cout<<"After call to multmm\n";
  ...
  struct MyCleanup<true,true,RT> // both complex
  {
        ...
        Kcleanup* cleanup)
    {
      ...
      // I couldn't figure out any way to get it with template magic, since 
      // the cleanup object doesn't have the value we need, and its type
      // is Kcleanup which is a generic call function, so we can't even 
      // do something like a Traits specialization.
  ...
  template <bool iscomplex_x, bool iscomplex2, bool add, int ix, class T, int cs, int rs, class M2>
  struct MyAssign // both real
  {
    typedef typename M2::real_type RT;
    typedef typename M2::value_type T2; // T2 might be complex, but m1 isn't.
  ...
  struct MyAssign<false,true,add,ix,T,cs,rs,M2> // m2 is complex
  {
    typedef typename M2::real_type RT;
    typedef ConstMatrixView<RT,1> M1r;
    typedef typename M2::realview_type M2r;
    typedef typename M2::imagview_type M2i;
    ...
    static F* call(const bool add, const T& x, const M2& m)
    {
      if (add)
        ...
        if (TMV_IMAG(x) == RT(0))
          if (TMV_REAL(x) == RT(1))
            if (m.isrm()) 
              return &MyAssign<zx,z2,false,1,RT,cs,rs,M2rm>::call;
            else if (m.iscm())
              return &MyAssign<zx,z2,false,1,RT,cs,rs,M2cm>::call;
            else
            ...
            if (m.isrm()) 
              return &MyAssign<zx,z2,false,0,RT,cs,rs,M2rm>::call;
            else if (m.iscm())
              return &MyAssign<zx,z2,false,0,RT,cs,rs,M2cm>::call;
            else
  ...
  //   Mb,Nb include the last partial block in that direction if there is one.
  ...
  //   copy the data into the temporary storage (or zero out m3 in that case).
      ...
      CleanupF* mycleanup, KF* kf, KF* kfa, KF* kfb, KF* mykf)
  {
    ...
    else if (Mb == 1 && Nb == 1 && Kb == 1) 
    {
      ...
      std::cout<<"end of m2 = "<<ConstVectorView<long,1>((const long*)glob_m2p_end,10)<<std::endl;
#endif
      if (firstm1) 
        (*mycopy1) (m1.CSubMatrix(i1,i2,k1,k1+KB).Transpose(),m1p,KB,MB,1,KB);
      if (firstm2) 
...
#endif
          if (firstm1) 
            (*mycopy1_k) (m1.CSubMatrix(i1,i2,k1,k2).Transpose(),m1p,Kc,MB,1,Kd);
          if (firstm2) 
            (*mycopy2_k) (m2.CSubMatrix(k1,k2,j1,j2),m2p,Kc,NB,1,Kd);
  ...
  // RCC algorithm is generally the fastest, we copy m1 into
  // rowmajor blocks, and m2 and m3 into colmajor blocks.
      ...
      CleanupF* cleanup, KF* kf, KF* kfa, KF* kfb, KF* kfc)
  {
    ...
    std::cout<<"Mbx,Mcx,Nbx,Ncx = "<<Mbx<<','<<Mcx<<','<<Nbx<<','<<Ncx<<std::endl;
  ...
  static void DoRecursiveBlockMultMM(
      const Scaling<ix,T>& x, const M1& m1, const M2& m2, M3& m3)
  {
    ...
    enum { x_z = Traits<T>::iscomplex };
    enum { m1_z = Traits<T1>::iscomplex };
    enum { m2_z = Traits<T2>::iscomplex };
    enum { m3_z = Traits<PT3>::iscomplex };
    enum { MB    = 16 };
    enum { NB    = 16 };
#if 0
    enum { KB    = 16 };
    ...
    enum { KB    = (
        ...
        16 ) };
#endif
    enum { lgMB  = IntLog<MB>::log };
    enum { lgNB  = IntLog<NB>::log };
    enum { lgKB  = IntLog<KB>::log };
    enum { csx = cs == UNKNOWN ? UNKNOWN : (cs-((cs>>lgMB)<<lgMB)) };
    enum { rsx = rs == UNKNOWN ? UNKNOWN : (rs-((rs>>lgNB)<<lgNB)) };
    enum { xsx = xs == UNKNOWN ? UNKNOWN : (xs-((xs>>lgKB)<<lgKB)) };
    ...
    DoRecursiveBlockMultMM2(
        (RT*)(&xx),m1.Real().XView(),m2.Real().XView(),m3.Real().XView(),
  ...
  // DoBlockMultMM is the routine that does the inner K loop of the below
  ...
  // in terms of just RT, rather than T.  So m1,m2,m3 are the Real()
      ...
      const int Ka, const int K, const int Kc, const int Kd)
  {
    ...
    {
      if (firstm1) 
        (*mycopy1) (m1.CSubMatrix(i1,i2,k*KB,(k+1)*KB).Transpose(),m1p,KB,MB,1,KB);
      if (firstm2) 
    ...
    if (kf) {
      if (firstm1) 
        (*mycopy1_k) (m1.CSubMatrix(i1,i2,Ka,K).Transpose(),m1p,Kc,MB,1,Kd);
      if (firstm2) 
  ...
  // Now we have all the manipulations to make this instantiate correctly
  // when we are compiling the library and also when we are doing TMV_INLINE
  // or have non-instantiated types.


  // Generic versions for non-instantiated types:
  template <int cs, int rs, int xs, bool add, int ix, class T, class M1, class M2, class M3>
  static void RecursiveBlockMultMM(
      const Scaling<ix,T>& x, const M1& m1, const M2& m2, M3& m3)
  { DoRecursiveBlockMultMM<cs,rs,xs,add>(x,m1,m2,m3); }

  template <int cs, int rs, int xs, bool add, int ix, class T, class M1, class M2, class M3>
  static void BlockMultMM(
      const Scaling<ix,T>& x, const M1& m1, const M2& m2, M3& m3)
  { DoBlockMultMM<cs,rs,xs,add>(x,m1,m2,m3); }

#if !defined(TMV_CompilingBlockMultMM) && defined(TMV_INST_FLOAT)
  ...
  void BlockMultMM(
      ...
      MatrixView<std::complex<float> >& m3);
#elif \
  (defined(TMV_CompilingBlockMultMM) && defined(TMV_INST_FLOAT)) || \
  (!defined(TMV_CompilingBlockMultMM) && !defined(TMV_INST_FLOAT)) 
  // comp   inst_float
  // !comp  !inst_float
  template <bool CC1, bool CC2>
  TMV_STATIC void RecursiveBlockMultMM(
      const bool add, const float& x,
      const ConstMatrixView<float,UNKNOWN,UNKNOWN,CC1>& m1,
      const ConstMatrixView<float,UNKNOWN,UNKNOWN,CC2>& m2,
      MatrixView<float>& m3)
  ...
  { DoBlockMultMM(add,x,m1,m2,m3); }
#endif
#if !defined(TMV_CompilingBlockMultMM) && defined(TMV_INST_DOUBLE)
  template <bool CC1, bool CC2>
  void RecursiveBlockMultMM(
  ...
  void BlockMultMM(
      ...
      MatrixView<std::complex<double> >& m3);
#elif \
  (defined(TMV_CompilingBlockMultMM) && defined(TMV_INST_DOUBLE)) || \
  (!defined(TMV_CompilingBlockMultMM) && !defined(TMV_INST_DOUBLE)) 
  // comp   inst_double
  // !comp  !inst_double
  template <bool CC1, bool CC2>
  ...
  { DoBlockMultMM(add,x,m1,m2,m3); }
#endif
#if !defined(TMV_CompilingBlockMultMM) && defined(TMV_INST_INT)
  template <bool CC1, bool CC2>
  void RecursiveBlockMultMM(
  ...
  void BlockMultMM(
      ...
      MatrixView<std::complex<int> >& m3);
#elif \
  (defined(TMV_CompilingBlockMultMM) && defined(TMV_INST_INT)) || \
  (!defined(TMV_CompilingBlockMultMM) && !defined(TMV_INST_INT)) 
  // comp   inst_int
  // !comp  !inst_int
  template <bool CC1, bool CC2>
  ...
  { DoBlockMultMM(add,x,m1,m2,m3); }
#endif
#if !defined(TMV_CompilingBlockMultMM) && defined(TMV_INST_LONGDOUBLE)
  template <bool CC1, bool CC2>
  void RecursiveBlockMultMM(
  ...
  void BlockMultMM(
      ...
      MatrixView<std::complex<long double> >& m3);
#elif \
  (defined(TMV_CompilingBlockMultMM) && defined(TMV_INST_LONGDOUBLE)) || \
  (!defined(TMV_CompilingBlockMultMM) && !defined(TMV_INST_LONGDOUBLE)) 
  ...
  // This helper structure breaks out the add,ix if we have an instantiated
  // function to use.
  template <bool inst, int cs, int rs, int xs, bool add, int ix, class T, class M1, class M2, class M3>
  struct CallRecursiveBlockMultMM // inst = true
  {
    static void call(
        const Scaling<ix,T>& x, const M1& m1, const M2& m2, M3& m3)
    { 
      typename M3::xview_type m3x = m3.XView();
      RecursiveBlockMultMM(add,T(x),m1.XView(),m2.XView(),m3x);
    }
  };

  template <bool inst, int cs, int rs, int xs, bool add, int ix, class T, class M1, class M2, class M3>
  struct CallBlockMultMM // inst = true
  {
    static void call(
        const Scaling<ix,T>& x, const M1& m1, const M2& m2, M3& m3)
    { 
      typename M3::xview_type m3x = m3.XView();
      BlockMultMM(add,T(x),m1.XView(),m2.XView(),m3x);
    }
  };

  // If inst = false, then we call the regular inline version.
  template <int cs, int rs, int xs, bool add, int ix, class T, class M1, class M2, class M3>
  struct CallRecursiveBlockMultMM<false,cs,rs,xs,add,ix,T,M1,M2,M3> 
  {
    static void call(
        const Scaling<ix,T>& x, const M1& m1, const M2& m2, M3& m3)
    { RecursiveBlockMultMM<cs,rs,xs,add>(x,m1,m2,m3); }
  };

  template <int cs, int rs, int xs, bool add, int ix, class T, class M1, class M2, class M3>
  struct CallBlockMultMM<false,cs,rs,xs,add,ix,T,M1,M2,M3> 
  {
    static void call(
        const Scaling<ix,T>& x, const M1& m1, const M2& m2, M3& m3)
    { BlockMultMM<cs,rs,xs,add>(x,m1,m2,m3); }
  };
