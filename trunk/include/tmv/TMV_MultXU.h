...
#define TMV_XU_UNROLL 0
    ...
    struct MultXU_Helper<11,s,add,ix,T,M1,M2>
    {
        static void call(const Scaling<ix,T>& x, const M1& m1, M2& m2)
        {
            ...
            int M=1;
            for(;N;--N) {
                MultXV_Helper<-4,TMV_UNKNOWN,add,ix,T,M1c,M2c>::call2(
                    M++,x,it1,it2);
                it1.shiftP(step1);
                it2.shiftP(step2);
            }
    ...
    struct MultXU_Helper<12,s,add,ix,T,M1,M2>
    {
        static void call(const Scaling<ix,T>& x, const M1& m1, M2& m2)
        {
            ...
            for(;N;--N) {
                MultXV_Helper<-4,TMV_UNKNOWN,add,ix,T,M1r,M2r>::call2(
                    N,x,it1,it2);
                it1.shiftP(step1);
                it2.shiftP(step2);
            }
    ...
    struct MultXU_Helper<98,s,true,ix,T,M1,M2>
    {
        static void call(const Scaling<ix,T>& x, const M1& m1, M2& m2)
        {
            if ( !SameStorage(m1,m2) ||
                ...
                M1c m1c = m1;
                MultXU_Helper<-2,s,true,ix,T,M1c,M2>::call(x,m1c,m2);
            }
        }
    };
