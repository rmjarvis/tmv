...
namespace tmv {
    ...
    inline void InlinePermuteRows(
        BaseMatrix_Rec_Mutable<M>& m, const int*const p, int i1, int i2)
    {
        ...
        if (Nx != 0) {
            for(int j=0;j<Nx;) {
                ...
                for(int i=i1;i<i2;++i,++pi) {
                    ...
                    m.cColRange(j,j2).cSwapRows(i,*pi);
                }
                j = j2;
            }
        }
        if (Nx != N) {
            ...
            for(int i=i1;i<i2;++i,++pi) {
                TMVAssert(*pi < int(m.colsize()));
                m.cColRange(Nx,N).cSwapRows(i,*pi);
            }
        }
    }
    ...
    template <class T>
    inline void InstPermuteRows(
        MatrixView<T,UNKNOWN,UNKNOWN,true> m, 
        const int*const p, int i1, int i2)
    ...
    void InstReversePermuteRows(
        MatrixView<T> m, const int*const p, int i1, int i2);

    template <class T>
    inline void InstReversePermuteRows(
        MatrixView<T,UNKNOWN,UNKNOWN,true> m, 
        const int*const p, int i1, int i2)
