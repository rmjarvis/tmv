...
namespace tmv {
  ...
  template <class M>
  inline void InlineWriteCompact(std::ostream& os, const BaseMatrix_Tri<M>& m)
  {
  ...
  void InstWriteCompact(std::ostream& os,
      const ConstUpperTriMatrixView<T,D,UNKNOWN,UNKNOWN,C>& m);
  template <class T, DiagType D, bool C>
  void InstWriteCompact(std::ostream& os,
      const ConstLowerTriMatrixView<T,D,UNKNOWN,UNKNOWN,C>& m);
  ...
  public :
    ...
    inline TriMatrixReadError(int _i, int _j, const BaseMatrix_Tri<M>& _m,
      ...
      unitgot(T(1)), s(_m.size()),
      is(_is), iseof(_is.eof()), isbad(_is.bad()) {}
    inline TriMatrixReadError(std::istream& _is, char _e, char _g) throw() :
      ReadError("TriMatrix"),
    ...
    inline void Write(std::ostream& os) const throw()
    {
      ...
      if (ShapeTraits<M::mshape>::upper) os<<"UpperTriMatrix\n";
      ...
      if (m.get()) {
        const int N = m->size();
        os<<"The portion of the TriMatrix which was successfully read is: \n";
  ...
  // Defined in TMV_TriMatrix.cpp
  template <class T, DiagType D, bool C>
  void InstRead(std::istream& is, UpperTriMatrixView<T,D,UNKNOWN,UNKNOWN,C> m);
  template <class T, DiagType D, bool C>
  void InstRead(std::istream& is, LowerTriMatrixView<T,D,UNKNOWN,UNKNOWN,C> m);
  ...
  {
    char ul;
    char ul_exp = (ShapeTraits<M::mshape>::upper ? 'U' : 'L');
    is >> ul;
    if (!is)
#ifdef NOTHROW
    { std::cerr<<"TriMatrix ReadError: !is\n"; exit(1); }
    ...
    if (ul != ul_exp)
#ifdef NOTHROW
    { std::cerr<<"TriMatrix ReadError: "<<ul<<" != "<<ul_exp; exit(1); }
#else
