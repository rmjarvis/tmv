...
namespace tmv {
  ...
  struct CopyU_Helper<1,s,M1,M2>
  {
    static inline void call(const M1& m1, M2& m2)
    {
      ...
      CopyU_Helper<-1,sm1,M1o,M2o>::call(m1o,m2o);
      Maybe<!M2::munit>::unitdiag(m2);
    }
  };
  ...
  struct CopyU_Helper<2,s,M1,M2>
  {
    static inline void call(const M1& m1, M2& m2)
    {
      ...
      for(;N;--N) {
        CopyV_Helper<-1,UNKNOWN,M1r,M2r>::call2(N,it1,it2);
        it1.ShiftP(step1);
        it2.ShiftP(step2);
      }
    }
  ...
  struct CopyU_Helper<3,s,M1,M2>
  {
    static inline void call(const M1& m1, M2& m2)
    {
      ...
      int M=1;
      for(;N;--N) {
        CopyV_Helper<-1,UNKNOWN,M1c,M2c>::call2(M++,it1,it2);
        it1.ShiftP(step1);
        it2.ShiftP(step2);
      } 
    }
  ...
  template <class T, DiagType D1, bool C1, DiagType D2>
  void InstCopy(const ConstUpperTriMatrixView<T,D1,UNKNOWN,UNKNOWN,C1>& m1,
      UpperTriMatrixView<T,D2> m2);

  template <bool checkalias, bool lower, bool conj, bool inst, class M1, class M2>
  struct CallCopyU;

  template <bool lower, bool conj, bool inst, class M1, class M2>
  struct CallCopyU<true,lower,conj,inst,M1,M2>  // checkalias = true
  {
    static inline void call(const M1& m1, M2& m2)
    {
      if (!SameStorage(m1,m2)) 
      {
        CallCopyU<false,lower,conj,inst,M1,M2>::call(m1,m2);
      }
      else 
      {
        if (m1.stepi() == m2.stepi() && m1.stepj() == m2.stepj()) 
        {
          ...
          // Except possibly for the diagona...
          if (M1::munit && !M2::munit) {
            m2.diag().SetAllTo(typename M2::value_type(1));
          }
        }
        // This next option is a common one where the two triangles
        // don't interfere.  It is the result of the statement:
        // m.UpperTri() = m.LowerTri().Transpose();
        else if (m1.stepi() == m2.stepj() && m1.stepj() == m2.stepi()) 
        {
          CallCopyU<false,lower,conj,inst,M1,M2>::call(m1,m2);
        }
        // Final option: forced to use a copy.
        else m2 = m1.copy();
      }
    }
  };
  template <bool conj, bool inst, class M1, class M2>
  struct CallCopyU<false,true,conj,inst,M1,M2> // lower = true
  {
    static inline void call(const M1& m1, M2& m2)
    {
      ...
      CallCopyU<false,false,conj,inst,M1t,M2t>::call(m1t,m2t);
    }
  };
  template <bool inst, class M1, class M2>
  struct CallCopyU<false,false,true,inst,M1,M2> // conj = true
  {
    static inline void call(const M1& m1, M2& m2)
    {
      ...
      CallCopyU<false,false,false,inst,M1c,M2c>::call(m1c,m2c);
    }
  };
  template <class M1, class M2>
  struct CallCopyU<false,false,false,true,M1,M2> // inst = true
  {
    static inline void call(const M1& m1, M2& m2)
    { InstCopy(m1.XView(),m2.XView()); }
  };
  template <class M1, class M2>
  struct CallCopyU<false,false,false,false,M1,M2> // inst = false
  {
    static inline void call(const M1& m1, M2& m2)
    { InlineCopy(m1,m2); }
  };
      ...
      const BaseMatrix_Tri<M1>& m1, BaseMatrix_Tri_Mutable<M2>& m2)
  {
    ...
    CallCopyU<checkalias,lower,M2::mconj,inst,M1,M2>::call(m1.mat(),m2.mat());
  }
