...
template <class T, int M, int N> void TestSmallMatrixArith_5(std::string label)
{
    if (showstartdone) {
        std::cout<<"Start Arith_5: M,N = "<<M<<','<<N<<std::endl;
    }
    ...
    TestMatrixArith5<T>(a1,ca1,label+" ColMajor");
    TestMatrixArith5<T>(a2,ca2,label+" RowMajor");
    ...
    TestMatrixArith5<T>(a3,ca3,label+" NonMajor");
#endif
}
