...
template <class T1, IndexStyle I1, class T2, IndexStyle I2> bool CanAddEq(
    const BandMatrixView<T1,I1>& a, const MatrixView<T2,I2>& b)
{ return false; }
...
template <class T1, IndexStyle I1, class T2, IndexStyle I2> bool CanMultEq2(
    const MatrixView<T1,I1>& a, const BandMatrixView<T2,I2>& b)
{ return false; }
...
template <class T> void TestBandMatrixArith_B()
{
  ...
  BandMatrix<T> B4c(B4);
  BandMatrix<T> B4d(B4);
  BandMatrix<T> B15(a1,1,N-1);
  BandMatrix<T> B16(a1,3,N-1);
  ...
  for(size_t i=0;i<b.size();i++) {
    ...
    if (doallarith) {
	  ...
	  a1.View(),b1,ca1.View(),cbi.View(),"SquareM/Band");
      TestMatrixArith<T,Matrix<T>,Matrix<complex<T> > >(
	  a3.View(),b1,ca3.View(),cbi.View(),"NonSquareM/Band");
      TestMatrixArith<T,Matrix<T>,Matrix<complex<T> > >(
	  a4.View(),b1,ca4.View(),cbi.View(),"DegenerateM/Band");
    }
#endif
  }
}
