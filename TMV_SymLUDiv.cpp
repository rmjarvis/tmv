...
#undef XD
  ...
  // MJ: This is slower for row major, even with blocking, so write the
...
#ifdef XDEBUG
    Matrix<T> L(N,N,T(0));
    LowerTriMatrixViewOf(L) = A.LowerTri();
  ...
  template <class T> void DoSymLU_BlockUpdate(
      const SymMatrixView<T>& A, const GenMatrix<T>& L,
      const GenMatrix<T>& U)
  {
    // Do A -= L * U where LU is taken to be symmetric (or hermitian as
    // appropriate), so only the lower half of A is updated

    TMVAssert(A.size() == L.colsize());
    TMVAssert(A.size() == U.rowsize());
    TMVAssert(L.rowsize() == U.colsize());
    TMVAssert(A.uplo()==Lower);

    const size_t nb = SYM_LU_BLOCKSIZE1;
    size_t N = A.size();

    if (N <= SYM_LU_BLOCKSIZE2) {
      if (A.iscm()) 
	for(size_t j=0;j<N;j++) 
	  A.col(j,j,N) -= L.Rows(j,N) * U.col(j);
      else 
	for(size_t j=0;j<N;j++) 
	  A.row(j,0,j+1) -= L.row(j) * U.Cols(0,j+1);
    } else {
      size_t k = N/2;
      if (k > nb) k = k/nb*nb;
      DoSymLU_BlockUpdate(A.SubSymMatrix(0,k),L.Rows(0,k),U.Cols(0,k));
      A.SubMatrix(k,N,0,k) -= L.Rows(k,N) * U.Cols(0,k);
      DoSymLU_BlockUpdate(A.SubSymMatrix(k,N),L.Rows(k,N),U.Cols(k,N));
    }
  }
      ...
      size_t* P, RealType(T)& det)
  {
	...
	// Do A22 -= L21 * LD21.Adjoint()
	// This isn't a standard thing you're allowed to do, though.
	// In our case L21 * LD21t is Hermitian, but since this isn't 
	// generally true for arbitrary matrices, there is no
	// S += M1 * M2 function.  
	// So function to do this is given above.
	DoSymLU_BlockUpdate(A22,L21,LD21.Adjoint());
      ...
      size_t* P, T& det)
  {
	...
	DoSymLU_BlockUpdate(A22,L21,LD21.Transpose());
