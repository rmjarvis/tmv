...
template <class T> 
void TestSmallMatrixDet()
{
    tmv::SmallMatrix<T,1,1> m1;
    m1 << 
        6;

    tmv::SmallMatrix<T,2,2> m2;
    m2 << 
        6, 1,
        3, 2;

    tmv::SmallMatrix<T,3,3> m3;
    m3 << 
        6, 1, 2,
        3, 2, 1,
        5, 6, 7;

    tmv::SmallMatrix<T,4,4> m4;
    m4 << 
        6, 1, 2, 4,
        3, 2, 1, 8,
        5, 6, 7, 1,
    ...
    tmv::SmallMatrix<T,5,5> m9 = m5.row(0) ^ m5.row(0);

    tmv::SmallMatrix<T,5,5> m10 = m5.rowRange(0,4).transpose() * m5.rowRange(0,4);

    tmv::SmallMatrix<T,10,10> m11;
    m11 << 
        0, 2, 3, 0, 0, 2, 6, 0, 1, 0,
        0, 0, 2, 6, 0, 1, 0, 2, 1, 0,
        1, 4, 0, 0, 3, 0, 3, 0, 0, 3,
    ...
    tmv::SmallMatrix<std::complex<T>,5,5> cm1;
    cm1.realPart() = m5;
    cm1.imagPart() = m7.transpose();

    tmv::SmallMatrix<std::complex<T>,10,10> cm2;
    ...
    tmv::SmallMatrix<std::complex<T>,10,10> cm3;
...
#ifdef TEST_DOUBLE
template void TestSmallMatrixDet<double>();
#endif
#ifdef TEST_FLOAT
template void TestSmallMatrixDet<float>();
#endif
#ifdef TEST_LONGDOUBLE
template void TestSmallMatrixDet<long double>();
#endif
#ifdef TEST_INT
template void TestSmallMatrixDet<int>();
#endif
