...
template <class T, int M, int N> void TestSmallMatrixArith_6(std::string label)
{
    if (showstartdone) {
        std::cout<<"Start Arith_6: M,N = "<<M<<','<<N<<std::endl;
    }
    ...
    TestMatrixArith6<T>(a1,ca1,label+" ColMajor");
    TestMatrixArith6<T>(a2,ca2,label+" RowMajor");
    ...
    TestMatrixArith6<T>(a3,ca3,label+" NonMajor");
#endif
}
