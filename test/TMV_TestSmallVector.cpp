...
template <class T> static void TestSmallVectorReal()
{
  ...
  for (int i=0; i<N/2; ++i) v2b[i] = T(i + 1000);
  ...
  double w1[NN] = {3.3,1.2,5.4,-1.2,4.3,-9.4,0.,-2.,4.,-11.5,-12.,14.,33.,1.,-9.3,-3.9,4.9,10.,-31.,1.e-33};

  tmv::Vector<double> vw1(NN,w1);
  tmv::SmallVector<T,NN> w(vw1);
...
template <class T> static void TestSmallVectorComplex()
{
  ...
  double rw1[20] = {3.3,1.2,5.4,-1.2,4.3,-9.4,0.,-2.,4.,-11.5,-12.,14.,33.,1.,-9.3,-3.9,4.9,10.,-31.,1.e-33};
  double iw1[20] = {1.4,9.8,-0.2,-8.6,3.0,-4.4,3.,9.,-1.9,-11.4,11.1,-140.,-23.,11.,5.2,-3.8,4.9,99.,-71.,-0.5};

  tmv::SmallVector<double,NN> rw(rw1);
  tmv::SmallVector<double,NN> iw(iw1);
...
template <class T> static void TestSmallVectorArith()
{
  tmv::SmallVector<T,NN> a;
  for(int i=0;i<NN;++i) a(i) = i+10.;
  tmv::SmallVector<T,NN> b;
  for(int i=0;i<NN;++i) b(i) = -3.*i+2.;

  tmv::SmallVector<std::complex<T>,NN> ca = a*std::complex<T>(2,-1);;
  tmv::SmallVector<std::complex<T>,NN> cb = b*std::complex<T>(-5,1);

  tmv::SmallVectorView<T,NN> aa = a.View();
  tmv::SmallVectorView<T,NN,1,false,tmv::FortranStyle> af = aa;
  tmv::SmallVector<T,10*NN> a10;
  tmv::SmallVectorView<T,NN,10> as = a10.SubVector(0,10*NN,10);
  tmv::SmallVectorView<T,NN,10,false,tmv::FortranStyle> ag = as;
  
  tmv::SmallVectorView<T,NN> bb = b.View();
  tmv::SmallVectorView<T,NN,1,false,tmv::FortranStyle> bf = bb;
  tmv::SmallVector<T,10*NN> b10;
  tmv::SmallVectorView<T,NN,10> bs = b10.SubVector(0,10*NN,10);
  tmv::SmallVectorView<T,NN,10,false,tmv::FortranStyle> bg = bs;

  tmv::SmallVectorView<std::complex<T>,NN> caa = ca.View();
  tmv::SmallVectorView<std::complex<T>,NN,1,false,tmv::FortranStyle> caf = caa;
  tmv::SmallVector<std::complex<T>,10*NN> ca10;
  tmv::SmallVectorView<std::complex<T>,NN,10> cas = ca10.SubVector(0,10*NN,10);
  tmv::SmallVectorView<std::complex<T>,NN,10,false,tmv::FortranStyle> cag = cas;
  
  tmv::SmallVectorView<std::complex<T>,NN> cbb = cb.View();
  tmv::SmallVectorView<std::complex<T>,NN,1,false,tmv::FortranStyle> cbf = cbb;
  tmv::SmallVector<std::complex<T>,10*NN> cb10;
  tmv::SmallVectorView<std::complex<T>,NN,10> cbs = cb10.SubVector(0,10*NN,10);
  tmv::SmallVectorView<std::complex<T>,NN,10,false,tmv::FortranStyle> cbg = cbs;
...
template <class T> static void TestSmallVectorIO()
{
  ...
  //system("rm tmvtest_smallvector_io.dat");
#endif
}
