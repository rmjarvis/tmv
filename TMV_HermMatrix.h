...
//---------------------------------------------------------------------------
  ...
  //
  // Get Rid of ConjIt<T> for non complex T:
  //
  template <class T, class IT> inline 
    SymMatrixView<T,typename IT::ConjIT::ConjIT> Regularize(
	const SymMatrixView<T,IT>& x)
    { return x; }
  template <class T> 
    inline SymMatrixView<T,It<T> > Regularize(
	const SymMatrixView<T,ConjIt<T> >& x)
    { return x.Conjugate(); }
  template <class T> 
    inline SymMatrixView<complex<T>,ConjIt<complex<T> > > Regularize(
	const SymMatrixView<complex<T>,ConjIt<complex<T> > >& x)
    { return x; }
