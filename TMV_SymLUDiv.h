...
namespace tmv {
  ...
  template <class T1, class T2> void HermLU_RDivEq(
      const GenSymMatrix<T1>& L, const GenVector<T1>& xD, const size_t* P, 
      const MatrixView<T2>& m);
  template <class T1, class T2> void SymLU_LDivEq(
      const GenSymMatrix<T1>& L, const GenVector<T1>& xD, const size_t* P, 
      const MatrixView<T2>& m);
  template <class T1, class T2> void SymLU_RDivEq(
      const GenSymMatrix<T1>& L, const GenVector<T1>& xD, const size_t* P, 
      const MatrixView<T2>& m);

#define CT complex<T>
  template <class T> inline void HermLU_LDivEq(
      const GenSymMatrix<CT>& L, const GenVector<CT>& xD, const size_t* P, 
      const MatrixView<T>& m)
  { TMVAssert(false); }
  template <class T> inline void HermLU_RDivEq(
      const GenSymMatrix<CT>& L, const GenVector<CT>& xD, const size_t* P, 
      const MatrixView<T>& m)
  { TMVAssert(false); }
...
#undef CT
    ...
    virtual public Divider<T> 
  {
    ...
    public :
      ...
      HermLUDiv(const GenSymMatrix<T>& A, bool _inplace);
      ~HermLUDiv();
...
#include "TMV_AuxAllDiv.h"
      ...
      const size_t* GetP() const { return P; }
      const GenSymMatrix<T>& GetLL() const { return LLx; }
      const GenVector<T>& GetxD() const { return xD; }

      inline std::string Type() const
      { return std::string("HermLUDiv<") + tmv::Type(T()) + ">"; }
    ...
    virtual public Divider<T> 
  {
    ...
    public :
      ...
      { 
	TMVAssert(LLx.size() == m.colsize());
	SymLU_LDivEq(LLx,xD,P,m); 
      } 

      template <class T1> inline void DoRDivEq(const MatrixView<T1>& m) const 
      {
	TMVAssert(LLx.size() == m.rowsize());
	SymLU_RDivEq(LLx,xD,P,m); 
      } 
