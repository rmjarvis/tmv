...
namespace tmv {
  ...
  template <class T1, class Ta, class Tx, class T2, class Ty>
	...
	Ty temp;
	// temp = A.row(i,j1,j2) * x.SubVector(j1,j2);
	if (A.isconj())
	  if (x.isconj())
  ...
  template <class T1, class Ta, class Tx, class T2, class Ty>
	...
	if (*xj != Tx(0)) {
	  ...
	  // y.SubVector(i1,i2) += ax * A.col(j,i1,i2);
  ...
  template <class T1, class Ta, class Tx, class T2, class Ty>
	...
	y *= beta;
	for(int k=-A.nlo(); k<=A.nhi(); ++k) {
	  // y.SubVector(i1,i2) += alpha * DiagMatrixViewOf(A.diag(k)) * 
	  //     x.SubVector(j1,j2);
	  if (A.isconj())
	    if (x.isconj())
      ...
      const T beta, const VectorView<T>& y)
  {
    ...
    if (beta == T(0)) {
      y.Zero();
      for(int k=-A.nlo(); k<=A.nhi(); ++k) {
	y.SubVector(i1,i2) += DiagMatrixViewOf(A.diag(k)) * x.SubVector(j1,j2);
	if (k<0) --i1; else ++j1;
