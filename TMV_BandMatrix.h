...
namespace tmv {
    ...
    public GenBandMatrix<T>
  {
	...
	if (nlo() == mcomp.nlo() && nhi() == mcomp.nhi())
	  mcomp.AssignTo(*this);
	else {
	  Zero();
...
#define NEW_SIZE(cs,rs,lo,hi) \
      ...
      itssi(lo+hi) DEFFIRSTLAST(itsm,itsm+itslen)
	...
	itscs(rhs.itscs), itsrs(rhs.itsrs),
	itsnlo(rhs.itsnlo), itsnhi(rhs.itsnhi),
	itssi(rhs.itssi) DEFFIRSTLAST(itsm,itsm+itslen)
      {
	TMVAssert(S == RowMajor);
...
#undef NEW_SIZE
	...
	return BandMatrixView<T>(ptr(),rowsize(),colsize(),nhi(),nlo(),
      ...
      inline int diagstep() const { return itssi+1; }
...
#define NEW_SIZE(cs,rs,lo,hi) \
      ...
      itsm(new T[itslen]), itscs(cs), itsrs(rs), itsnlo(lo), itsnhi(hi), \
      itssj(lo+hi) DEFFIRSTLAST(itsm,itsm+itslen)
	...
	itsnlo(rhs.itsnlo), itsnhi(rhs.itsnhi),
	itssj(rhs.itssj) DEFFIRSTLAST(itsm,itsm+itslen)
      { memmove(itsm,rhs.itsm,itslen*sizeof(T)); }
...
#undef NEW_SIZE
	...
	return BandMatrixView<T>(ptr(),rowsize(),colsize(),nhi(),nlo(),
      ...
      inline int stepj() const { return itssj; }
      inline int diagstep() const { return itssj+1; }
