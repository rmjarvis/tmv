...
template <class T, tmv::UpLoType U, tmv::StorageType S> void TestBasicSymMatrix()
{
  ...
  SymMatrix<complex<T>,U,S> s1(N);
  SymMatrix<complex<T>,U,S> s2(N);
  HermMatrix<complex<T>,U,S> h1(N);
  HermMatrix<complex<T>,U,S> h2(N);
  SymMatrix<complex<T>,U,S,FortranStyle> s1f(N);
  SymMatrix<complex<T>,U,S,FortranStyle> s2f(N);
  HermMatrix<complex<T>,U,S,FortranStyle> h1f(N);
  HermMatrix<complex<T>,U,S,FortranStyle> h2f(N);
  ...
  for (size_t i=0, k=1; i<N; ++i) for (size_t j=0; j<N; ++j, ++k) {
    complex<T> value(k,2*k);
    complex<T> hvalue = i==j ? complex<T>(k,0) : value;
    if (i<=j) {
  ...
  SymMatrixView<complex<T> > s1v = s1.View();
  ConstSymMatrixView<complex<T> > s1cv = s1.View();
  SymMatrixView<complex<T> > s2v = s2.View();
  ConstSymMatrixView<complex<T> > s2cv = s2.View();
  SymMatrixView<complex<T>,FortranStyle> s1fv = s1f.View();
  ConstSymMatrixView<complex<T>,FortranStyle> s1fcv = s1f.View();
  SymMatrixView<complex<T>,FortranStyle> s2fv = s2f.View();
  ConstSymMatrixView<complex<T>,FortranStyle> s2fcv = s2f.View();
  SymMatrixView<complex<T> > h1v = h1.View();
  ConstSymMatrixView<complex<T> > h1cv = h1.View();
  SymMatrixView<complex<T> > h2v = h2.View();
  ConstSymMatrixView<complex<T> > h2cv = h2.View();
  SymMatrixView<complex<T>,FortranStyle> h1fv = h1f.View();
  ConstSymMatrixView<complex<T>,FortranStyle> h1fcv = h1f.View();
  SymMatrixView<complex<T>,FortranStyle> h2fv = h2f.View();
  ConstSymMatrixView<complex<T>,FortranStyle> h2fcv = h2f.View();
  const SymMatrix<complex<T>,U,S>& s1x = s1;
  const SymMatrix<complex<T>,U,S>& s2x = s2;
  const HermMatrix<complex<T>,U,S>& h1x = h1;
  const HermMatrix<complex<T>,U,S>& h2x = h2;
  const SymMatrix<complex<T>,U,S,FortranStyle>& s1fx = s1f;
  const SymMatrix<complex<T>,U,S,FortranStyle>& s2fx = s2f;
  const HermMatrix<complex<T>,U,S,FortranStyle>& h1fx = h1f;
  const HermMatrix<complex<T>,U,S,FortranStyle>& h2fx = h2f;

  for (size_t i=0, k=1; i<N; ++i) for (size_t j=0; j<N; ++j, ++k) {
    complex<T> value(k,2*k);
    complex<T> hvalue = i==j ? complex<T>(k,0) : value;
    if (showtests) cout<<"i,j = "<<i<<','<<j<<endl;
    if (i<=j) {
  ...
  SymMatrix<complex<T>,U,S> s3(N);
  s3 = s1+s2;

  HermMatrix<complex<T>,U,S> h3(N);
  h3 = h1+h2;
  ...
  Matrix<complex<T> > m1 = s1;
  Matrix<complex<T> > n1 = h1;
  for (size_t i=0; i<N; ++i) for (size_t j=0; j<N; ++j) {
    Assert(s1(i,j) == m1(i,j),"SymMatrix -> Matrix");
    Assert(h1(i,j) == n1(i,j),"HermMatrix -> Matrix");
  }
  Assert(s1 == SymMatrix<complex<T>,U,S>(m1),"Matrix -> SymMatrix");

  SymMatrix<complex<T>,tmv::Upper,tmv::RowMajor> sur = s1;
  ...
  HermMatrix<complex<T>,tmv::Upper,tmv::RowMajor> hur = h1;
