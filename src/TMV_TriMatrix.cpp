...
#define RT RealType(T)
  ...
  template <class T> T GenUpperTriMatrix<T>::cref(int i, int j) const
  {
  ...
  template <class T> T GenLowerTriMatrix<T>::cref(int i, int j) const
  {
    TMVAssert(i < int(size()));
    TMVAssert(j < int(size()));
    if (i==j && isunit()) return T(1);
  ...
  template <class T> RT GenUpperTriMatrix<T>::NormSq(const RT scale) const
  {
    const int N = size();
    RT sum(0);
    if (isrm()) 
  ...
  template <class T> static RT NonLapNormInf(
      const GenUpperTriMatrix<T>& m)
  { 
    RT max(0);
    const int N = m.size();
  ...
  template <class T> RT GenUpperTriMatrix<T>::MaxAbsElement() const
  {
#ifdef XLAP
  ...
  template <class T> RT GenUpperTriMatrix<T>::Norm1() const
  {
#ifdef XLAP
  ...
  template <class T> RT GenUpperTriMatrix<T>::NormInf() const
  {
#ifdef XLAP
  ...
  template <class T> RT GenUpperTriMatrix<T>::NormF() const
  {
#ifdef XLAP
  ...
  template <class T> RT GenUpperTriMatrix<T>::DoNorm2() const
  { return Matrix<T>(*this).DoNorm2(); }

  template <class T> RT GenUpperTriMatrix<T>::DoCondition() const
  ...
  template <class T> RT GenUpperTriMatrix<T>::LogDet(T* sign) const
  { 
    if (isunit()) {
      if (sign) *sign = T(1);
      return RT(0);
  ...
  template <bool conj, bool rm, bool compact, bool th, class T> 
    static void DoWrite(std::ostream& os, const GenUpperTriMatrix<T>& m,
	RT thresh)
  {
    ...
    public ReadError
  {
    public :
      ...
      UpperTriMatrixReadError(int _i, int _j,
	...
	is(_is), iseof(_is.eof()), isbad(_is.bad()) {}
      UpperTriMatrixReadError(std::istream& _is, 
	  char _e, char _g) throw() :
	ReadError("UpperTriMatrix."),
	...
	is(_is), iseof(_is.eof()), isbad(_is.bad()) {}
      UpperTriMatrixReadError(const GenUpperTriMatrix<T>& _m,
  ...
  template <bool conj, bool rm, bool compact, bool th, class T> 
    static void DoWrite(std::ostream& os, const GenLowerTriMatrix<T>& m,
	RT thresh)
  {
    const T* mrowi = m.cptr();
    const int sj = rm?1:m.stepj();
    ...
    public ReadError
  {
	...
	exp(0), got(0), unitgot(T(1)), s(_m.size()),
	is(_is), iseof(_is.eof()), isbad(_is.bad()) {}
      LowerTriMatrixReadError(std::istream& _is) :
	ReadError("LowerTriMatrix."),
	i(0), j(0), m(0), exp(0), got(0), unitgot(T(1)), s(0),
	is(_is), iseof(_is.eof()), isbad(_is.bad()) {}
      LowerTriMatrixReadError(int _i, int _j,
	...
	exp(_e), got(_g), unitgot(T(1)), s(_m.size()),
	is(_is), iseof(_is.eof()), isbad(_is.bad()) {}
      LowerTriMatrixReadError(std::istream& _is, char _e, char _g) :
	ReadError("LowerTriMatrix."),
	i(0), j(0), m(0), exp(_e), got(_g), unitgot(T(1)), s(0),
	is(_is), iseof(_is.eof()), isbad(_is.bad()) {}
      LowerTriMatrixReadError(int _i, int _j,
	...
	is(_is), iseof(_is.eof()), isbad(_is.bad()) {}
      ...
      LowerTriMatrixReadError(const LowerTriMatrixReadError<T>& rhs) :
	i(rhs.i), j(rhs.j), m(rhs.m), exp(rhs.exp), got(rhs.got),
	unitgot(rhs.unitgot), s(rhs.s),
	is(rhs.is), iseof(rhs.iseof), isbad(rhs.isbad) {}
