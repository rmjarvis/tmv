...
namespace tmv {
  ...
  template <bool ca, class T, class Ta, class Tb> 
    static void ColMult(int M, const Tb b00,
	const Ta* A, T* C PARAMFIRSTLAST(T) )
    {
      for(int i=M/8;i;--i) {
#ifdef TMVFLDEBUG
	TMVAssert(C >= _first);
  ...
  template <bool ca, bool cb, class T, class Ta, class Tb> 
	...
	const int Ask, const int Bsj, const int Csj PARAMFIRSTLAST(T) )
    {
      if (K > N) {
	int K1 = K/2;
	...
	ColMult<ca>(M,(cb ? CONJ(B[0]) : B[0]),A,C FIRSTLAST1(_first,_last));
      }
    }
...
#pragma omp for nowait 
	    ...
	    for(int ij=0;ij<int(MbNb);ij++) {
	      ...
	      Ctemp.Zero();
	      RecursiveCCCMultMM<ca,cb>(MB,NB,K,Ap+ii,Bp+jj*Bsj,Ct,
		  Ask,Bsj,Ctsj FIRSTLAST1(Ctemp.first,Ctemp.last));
...
#pragma omp for nowait schedule(guided)
	    ...
	    for(int i=0;i<int(Mb);i++) {
	      const int ii = i*MB;
	      Ctemp.Zero();
	      RecursiveCCCMultMM<ca,cb>(MB,N2,K,Ap+ii,Bp+N1*Bsj,Ct,
		  Ask,Bsj,Ctsj FIRSTLAST1(Ctemp.first,Ctemp.last));
...
#pragma omp for nowait schedule(guided)
	    ...
	    for(int j=0;j<int(Nb);j++) {
	      const int jj = j*NB;
	      Ctemp.Zero();
	      RecursiveCCCMultMM<ca,cb>(M2,NB,K,Ap+M1,Bp+jj*Bsj,Ct, 
		  Ask,Bsj,Ctsj FIRSTLAST1(Ctemp.first,Ctemp.last));
...
#pragma omp single
	    ...
	    {
	      Ctemp.Zero();
	      RecursiveCCCMultMM<ca,cb>(M2,N2,K,Ap+M1,Bp+N1*Bsj,Ct, 
		  Ask,Bsj,Ctsj FIRSTLAST1(Ctemp.first,Ctemp.last));
