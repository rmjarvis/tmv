...
#define DC_LIMIT 32
  ...
  // Both the divide step (recursing on SV_Decompose_From_Bidiagonal_DC) 
  ...
  {
    //std::cout<<"start FindDCV: k = "<<k<<endl;
    ...
    for(int iter = 0; iter < MAXITER; iter++) {
      ...
      T dt; // = d(tau)
      if (f*f1 < T(0)) {
        // Then we are too close to the pole.  Try doubling the distance 
        // from the pole.
        if (f < T(0)) {
      ...
      if (iter == MAXITER-1) {
        ...
        std::cout<<"Unsuccessful after "<<MAXITER<<" iterations.\n";
        exit(1);
      }
#endif
    }
    ...
    for(int iter = 0; iter < MAXITER; iter++) {
      //std::cout<<"k = "<<k<<", loop 2: iter = "<<iter<<std::endl;
      ...
      if (!threepoles) { // The normal case - only 2 poles used for approx.
        ...
        if (h*h2 > T(0)) {
          ...
          //std::cout<<"k = "<<k<<", loop 3: iter = "<<iter3<<std::endl;
  ...
  template <class T> static void FindDCSingularValues(
...
#pragma omp for
      for(int k=0;k<N;k++) {
        Sk = FindDCSingularValue(k,N,rho,D.cptr(),z.cptr(),
      ...
      bool UisI, bool VisI)
  {
    ...
    // So I always use QR for the S only calculation.
      ...
      if (U) {
        ...
        SV_Decompose_From_Bidiagonal_DC<RT>(0,D2,E2,V2.View(),
            false,V);
      }
        ...
        if (U || V) {
          ...
          if (V) {
            ...
            for(int j=0;j<N;j++) {
              ...
              dbgcout<<"Norm(y.row("<<j<<")) => "<<Norm(W.col(j))<<endl;
            }
            // V = Y * V
            dbgcout<<"Before V = Y * V\n";
            dbgcout<<"Norm(VVt-1) (Rows 0-N) = "<<Norm(V->Rows(0,N)*V->Rows(0,N).Adjoint()-T(1))<<endl;
            dbgcout<<"Y = "<<W.Transpose()<<endl;
            dbgcout<<"YYt = "<<W.Transpose()*W<<endl;
            dbgcout<<"YtY = "<<W*W.Transpose()<<endl;
            dbgcout<<"Norm(YYt-1) = "<<Norm(W.Transpose()*W-T(1))<<endl;
            dbgcout<<"Norm(YtY-1) = "<<Norm(W*W.Transpose()-T(1))<<endl;
    ...
    if (U && V) {
      ...
      if (Norm(A2-A0) > 0.001*normA0 || 
          (V->colsize()>=V->rowsize() && Norm(V->Adjoint()*(*V)-T(1)) > 0.001*normA0) ||
          (V->rowsize()>=V->colsize() && Norm((*V)*V->Adjoint()-T(1)) > 0.001*normA0) ||
          (U->colsize()>=U->rowsize() && Norm(U->Adjoint()*(*U)-T(1)) > 0.001*normA0)) {
        ...
        cerr<<"U-U_QR = "<<*U-U_QR<<endl; cerr<<"Norm(U-U_QR) = "<<Norm(*U-U_QR)<<endl; 
        cerr<<"UBV = "<<A0<<endl;
        cerr<<"USV = "<<A2<<endl;
        abort();
      }
