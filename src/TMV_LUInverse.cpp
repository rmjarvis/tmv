...
namespace tmv {
  ...
  template <class T> static void NonLapLUInverse(const MatrixView<T>& minv)
  {
    // m = P L U
    // m^-1 = U^-1 L^-1 Pt
...
#ifdef ALAP
  template <class T> static inline void LapLUInverse(const MatrixView<T>& minv)
  ...
  template <class T, class T1> void LU_Inverse(const GenMatrix<T>& LUx,
      const int* P, const MatrixView<T1>& minv)
  {
	...
	NonLapLUInverse(minv);
#endif
	//cout<<"before permute: "<<minv<<endl;
	minv.ReversePermuteCols(P);
	//cout<<"after permute: "<<minv<<endl;
      }
    }
