...
#define MM_BLOCKSIZE 64
...
#ifdef ELAP
  template <class T, class Ta, class Tb> static inline void LapTriDiagMultMM(
      const GenBandMatrix<Ta>& A, const GenMatrix<Tb>& B, 
...
#ifdef INST_DOUBLE
  template <> void LapTriDiagMultMM(
      ...
      const int beta, const MatrixView<double>& C)
  {
    ...
    double xbeta(beta);
    int ldC = C.stepj();
    LAPNAME(dlagtm) (LAPCM LAPCH_T,LAPV(n),LAPV(nrhs),
      ...
      const int beta, const MatrixView<std::complex<double> >& C)
  {
    ...
    double xbeta(beta);
    int ldC = C.stepj();
    LAPNAME(zlagtm) (LAPCM B.isconj()?LAPCH_CT:LAPCH_T,LAPV(n),LAPV(nrhs),
        LAPV(a),LAPP(A.cptr()+A.stepj()),
        LAPP(A.cptr()),LAPP(A.cptr()+A.stepi()),
        LAPP(B.cptr()),LAPV(ldB),LAPV(xbeta),LAPP(C.ptr()),LAPV(ldC) LAP1);
  }
      ...
      const int beta, const MatrixView<float>& C)
  {
    ...
    float xbeta(beta);
    int ldC = C.stepj();
    LAPNAME(slagtm) (LAPCM LAPCH_T,LAPV(n),LAPV(nrhs),
        LAPV(a),LAPP(A.cptr()+A.stepj()),
        LAPP(A.cptr()),LAPP(A.cptr()+A.stepi()),
        LAPP(B.cptr()),LAPV(ldB),LAPV(xbeta),LAPP(C.ptr()),LAPV(ldC) LAP1);
  }
      ...
      const int beta, const MatrixView<std::complex<float> >& C)
  {
    ...
    float xbeta(beta);
    int ldC = C.stepj();
    LAPNAME(clagtm) (LAPCM B.isconj()?LAPCH_CT:LAPCH_T,LAPV(n),LAPV(nrhs),
        LAPV(a),LAPP(A.cptr()+A.stepj()),
        LAPP(A.cptr()),LAPP(A.cptr()+A.stepi()),
        LAPP(B.cptr()),LAPV(ldB),LAPV(xbeta),LAPP(C.ptr()),LAPV(ldC) LAP1);
  }
  ...
  template <bool add, class T, class Ta, class Tb> static inline void DoTriDiagMultMM(
      const GenBandMatrix<Ta>& A, const GenMatrix<Tb>& B,
      const MatrixView<T>& C)
  {
#ifdef ELAP
  ...
  template <bool add, class T, class Ta, class Tb> static void TriDiagMultMM(
      const T alpha, const GenBandMatrix<Ta>& A, const GenMatrix<Tb>& B,
      const MatrixView<T>& C)
  {
    if (alpha == T(1) && A.isdm()) {
      ...
      const MatrixView<T>& C)
  {
    ...
    if (Norm(C2-C) > 0.001*(ABS(alpha)*Norm(A0)*Norm(B0)+
      ...
      cerr<<"C = "<<TypeText(C)<<"  "<<C.cptr()<<"  "<<C0<<endl;
      cerr<<"--> C = "<<C<<endl;
      cerr<<"C2 = "<<C2<<endl;
      abort();
    }
