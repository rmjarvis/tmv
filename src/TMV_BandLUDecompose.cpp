...
namespace tmv {
  ...
  template <class T> static void NonLapBandLU_Decompose(
  ...
  template <class T> static void NonLapTriDiagLU_Decompose(
      const BandMatrixView<T>& A, int* P, T& det)
  {
    ...
    for (int j=0; j<N-1; ++j,++Ujj,++Lj,++Uj1,++Uj2,++Pj)
    {
      ...
      if (pivot) {
        ...
        __TMV_SWAP(*Lj,*Ujj);
        if (j+1<N) __TMV_SWAP(*(Ujj+1),*Uj1);
        if (j+2<N) __TMV_SWAP(*(Uj1+1),*Uj2);
        *Pj = j+1;
        det = -det;
      } else *Pj = j;
...
#ifdef LAP
  template <class T> static inline void LapBandLU_Decompose(
      const BandMatrixView<T>& A, int* P, T& det)
  { NonLapBandLU_Decompose(A,P,det); }
#ifdef INST_DOUBLE
  template <> void LapBandLU_Decompose(
  ...
  template <class T> static inline void LapTriDiagLU_Decompose(
      const BandMatrixView<T>& A, int* P, T& det)
  { NonLapTriDiagLU_Decompose(A,P,det); }
#ifdef INST_DOUBLE
  template <> void LapTriDiagLU_Decompose(
      const BandMatrixView<double>& A, int* P, double& det)
  {
    TMVAssert(A.IsSquare());
    TMVAssert(A.isdm());
  ...
  {
#ifdef XDEBUG
    //cout<<"BandLU_Decompose: A = "<<TypeText(A)<<"  "<<A<<endl;
    BandMatrix<T> A0 = A;
#endif
    ...
    if (!(Norm(AA-A0) < 0.001 * Norm(A0))) {
      cerr<<"BandLU_Decompose: A = "<<TypeText(A)<<A0<<endl;
      cerr<<"AA = "<<AA<<endl;
