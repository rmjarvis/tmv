...
namespace tmv {
  ...
  template <bool a1, bool ca, class T, class T1,  class Ta> 
    inline void DoMultEqMM(const T1 alpha, const GenDiagMatrix<Ta>& A,
    ...
    // B = alpha * A * B
  {
    ...
    TMVAssert(B.dt() == NonUnitDiag);
#ifdef XDEBUG
    //cerr<<"Start MultEqMM: alpha = "<<alpha<<endl;
  ...
  template <bool a1, bool ca, bool ub, bool cb, class T, class T1, class Ta, class Tb> 
    inline void DoAddMultMM(const T1 alpha,
  ...
  template <bool a1, bool ca, class T, class T1, class Ta, class Tb> 
    inline void DoAddMultMMa(const T1 alpha,
	const GenDiagMatrix<Ta>& A, const GenUpperTriMatrix<Tb>& B,
	const UpperTriMatrixView<T>& C)
    {
      if (B.isunit())
    ...
    // C += alpha * A * B
  {
    ...
    TMVAssert(C.dt() == NonUnitDiag);
#ifdef XDEBUG
    //cerr<<"Start AddMultMM: alpha = "<<alpha<<endl;
    //cerr<<"A = "<<Type(A)<<"  "<<A<<endl;
    //cerr<<"B = "<<Type(B)<<"  "<<B<<endl;
...
#define InstFile "TMV_DiagTriArith.inst"
#include "TMV_Inst.h"
#undef InstFile
