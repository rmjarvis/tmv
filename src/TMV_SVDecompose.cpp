...
#define RT RealType(T)
      ...
      const VectorView<RT>& E, MVP<T> V, bool SetUV)
  {
    ...
    //if (U) cout<<"U = "<<TypeText(*U)<<"  "<<*U<<endl;
    //cout<<"D = "<<TypeText(D)<<"  step "<<D.step()<<"  "<<D<<endl;
    //cout<<"E = "<<TypeText(E)<<"  step "<<E.step()<<"  "<<E<<endl;
    //if (V) cout<<"V = "<<TypeText(*V)<<"  "<<*V<<endl;
      ...
      MVP<T> V, RT& logdet, T& signdet, bool StoreU)
  {
    ...
    if (M > 5*N/3) {
      if (StoreU) {
        ...
        if (N > 1) U.Rows(0,N).LowerTri().OffDiag().Zero();
