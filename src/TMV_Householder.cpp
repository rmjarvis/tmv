...
#define RT RealType(T)
  ...
  {
    //std::cout<<"Start Householder_LMult"<<std::endl;
      ...
      const MatrixView<T2>& m)
  {
    ...
    if (m.iscm()) {
      ...
      Y.Rows(0,N).LowerTri(UnitDiag).Adjoint() * m.Rows(0,N);
      ZYtm += Y.Rows(N,M).Adjoint() * m.Rows(N,M);
      ZYtm = Z * ZYtm;
      m.Rows(0,N) -= Y.Rows(0,N).LowerTri(UnitDiag) * ZYtm;
      ...
      const MatrixView<T2>& m)
  {
    ...
    if (m.isrm()) {
      ...
      m.Rows(0,N) -= Y.Rows(0,N).LowerTri(UnitDiag) * ZtYtm;
      ...
      const MatrixView<T>& m)
  {
    ...
    -Z * Y.Rows(0,N).LowerTri(UnitDiag).Adjoint();
    Y.Rows(N,M) *= temp;
    Y.Rows(0,N) = Y.Rows(0,N).LowerTri(UnitDiag) * temp;
