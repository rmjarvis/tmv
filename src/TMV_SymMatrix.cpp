...
#define RT RealType(T)
...
#ifdef XDEBUG
      if (Norm(M-*this) > 1.e-5*MAX(RT(1),Norm(M))) {
        cerr<<"SwapRowsCols: i1,i2 = "<<i1<<','<<i2<<endl;
        cerr<<"M0 = "<<TypeText(*this)<<"  "<<M0<<endl;
        cerr<<"M = "<<M<<endl;
        cerr<<"S = "<<*this<<endl;
        abort();
      }
...
#endif
    double norm = LAPNAME(dlansy) (LAPCM LAPV(cc),
      ...
      const GenSymMatrix<std::complex<double> >& m)
  {
    ...
    LAPNAME(zlanhe) (LAPCM LAPV(cc),
        (m.iscm() == (m.uplo()==Upper) ? LAPCH_UP : LAPCH_LO),
        LAPV(N),LAPP(m.cptr()),LAPV(lda) LAPWK(work.get()) LAP1 LAP1) :
    LAPNAME(zlansy) (LAPCM LAPV(cc),
        (m.iscm() == (m.uplo()==Upper) ? LAPCH_UP : LAPCH_LO),
        LAPV(N),LAPP(m.cptr()),LAPV(lda) LAPWK(work.get()) LAP1 LAP1);
    return norm;
  }
...
#endif
    float norm = LAPNAME(slansy) (LAPCM LAPV(cc),
        (m.iscm() == (m.uplo()==Upper) ? LAPCH_UP : LAPCH_LO),
        LAPV(N),LAPP(m.cptr()),LAPV(lda) LAPWK(work.get()) LAP1 LAP1);
    return norm;
  }
...
#endif
    float norm = m.isherm() ?
    LAPNAME(clanhe) (LAPCM LAPV(cc),
        (m.iscm() == (m.uplo()==Upper) ? LAPCH_UP : LAPCH_LO),
        LAPV(N),LAPP(m.cptr()),LAPV(lda) LAPWK(work.get()) LAP1 LAP1) :
    LAPNAME(clansy) (LAPCM LAPV(cc),
        (m.iscm() == (m.uplo()==Upper) ? LAPCH_UP : LAPCH_LO),
        LAPV(N),LAPP(m.cptr()),LAPV(lda) LAPWK(work.get()) LAP1 LAP1);
    return norm;
  }
