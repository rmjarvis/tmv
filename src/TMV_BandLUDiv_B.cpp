...
namespace tmv {
  ...
  template <> inline void LapBandLU_LDivEq(
      ...
      const size_t* P, const MatrixView<std::complex<double> >& m)
  {
    TMVAssert(m.colsize() == LUx.colsize());
    TMVAssert(LUx.IsSquare());
    TMVAssert(m.rowsize() > 0);
  ...
  template <> inline void LapBandLU_LDivEq(
      ...
      const size_t* P, const MatrixView<std::complex<float> >& m)
  {
    TMVAssert(m.colsize() == LUx.colsize());
    TMVAssert(LUx.IsSquare());
    TMVAssert(m.rowsize() > 0);
  ...
  template <bool cm, bool ca, bool ua, class T, class Ta> 
	...
	const GenBandMatrix<Ta>& A, const VectorView<T>& b)
    {
      ...
      size_t i2=std::min(size_t(A.nlo())+1,A.colsize());
      ...
      const GenBandMatrix<Ta>& A, const VectorView<T>& b, DiagType dt)
  {
	...
	if (i2==0) return;
	  ...
	  if (dt == NonUnitDiag) 
	    *b.ptr() /= A.isconj()?CONJ(*A.cptr()):*A.cptr();
	}
	else if (i2 == N) 
	...
	if (i1==N) return;
	  ...
	  if (dt == NonUnitDiag) {
	    T* bend = b.ptr() + (N-1)*b.step();
	    const Ta* Aend = A.cptr() + (N-1)*(A.stepi()+A.stepj());
	    *bend /= A.isconj()?CONJ(*Aend):*Aend;
	    // b(N-1) /= A(N-1,N-1);
	  }
	}
