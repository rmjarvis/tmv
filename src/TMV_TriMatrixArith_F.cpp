...
#define TRI_MM_BLOCKSIZE2 32
      ...
      T alpha, const GenUpperTriMatrix<Ta>& A, const UpperTriMatrixView<T>& B)
  {
    ...
    //cerr<<"CMultEqMM Upper\n";
    //cerr<<"alpha = "<<alpha<<endl;
    //cerr<<"A = "<<Type(A)<<" = "<<A<<endl;
    //cerr<<"B = "<<Type(B)<<" = "<<B<<endl;
    //cerr<<"alpha*A*B = "<<B2<<std::endl;
#endif
	...
	for(size_t j=N-1;j>0;--j) {
      ...
      if (SameStorage(A,B)) {
	Vector<T> Btemp(N);
	for(size_t jj=N,j=jj-1;jj>0;--jj,--j) { // jj = j+1
      ...
      T alpha, const GenLowerTriMatrix<Ta>& A, const LowerTriMatrixView<T>& B)
  {
    ...
    if (A.isunit()) {
      if (B.isunit()) {
	for(size_t j=N-1,jj=N; jj>0; --j,--jj) {
	...
	for(size_t j=N-1,jj=N; jj>0; --j,--jj) 
      ...
      TMVAssert(!B.isunit());
      for(size_t j=N-1,jj=N; jj>0; --j,--jj,Ajj-=Ads) {
	B.SubMatrix(jj,N,0,jj) += A.col(j,jj,N) ^ B.row(j,0,jj);
	B.row(j,0,jj) *= A.isconj()?CONJ(*Ajj):*Ajj;
      }
    } 
    B *= alpha;
  }
  ...
  { 
	...
	if (SameStorage(A,C)) {
	  if (SameStorage(B,C)) {
	    if (C.SameAs(B)) MultEqMM(alpha,A,C);
