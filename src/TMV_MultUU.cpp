...
#define TRI_MM_BLOCKSIZE2 32
  ...
  template <class T, class Ta> static void RRMultEqMM(
    ...
    // B = alpha * A * B
  {
    ...
    TMVAssert(B.size() > 0);
#ifdef XDEBUG
    //cout<<"MultEqMM: "<<alpha<<"  "<<Type(A)<<"  "<<A<<"  "<<Type(B)<<"  "<<B<<endl;
    Matrix<Ta> A0 = A;
    Matrix<T> B0 = B;
    Matrix<T> B2 = alpha*A0*B0;
#endif
  ...
  template <class T, class Ta, class Tb> static void RRAddMultMM(
