...
#define SYM_TRIDIAG_BLOCKSIZE 64
      ...
      T& det, size_t j1=0)
  {
    ...
    T* bj = beta.ptr() + j1;
    for(size_t j=j1;j<N-1;++j,++bj) {
      *bj = Householder_Reflect(A.col(j,j+1,N),det);
      if (*bj != T(0)) 
      ...
      const VectorView<Td>& D, const VectorView<RealType(T)>& E, T& det)
  {
    ...
    Matrix<T,ColMajor> ZYtmT(N,std::min(size_t(SYM_TRIDIAG_BLOCKSIZE),Nm1));
    ...
    for(size_t j1=0;j1<Nm1;) {
      size_t j2 = std::min(Nm1,j1+SYM_TRIDIAG_BLOCKSIZE);

      if (j2 < N) {
	for(size_t j=j1,jj=0;j<j2;++j,++jj,++bj) { // jj = j-j1
	  ...
	  *bj = Householder_Reflect(u,det);
	  T* u0 = u.ptr();
	  TMVAssert(IMAG(*u0) == RealType(T)(0));
	  E(j) = REAL(*u0);
