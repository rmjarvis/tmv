...
#define DC_LIMIT 32
  ...
  {
#ifdef XDEBUG
    //cerr<<"Start NonBlockBidiag: A = "<<A<<endl;
    Matrix<T> A0(A);
#endif
    ...
    T* Uj = Ubeta.ptr();
    T* Vj = Vbeta.ptr();
    for(size_t j=0;j<N-1;++j,++Uj,++Vj) {
      *Uj = Householder_Reflect(A.SubMatrix(j,M,j,N),det);
      *Vj = Householder_Reflect(A.Transpose().SubMatrix(j+1,N,j,M),det);
    }
    *Uj = Householder_Reflect(A.col(N-1,N-1,M),det);
      ...
      const VectorView<RealType(T)>& E, T& det)
  {
    ...
    Matrix<T,RowMajor> ZYtm(std::min(size_t(BIDIAG_BLOCKSIZE),N-1),N);
    Matrix<T,ColMajor> mXtW(M,std::min(size_t(BIDIAG_BLOCKSIZE),N-1));
    ...
    for(size_t j1=0;j1<N-1;) {
      size_t j2 = std::min(N-1,j1+BIDIAG_BLOCKSIZE);
      for(size_t j=j1,jj=0;j<j2;++j,++jj,++Uj,++Vj,Dj+=Ds,++Ej) { // jj = j-j1
	...
	*Uj = bu;
	TMVAssert(IMAG(*u.cptr()) == RealType(T)(0)); 
	*Dj = REAL(*u.cptr());
	*u.ptr() = T(1);
	...
	*Vj = bv;
	TMVAssert(IMAG(*v.cptr()) == RealType(T)(0));
	*Ej = REAL(*v.cptr());
	*v.ptr() = T(1);
    ...
    *Uj = Householder_Reflect(A.col(N-1,N-1,M),det);
    ...
    TMVAssert(IMAG(Aend) == RealType(T)(0));
    *Dj = REAL(Aend);
      ...
      const VectorView<double>& E, double& det)
  {
    ...
    if (det) {
      ...
      for(size_t i=0;i<Ubeta.size();++i,++Ubi) if (*Ubi != double(0)) 
	det = -det;
      const double* Vbi = Vbeta.cptr();
      for(size_t i=0;i<Vbeta.size();++i,++Vbi) if (*Vbi != double(0)) 
	det = -det;
    }
#ifdef TIME
    gettimeofday(&tp,0);
      ...
      std::complex<double>& det)
  {
    ...
    if (det!=double(0)) {
      ...
      for(size_t i=0;i<Ubeta.size();++i,++Ubi) if (*Ubi != double(0)) {
	det *= CONJ((*Ubi)*(*Ubi))/norm(*Ubi);
      }
      const std::complex<double>* Vbi = Vbeta.cptr();
      for(size_t i=0;i<Vbeta.size();++i,++Vbi) if (*Vbi != double(0)) {
	det *= -CONJ((*Vbi)*(*Vbi))/norm(*Vbi);
      }
    }
  }
#endif
      ...
      const VectorView<float>& E, float& det)
  {
    ...
    if (det) {
      ...
      for(size_t i=0;i<Ubeta.size();++i,++Ubi) if (*Ubi != float(0)) 
	det = -det;
      const float* Vbi = Vbeta.cptr();
      for(size_t i=0;i<Vbeta.size();++i,++Vbi) if (*Vbi != float(0)) 
	det = -det;
    }
  }
  template <> inline void LapBidiagonalize(
