...
#define RT RealType(T)
  ...
  template <class T> void UnsortedHermEigen(const MatrixView<T>& U,
      const VectorView<RT>& S);
  template <class T> void UnsortedEigen(const SymMatrixView<T>& A,
      const VectorView<RT>& S);
  ...
  template <class T> void SV_Decompose(const SymMatrixView<T>& A,
      const DiagMatrixView<RT>& S, MVP<T> V);

  template <class T, class T1> void HermSV_Inverse(
      const GenMatrix<T>& U, const GenDiagMatrix<RT>& S, int kmax,
      const SymMatrixView<T1>& sinv);
  template <class T, class T1> void SymSV_Inverse(
  ...
  template <class T> void Eigen_From_Tridiagonal_QR(
      MVP<T> U, const VectorView<RT>& D, const VectorView<RT>& E);
