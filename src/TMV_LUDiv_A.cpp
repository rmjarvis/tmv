...
#define LU_BLOCKSIZE2 2
      ...
      const MatrixView<T>& A, size_t* P, T& det)
  {
	...
	LowerTriMatrixViewOf(A.SubMatrix(0,j,0,j),UnitDiag).LDivEq(
	    A.col(j,0,j));
      ...
      const MatrixView<T>& A, size_t* P, T& det)
  {
    ...
    if (R > LU_BLOCKSIZE2) {
      ...
      LowerTriMatrixViewOf(A00,UnitDiag).LDivEq(A01);
  ...
  template <> inline void LapLU_Decompose(
      const MatrixView<std::complex<double> >& A, size_t* P,
  ...
  template <> inline void LapLU_Decompose(
      const MatrixView<std::complex<float> >& A, size_t* P, 
