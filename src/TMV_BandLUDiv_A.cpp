...
namespace tmv {
      ...
      const BandMatrixView<T>& A, size_t* P, T& det)
  {
	...
	std::swap(*Lj,*Ujj);
	if (j+1<N) std::swap(*(Ujj+1),*Uj1);
	if (j+2<N) std::swap(*(Uj1+1),*Uj2);
	*Pj = j+1;
	det = -det;
      } else *Pj = j;
  ...
  template <> inline void LapBandLU_Decompose(
      const BandMatrixView<std::complex<double> >& A, size_t* P,
  ...
  template <> inline void LapBandLU_Decompose(
      const BandMatrixView<std::complex<float> >& A, size_t* P,
