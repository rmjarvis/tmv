...
#define QR_BLOCKSIZE 64
  ...
  template <class T> static void NonBlockGetQFromQR(
  ...
  template <class T> static void BlockGetQFromQR(
      const MatrixView<T>& Q, const GenVector<T>& beta)
  {
    TMVAssert(Q.colsize() >= Q.rowsize());
    TMVAssert(Q.rowsize() == beta.size());
    ...
    if (Norm(Q-Q2) > 0.001*Norm(Q)*Norm(beta)) {
      cerr<<"BlockGetQ: Q = "<<TypeText(Q)<<"  "<<Q0<<endl;
      cerr<<"beta = "<<beta<<endl;
      cerr<<"-> Q = "<<Q<<endl;
  ...
  template <class T> static inline void NonLapGetQFromQR(
      const MatrixView<T>& Q, const GenVector<T>& beta)
  {
    TMVAssert(Q.colsize() >= Q.rowsize());
    TMVAssert(Q.rowsize() == beta.size());
...
#ifdef LAP
  template <class T> static inline void LapGetQFromQR(
      const MatrixView<T>& Q, const GenVector<T>& beta)
...
#ifdef XDEBUG
      //cout<<"GetQFromQR:\n";
      //cout<<"compact Q = "<<Q<<endl;
      //cout<<"beta = "<<beta<<endl;
      Matrix<T> Q0(Q);
      Matrix<T> Q2(Q);
      NonBlockGetQFromQR(Q2.View(),beta);
      //cout<<"Non-Block Q = "<<Q2<<endl;
#endif
      LapGetQFromQR(Q,beta);
#ifdef XDEBUG
      //cout<<"Q => "<<Q<<endl;
      if (Norm(Q-Q2) > 0.001*Norm(Q)*Norm(beta)) {
        cerr<<"LapGetQ: Q = "<<TypeText(Q)<<"  "<<Q0<<endl;
        cerr<<"beta = "<<beta<<endl;
        cerr<<"-> Q = "<<Q<<endl;
        cerr<<"NonBlock Q = "<<Q2<<endl;
        abort(); 
