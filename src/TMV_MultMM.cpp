...
namespace tmv {
      ...
      const MatrixView<T>& C)
  {
    ...
    //cout<<"A = "<<Text(A.stor())<<endl;
    //cout<<"B = "<<Text(B.stor())<<endl;
    //cout<<"C = "<<Text(C.stor())<<endl;
      ...
      const int beta, const MatrixView<std::complex<double> >& C)
  {
    ...
    if (A.iscm() && ((!A.isconj() && IMAG(alpha)==0.) || beta == 0)) {
      int m = 2*C.colsize();
      int n = C.rowsize();
      int k = A.rowsize();
      int lda = 2*A.stepj();
      ...
      if (IMAG(alpha) == 0.) {
        ...
        A1 = A.Imag();
        if (A.isconj()) C1 = -REAL(alpha)*A1*B;
      ...
      const int beta, const MatrixView<std::complex<float> >& C)
  {
    ...
    if (A.iscm() && !A.isconj() && (IMAG(alpha)==0.F || beta == 0)) {
      int m = 2*C.colsize();
      int n = C.rowsize();
      int k = A.rowsize();
      int lda = 2*A.stepj();
      ...
      if (IMAG(alpha) == 0.F) {
        ...
        A1 = A.Imag();
        if (A.isconj()) C1 = -REAL(alpha)*A.Imag()*B;
        else C1 = REAL(alpha)*A1*B;
        if (beta == 0) C.Imag() = C1;
        else C.Imag() += C1;
      } else {
