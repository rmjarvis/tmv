...
//#define XDEBUG
      ...
      const Tx x, const VectorView<T>& v)
  {
    ...
    if (step == 1) 
      for(size_t i=v.size();i>0;--i,++vptr) *vptr *= x;
    else
      for(size_t i=v.size();i>0;--i,vptr+=step) *vptr *= x;
  }
      ...
      const VectorView<double>& v)
  {
    ...
    TMVAssert(v.size()>0);
    TMVAssert(v.step()>0);
    cblas_dscal(v.size(),x,v.ptr(),v.step()); 
  }
  template <> inline void DoMultXV(const double x,
      const VectorView<complex<double> >& v)
  {
    ...
    TMVAssert(v.step()>0);
    TMVAssert(v.ct() == NonConj);
    cblas_zdscal(v.size(),x,v.ptr(),v.step()); 
  }
  ...
  { 
    ...
    TMVAssert(v.step()>0);
    TMVAssert(v.ct() == NonConj);
    cblas_zscal(v.size(),&x,v.ptr(),v.step()); 
  }
#ifndef NOFLOAT
      ...
      const VectorView<float>& v)
  {
    ...
    TMVAssert(v.size()>0);
    TMVAssert(v.step()>0);
    cblas_sscal(v.size(),x,v.ptr(),v.step()); 
  }
  template <> inline void DoMultXV(const float x,
      const VectorView<complex<float> >& v)
  {
    ...
    TMVAssert(v.step()>0);
    TMVAssert(v.ct() == NonConj);
    cblas_csscal(v.size(),x,v.ptr(),v.step()); 
  }
  ...
  { 
    ...
    TMVAssert(v.step()>0);
    TMVAssert(v.ct() == NonConj);
    cblas_cscal(v.size(),&x,v.ptr(),v.step()); 
  }
#endif
#endif
  ...
  template <bool c1, class T, class Tx, class T1> void NonBlasAddVV(
      const Tx x, const GenVector<T1>& v1, const VectorView<T>& v2)
  {
    TMVAssert(v1.size() == v2.size());
    TMVAssert(v2.size()>0);
  ...
  { 
    ...
    TMVAssert(v1.step()>0);
    TMVAssert(v2.step()>0);
    cblas_daxpy(v2.size(),x,v1.cptr(),v1.step(),v2.ptr(),v2.step()); 
  }
  ...
  { 
    ...
    TMVAssert(v1.step()>0);
    TMVAssert(v2.step()>0);
    cblas_zaxpy(v2.size(),&x,v1.cptr(),v1.step(),v2.ptr(),v2.step()); 
  }
#ifdef FLOAT
      ...
      const GenVector<float>& v1, const VectorView<float>& v2)
  {
    ...
    TMVAssert(v1.step()>0);
    TMVAssert(v2.step()>0);
    cblas_saxpy(v2.size(),x,v1.cptr(),v1.step(),v2.ptr(),v2.step()); 
  }
  ...
  { 
    ...
    TMVAssert(v1.step()>0);
    TMVAssert(v2.step()>0);
    cblas_caxpy(v2.size(),&x,v1.cptr(),v1.step(),v2.ptr(),v2.step()); 
  }
#endif
#endif
...
#ifdef XDEBUG
    // Can't just say Norm(vx-v2), since that would call this function
    RealType(T) nm(0);
    for(size_t i=0;i<v2.size();i++) nm += NORM(vx(i)-v2(i));
    if (nm > 0.001*max(RealType(T)(1),(abs(x)*Norm(v1)+Norm(v0)))) {
      cerr<<"AddVV: x = "<<x<<endl;
...
#ifdef XDEBUG
    if (Norm(vx-v2) > 0.001*max(RealType(T)(1),
	  (abs(x1)*Norm(v1)+abs(x2)*Norm(v0)))) {
      ...
      const GenVector<T>& v1, const GenVector<T2>& v2) 
  {
    ...
    if (N > TMV_MULTVV_RECURSE_SIZE) {
      ...
      const size_t N1 = (N/8)*4; // N/2 to nearest 4
	...
	NonBlasMultVV<unit,c2>(v1.SubVector(N1,N),v2.SubVector(N1,N));
      ...
      if (unit) {
	for(size_t i=N;i>0;--i,++v1ptr,++v2ptr) 
  ...
  { 
    ...
    TMVAssert(v1.step()>0);
    TMVAssert(v2.step()>0);
    return cblas_ddot(v1.size(),v1.cptr(),v1.step(),v2.cptr(),v2.step()); 
  }
  ...
  { 
    ...
    complex<double> res;
    if (v2.isconj())
      cblas_zdotc_sub(v1.size(),v2.cptr(),v2.step(),v1.cptr(),v1.step(),&res);
    else
      cblas_zdotu_sub(v1.size(),v1.cptr(),v1.step(),v2.cptr(),v2.step(),&res);
    return res;
  }
#ifndef NOFLOAT
  ...
  { 
    ...
    TMVAssert(v1.step()>0);
    TMVAssert(v2.step()>0);
    return cblas_sdot(v1.size(),v1.cptr(),v1.step(),v2.cptr(),v2.step()); 
  }
  ...
  { 
    ...
    complex<float> res;
    if (v2.isconj())
      cblas_cdotc_sub(v1.size(),v2.cptr(),v2.step(),v1.cptr(),v1.step(),&res);
    else
      cblas_cdotu_sub(v1.size(),v1.cptr(),v1.step(),v2.cptr(),v2.step(),&res);
    return res;
  }
#endif
#endif // BLAS
      ...
      const GenVector<T>& v1, const GenVector<T2>& v2) 
  {
    ...
    if (v2.SameAs(v1)) return v1.NormSq();
    else 
#ifdef BLAS
...
#ifdef XDEBUG
    T resx(0);
    for(size_t i=0;i<v1.size();i++) resx += v1(i)*v2(i);
#endif
  ...
  template <bool cx, bool cy, class T, class Ta, class Tx, class Ty>
    void DoAddElementProd(const Ta alpha, const GenVector<Tx>& x,
    ...
    // zi += alpha * xi * yi 
    {
      ...
      if (sx == 1 && sy == 1 && sz == 1)
	if (alpha == Ta(1))
	  for(size_t i=z.size();i>0;--i,++xp,++yp,++zp) 
	...
	if (alpha == Ta(1))
	  for(size_t i=z.size();i>0;--i,xp+=sx,yp+=sy,zp+=sz) 
	    *zp += (cx ? CONJ(*xp) : (*xp)) * (cy ? CONJ(*yp) : (*yp));
	else if (alpha == Ta(-1))
	  for(size_t i=z.size();i>0;--i,xp+=sx,yp+=sy,zp+=sz)
	    *zp -= (cx ? CONJ(*xp) : (*xp)) * (cy ? CONJ(*yp) : (*yp));
  ...
  template <bool cx, class T, class Ta, class Tx> void DoElementProd(
    ...
    // yi = alpha * xi * yi
  {
    ...
    if (sx == 1 && sy == 1)
      if (alpha == Ta(1))
	for(size_t i=y.size();i>0;--i,++xp,++yp) 
	  *yp *= cx ? CONJ(*xp) : (*xp);
      else 
	for(size_t i=y.size();i>0;--i,++xp,++yp) 
	  *yp *= alpha * (cx ? CONJ(*xp) : (*xp));
      ...
      if (alpha == Ta(1))
	for(size_t i=y.size();i>0;--i,xp+=sx,yp+=sy) 
	  *yp *= cx ? CONJ(*xp) : (*xp);
      else 
	for(size_t i=y.size();i>0;--i,xp+=sx,yp+=sy) 
	  *yp *= alpha * (cx ? CONJ(*xp) : (*xp));
