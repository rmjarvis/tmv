...
namespace tmv {
    ...
    public ReadError
  {
	...
	is(_is), iseof(_is.eof()), isbad(_is.bad()) {}
      inline MatrixReadError(istream& _is) throw() :
	ReadError("Matrix"),
	...
	is(_is), iseof(_is.eof()), isbad(_is.bad()) {}
	  ...
	  istream& _is, size_t _cs, size_t _rs) throw() :
	ReadError("Matrix"),
    ...
    public BaseMatrix<T>
  {
    ...
    public:
      ...
      inline GenMatrix(StorageType s, ConjItType c, size_t ls) : 
	itsstor(s), itsct(c), linsize(ls) {}
      inline GenMatrix(const GenMatrix<T>& rhs) : 
...
#define NEW_SIZE(cs,rs) GenMatrix<T>(S,NonConj,((cs)*(rs))), \
	...
	SetAllTo(x);
      ...
      inline Matrix(size_t _colsize, size_t _rowsize, const valarray<T>& vv) : 
	NEW_SIZE(_colsize,_rowsize) 
      { 
	TMVAssert(S==RowMajor || S==ColMajor);
	...
	memmove(itsm.get(),vv,ls()*sizeof(T)); 
      ...
      inline Matrix(size_t _colsize, size_t _rowsize, const vector<T>& vv) : 
	NEW_SIZE(_colsize,_rowsize) 
      { 
	TMVAssert(S==RowMajor || S==ColMajor);
	TMVAssert(vv.size() == ls());
	...
	memmove(itsm.get(),rhs.cptr(),ls()*sizeof(T)); 
      ...
      inline Matrix(const GenMatrix<T>& rhs) :
	NEW_SIZE(rhs.colsize(),rhs.rowsize()) 
      { 
	TMVAssert(S==RowMajor || S==ColMajor);
	Copy(rhs,View()); 
	...
	Copy(rhs,View()); 
      ...
      inline Matrix(const BaseMatrix<T>& rhs) : 
	  NEW_SIZE(rhs.colsize(),rhs.rowsize()) 
      { 
	TMVAssert(S==RowMajor || S==ColMajor);
	rhs.CopyToMatrix(View()); 
      ...
      template <class T2> inline Matrix(const BaseMatrix<T2>& rhs) : 
	  NEW_SIZE(rhs.colsize(),rhs.rowsize()) 
      { 
	TMVAssert(IsComplex(T()) || IsReal(T2()));
	TMVAssert(S==RowMajor || S==ColMajor);
	rhs.CopyToMatrix(View()); 
      ...
      inline Matrix(const MatrixComposite<T>& mcomp) :
...
//---------------------------------------------------------------------------
  ...
  template <class T> inline void DoCopy(
      const GenMatrix<complex<T> >&, const MatrixView<T>&)
  { TMVAssert(FALSE); }
