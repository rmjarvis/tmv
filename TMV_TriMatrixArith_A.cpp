...
namespace tmv {
      ...
      const GenLowerTriMatrix<Ta>& A, const VectorView<T>& x)
  {
    ...
    for(int i=N-1; i>=i1; --i,--xit) {
      if (A.isunit()) *xit += A.row(i,i1,i) * x.SubVector(i1,i);
      else *xit = A.row(i,i1,i+1) * x.SubVector(i1,i+1);
    }
  }
  ...
  template <class T1, class Ta, class Tx, class Ty> void RowMajorAddMultEqMV(
      const T1 alpha, const GenUpperTriMatrix<Ta>& A,
      const GenVector<Tx>& x, const VectorView<Ty>& y)
  {
    //cerr<<"RowMajor AddMultEq Upper\n";
    TMVAssert(A.isrm());
    TMVAssert(x.step()==1);
    TMVAssert(A.size() == x.size());
      ...
      const GenVector<Tx>& x, const VectorView<T>& y)
  {
	...
	if (alpha == T(1))
	  *yit += *xit + MultVV(A.row(i,i+1,N),x.SubVector(i+1,N));
	else
	  *yit += alpha * (*xit+MultVV(A.row(i,i+1,N),x.SubVector(i+1,N)));
    }
    else {
      for(size_t i=0; i<N; ++i,++yit) 
	if (alpha == T(1)) *yit += MultVV(A.row(i,i,N),x.SubVector(i,N));
	else *yit += alpha * MultVV(A.row(i,i,N),x.SubVector(i,N));
    }
  }

  template <class T1, class Ta, class Tx, class Ty> void ColMajorAddMultEqMV(
      const T1 alpha, const GenUpperTriMatrix<Ta>& A,
      const GenVector<Tx>& x, const VectorView<Ty>& y)
  {
    //cerr<<"ColMajor AddMultEq Upper\n";
    TMVAssert(A.iscm());
    TMVAssert(y.step()==1);
    TMVAssert(A.size() == x.size());
    TMVAssert(A.size() == y.size());
      ...
      const GenVector<Tx>& x, const VectorView<T>& y)
  {
	...
	if (alpha == T(1)) *yit += *xit+MultVV(A.row(i,i1,i),x.SubVector(i1,i));
	else *yit += alpha*(*xit+MultVV(A.row(i,i1,i),x.SubVector(i1,i)));
    }
    else {
      for(size_t i=i1; i<N; ++i,++yit) 
	if (alpha == T(1)) *yit += MultVV(A.row(i,i1,i+1),x.SubVector(i1,i+1));
	else *yit += alpha*MultVV(A.row(i,i1,i+1),x.SubVector(i1,i+1));
    }
  }
  ...
  template <class T, class Ta, class Tx> void AddMultEqMV(
      const T alpha, const GenUpperTriMatrix<Ta>& A,
      const GenVector<Tx>& x, const VectorView<T>& y)
    // y += alpha * A * x
  {
    ...
    if (y.size() > 0) {
      if (y.isconj()) AddMultEqMV(CONJ(alpha),A.QuickConjugate(),
	  x.Conjugate(),y.Conjugate());
      else if (A.isrm() && x.step()==1) 
	if (IMAG(alpha) == RealType(T)(0)) 
	  RowMajorAddMultEqMV(REAL(alpha),A,x,y);
	else RowMajorAddMultEqMV(alpha,A,x,y);
      else if (A.iscm() && y.step()==1) 
	if (IMAG(alpha) == RealType(T)(0)) 
	  ColMajorAddMultEqMV(REAL(alpha),A,x,y);
	else ColMajorAddMultEqMV(alpha,A,x,y);
      else RowAddMultEqMV(alpha,A,x,y);
    }
#ifdef XDEBUG
    if (Norm(y-y2) > 1.e-4) {
      cerr<<"AddMultEqMV: alpha = "<<alpha<<endl;
      cerr<<"A = "<<Type(A)<<"  "<<A<<endl;
