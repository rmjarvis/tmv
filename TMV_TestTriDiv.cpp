...
template <class T, tmv::DiagType D> void TestTriDiv() 
{
  ...
  m.SetDiv();
  if (showdiv) {
    Assert(a.CheckDecomp(&cout),"CheckDecomp");
  } else {
  ...
  UpperTriMatrix<T,D> ainv = a.Inverse();
  ...
  ca.SetDiv();
  if (showdiv) {
    Assert(ca.CheckDecomp(&cout),"CheckDecomp");
  } else {
...
template <class T> void TestAllTriDiv()
{
  ...
  cout<<"TriMatrix<"<<tmv::Type(T())<<"> Division passed all tests\n";
}
