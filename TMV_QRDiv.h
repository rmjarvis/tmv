...
#define CT complex<T>
  template <class T> void QR_LDivEq(
      const GenMatrix<CT>& QR, const GenVector<CT>& beta, const size_t* P,
      const MatrixView<T>& m, size_t N1=0)
  { TMVAssert(false); }
...
#include "TMV_AuxAllDiv.h"
    ...
    virtual public Divider<T> 
  {
	...
	TMVAssert(QRx.IsSquare());
	TMVAssert(m.colsize() == QRx.colsize());
	if (istrans) QR_LDivEq(QRx,beta,P,m.Transpose(),N1);
	else QR_LDivEq(QRx,beta,P,m,N1);
      } 
	...
	TMVAssert(QRx.IsSquare());
	TMVAssert(m.rowsize() == QRx.rowsize());
	if (istrans) QR_RDivEq(QRx,beta,P,m.Transpose(),N1);
	else QR_RDivEq(QRx,beta,P,m,N1);
      }
	...
	TMVAssert(m.colsize() == istrans ? QRx.rowsize() : QRx.colsize());
	TMVAssert(x.colsize() == istrans ? QRx.colsize() : QRx.rowsize());
	if (istrans) QR_RDiv(QRx,beta,P,m.Transpose(),x.Transpose(),N1);
	else QR_LDiv(QRx,beta,P,m,x,N1);
      } 
	...
	TMVAssert(m.rowsize() == istrans ? QRx.colsize() : QRx.rowsize());
	TMVAssert(x.rowsize() == istrans ? QRx.rowsize() : QRx.colsize());
	if (istrans) QR_LDiv(QRx,beta,P,m.Transpose(),x.Transpose(),N1);
	else QR_RDiv(QRx,beta,P,m,x,N1); 
      }
