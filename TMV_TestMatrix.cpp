...
template <class T, StorageType S> void TestBasicMatrix()
{
  ...
  Matrix<T,S> m(M,N);
  Matrix<T,S,FortranStyle> mf(M,N);
  ...
  tmv::ConstMatrixView<T,FortranStyle> mfcv = mf.View();
  tmv::MatrixView<T,FortranStyle> mfv = mf.View();
...
{
  Matrix<T> a1(4,4);
  Matrix<T,tmv::RowMajor,FortranStyle> a1f(4,4);
  ...
  Matrix<complex<T>,tmv::RowMajor,FortranStyle> c1f = a1;
  c1f(3,4) += complex<T>(2,3);
  c1f(2,1) *= complex<T>(0,2);
  c1f.col(2) *= complex<T>(-1,3);
  c1f.row(4) += Vector<complex<T> >(4,complex<T>(1,9));
  ...
  c2 -= a2;
  c2 *= complex<T>(1,-2);
  Matrix<T,tmv::ColMajor,FortranStyle> a2f = a2;
  Matrix<complex<T>,tmv::ColMajor,FortranStyle> c2f = c2;
  ...
  Matrix<T,tmv::RowMajor,FortranStyle> a3xf = a3x;
