...
#define RecursiveCH
  ...
  template <bool cm, class T> void NonBlockHermCH_Decompose(
  ...
  template <bool cm, class T> void BlockHermCH_Decompose(
      const SymMatrixView<T>& A)
  {
  ...
  template <bool cm, class T> void RecursiveHermCH_Decompose(
      const SymMatrixView<T>& A)
  {
#ifdef XDEBUG
...
#ifdef CLAP
    clapack_dpotrf(A.iscm() ? CblasColMajor : CblasRowMajor,
	CblasLower,A.size(),A.ptr(), A.iscm() ? A.stepj() : A.stepi());
#else
    char u = A.iscm() ? 'L' : 'U';
    int n = A.size();
...
#ifdef CLAP
    clapack_zpotrf(A.iscm() ? CblasColMajor : CblasRowMajor,
	CblasLower,A.size(),A.ptr(), A.iscm() ? A.stepj() : A.stepi());
#else
    char u = A.iscm() ? 'L' : 'U';
    int n = A.size();
...
#ifdef CLAP
    clapack_spotrf(A.iscm() ? CblasColMajor : CblasRowMajor,
	CblasLower,A.size(),A.ptr(), A.iscm() ? A.stepj() : A.stepi());
#else
    char u = A.iscm() ? 'L' : 'U';
    int n = A.size();
...
#ifdef CLAP
    clapack_cpotrf(A.iscm() ? CblasColMajor : CblasRowMajor,
	CblasLower,A.size(),A.ptr(), A.iscm() ? A.stepj() : A.stepi());
#else
    char u = A.iscm() ? 'L' : 'U';
    int n = A.size();
