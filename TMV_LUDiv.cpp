...
namespace tmv {
  ...
  // MJ: BLOCKSIZE2 need to be optimized
#else
  const size_t LU_BLOCKSIZE = 64;
  const size_t LU_BLOCKSIZE2 = 32;
#endif
...
#undef APTR
      ...
      const MatrixView<T>& A, size_t* P, T& det)
  {
    ...
    if (R <= LU_BLOCKSIZE2) NonBlockLU_Decompose(A,P,det);
    else if (R > 2) {
      ...
      const MatrixView<T>& A, size_t* P, T& det)
  {
    ...
    else if (A.rowsize() >= 2*LU_BLOCKSIZE) 
...
#ifndef NOFLOAT
#ifdef XLAP
  // This is giving me a weird runtime error sometimes:
  ...
  // So I'm cutting it out via the XLAP ifdef 
  template <> inline void LapLU_Decompose(
  ...
  {
    TMVAssert(A.ct()==NonConj);
    if (A.iscm() && A.colsize() > 0 && A.rowsize() > 0) {
#ifdef LAP
