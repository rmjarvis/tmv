  ...
  template <class T> void GenMatrix<T>::NewDivider() const
  {
    switch (this->itsdt) {
      ...
      case tmv::SV : this->itsdiv = new SVDiv<T>(*this,this->divinplace,
			  true,true); 
		      break;
      case tmv::SVS : this->itsdiv = new SVDiv<T>(*this,this->divinplace,
			  false,false); 
		      break;
      case tmv::SVU : this->itsdiv = new SVDiv<T>(*this,this->divinplace,
			  true,false); 
		      break;
      case tmv::SVV : this->itsdiv = new SVDiv<T>(*this,this->divinplace,
			  false,true); 
		      break;
...
#undef NEW_SIZE
  ...
  template <class T, IndexStyle I> 
    const MatrixView<T,I>& MatrixView<T,I>::SwapRows(size_t i1, size_t i2) const
    { 
  ...
  template <class T, IndexStyle I> 
    const MatrixView<T,I>& MatrixView<T,I>::SwapCols(size_t j1, size_t j2) const
    { 
  ...
  template <bool c1, class T> void NonLapCopy(
  ...
  { 
    ...
    is >> cs >> rs;
    if (!is) 
      tmv_error("reading size in Matrix::Read");
    if (m.colsize() != cs) 
      tmv_error("colsize does not match in Matrix::Read");
    if (m.rowsize() != rs) 
      tmv_error("rowsize does not match in Matrix::Read");
