...
#undef NEW_SIZE
  ...
  template <class T> void GenMatrix<T>::Write(ostream& os) const
  {
    ...
    for(size_t i=0;i<colsize();++i) {
      ...
      if (isrm()) {
	if (isconj()) {
	  const CVIt<T,Unit,Conj>  _end = row(i).end();
	  for(CVIt<T,Unit,Conj> it=row(i).begin(); it!=_end; ++it) 
	    os << ' '<<*it<<' ';
	} else {
	  const CVIt<T,Unit,NonConj>  _end = row(i).end();
	  for(CVIt<T,Unit,NonConj> it=row(i).begin(); it!=_end; ++it) 
	    os << ' '<<*it<<' ';
	}
      } else {
	if (isconj()) {
	  const CVIt<T,Step,Conj>  _end = row(i).end();
	  for(CVIt<T,Step,Conj> it=row(i).begin(); it!=_end; ++it) 
	    os << ' '<<*it<<' ';
	} else {
	  const CVIt<T,Step,NonConj>  _end = row(i).end();
	  for(CVIt<T,Step,NonConj> it=row(i).begin(); it!=_end; ++it) 
	    os << ' '<<*it<<' ';
	}
      }
      os << " )\n";
  ...
  template <class T> void MatrixView<T>::Read(istream& is) const
  {
    ...
    for(size_t i=0;i<colsize();++i) {
      ...
      if (isrm()) {
	if (isconj()) {
	  const VIt<T,Unit,Conj>  _end = row(i).end();
	  for(VIt<T,Unit,Conj> it=row(i).begin(); it!=_end; ++it) is >> *it;
	} else {
	  const VIt<T,Unit,NonConj>  _end = row(i).end();
	  for(VIt<T,Unit,NonConj> it=row(i).begin(); it!=_end; ++it) is >> *it;
	}
      } else {
	if (isconj()) {
	  const VIt<T,Step,Conj>  _end = row(i).end();
	  for(VIt<T,Step,Conj> it=row(i).begin(); it!=_end; ++it) 
	    is >> *it;
	} else {
	  const VIt<T,Step,NonConj>  _end = row(i).end();
	  for(VIt<T,Step,NonConj> it=row(i).begin(); it!=_end; ++it) 
	    is >> *it;
	}
      }
