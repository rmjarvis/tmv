...
namespace tmv {
  ...
  // These two don't have += form - they must called explicitly
  ...
  template <class T, class T2, class T3, bool trans> class ProdSS : 
  ...
  template <class T, class T2, class T3, bool trans> 
    inline const MatrixView<T>& operator+=(
      const MatrixView<T>& m, const ProdSS<T,T2,T3,trans>& pmm)
  { 
    MultMM(pmm.GetX1(),pmm.GetM2(),trans && pmm.GetM3().isherm() ? 
	pmm.GetM3().Transpose() : pmm.GetM3().View(),T(1),m); 
    return m; 
  }
  ...
  // m += (x*m^m)
  template <class T, class T2, bool trans> 
    inline const SymMatrixView<T>& operator+=(
	const SymMatrixView<T>& m0, const ProdMM<T,T2,T2,trans>& opmm)
    {
      TMVAssert(m0.size() == opmm.colsize());
      TMVAssert(m0.size() == opmm.rowsize());
      TMVAssert(opmm.GetM2().SameAs(trans ?
	    (m0.isherm() ? opmm.GetM3().Conjugate() : opmm.GetM3().View()) :
	    (m0.isherm() ? opmm.GetM3().Adjoint() : opmm.GetM3().Transpose())));
      RankKUpdate(opmm.GetX1(), opmm.GetM2(), m0);
      return m0;
    }

  template <class T, bool trans> inline const SymMatrixView<CT>& operator+=(
      const SymMatrixView<CT>& m0, const ProdMM<T,T,T,trans>& opmm)
  {
    TMVAssert(m0.size() == opmm.colsize());
    TMVAssert(m0.size() == opmm.rowsize());
    TMVAssert(opmm.GetM2().SameAs(trans ? 
	  opmm.GetM3().View() : opmm.GetM3().Transpose()));
