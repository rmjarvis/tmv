...
namespace tmv {
  ...
  template <class T> void GenBandMatrix<T>::NewDivider() const
  {
    switch (this->itsdt) {
      case LU : 
	this->itsdiv = new BandLUDiv<T>(*this); break;
      case QR : 
	this->itsdiv = new BandQRDiv<T>(*this); break;
      case SV : case SVF :
  ...
  template <class T> void GenBandMatrix<T>::Write(ostream& os) const
  {
	...
	if (isconj()) {
	  ...
	  const CVIt<T,Unit,Conj> _end = it+len;
	  for(; it!=_end; ++it) os << ' ' << *it << ' ';
	} else {
	  CVIt<T,Unit,NonConj> it(mptr,1);
	  const CVIt<T,Unit,NonConj> _end = it+len;
	  for(; it!=_end; ++it) os << ' ' << *it << ' ';
	}
      } else {
	if (isconj()) {
	  CVIt<T,Step,Conj> it(mptr,stepj());
	  const CVIt<T,Step,Conj> _end = it+len;
	  for(; it!=_end; ++it) os << ' ' << *it << ' ';
	} else {
	  CVIt<T,Step,NonConj> it(mptr,stepj());
	  const CVIt<T,Step,NonConj> _end = it+len;
	  for(; it!=_end; ++it) os << ' ' << *it << ' ';
	}
      }
      size_t j2 = j1+len;
  ...
  template <class T> void BandMatrixView<T>::Read(istream& is) const
  {
	...
	if (isconj()) {
	  ...
	  const VIt<T,Unit,Conj> _end = it+len;
	  for(; it!=_end; ++it) is >> *it;
	} else {
	  VIt<T,Unit,NonConj> it(mptr,1 FIRSTLAST);
	  const VIt<T,Unit,NonConj> _end = it+len;
	  for(; it!=_end; ++it) is >> *it;
	}
      } else {
	if (isconj()) {
	  ...
	  const VIt<T,Step,NonConj> _end = it+len;
	  for(; it!=_end; ++it) is >> *it;
	}
      }
