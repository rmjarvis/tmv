...
namespace tmv {
  ...
  // v2 = x * v1 + v2
  template <class T, class T1> void AddVV(const T x, const GenVector<T1>& v1,
  ...
  template <class T, class T2, class T4> class SumVV : public VectorComposite<T> 
  {
	...
	x1(_x1),v2(&_v2),x3(_x3), v4(&_v4)
      ...
      inline void DoAssignTo(const VectorView<T>& v0) const
      {
	// MJ: Move this to .cpp file.  Also, needs SameStorage checks
	// and if all three are samestorage
	TMVAssert(v0.size() == size());
  ...
  template <class T> inline complex<T> operator*(
      const GenVector<complex<T> >& v1, const GenVector<T>& v2)
  { return MultVV(v1,v2); }
