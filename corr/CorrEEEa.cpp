...
void DirectProcessV(
    EEEData& bin, const Cell& c1, const Cell& c2, const Cell& c3,
...
void Add(
    ...
    const std::vector<std::vector<EEEData> >& temp, 
    const Cell& c1, const Cell& c2, const Position2D& r3, double d3)
{
    ...
    for(int ku = 0; ku < int(data.size()); ku++) {
        std::vector<EEEData>& dataku = data[ku];
        const std::vector<EEEData>& tempku = temp[ku];
        for(int kv = 0; kv < int(dataku.size()); kv++) {
            const EEEData& tempbin = tempku[kv];
...
void FinalizeProcess(
    std::vector<std::vector<std::vector<EEEData> > >& data, double vare)
...
int main(int argc, char* argv[])
{
    ...
    dbg<<"wholefield size = "<<wholefield->Size()<<", meanpos = "<<wholefield->MeanPos()<<std::endl;
