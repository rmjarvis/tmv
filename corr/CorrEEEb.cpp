...
void AddData(
    EEEData& bindata, const PairCellData& p12, const Cell& c3, 
    bool swap12, double u, double v)
{
    ...
    std::complex<double> e3 = c3.WE() * p12.expm2ialpha3;
...
void DirectProcess11SingleU(
    std::vector<std::vector<EEEData> >& data,
    ...
    const std::complex<double>& rm, const double dm, const double u) 
{
    ...
    Assert(j>=0 && j < nubins);
    std::vector<EEEData>& dataj = data[j];

#ifdef ALLDEBUG2
    recursen++;
    dbg<<std::string(recursen,'.')<<"Start DirectP12: "<<c3.MeanPos()<<" -- "<<p12.MeanPos()<<"   D3 = "<<p12.MeanD3()<<"  DM = "<<dm<<std::endl;
#endif

    double usqover8 = u*u/8.;
    //dbg<<"initial: alpham = "<<alpham<<", k = "<<k<<", swap = "<<swap12<<std::endl;
    for(int i=0;i<p12.NBins();i++) {
        ...
        //dbg<<"single "<<j<<','<<k<<": thisr = "<<pairdatai.d3<<", meanr = "<<datajk.meanr/datajk.weight<<std::endl;
    }

#ifdef ALLDEBUG2
    dbg<<std::string(recursen,'.')<<"Done DirectP11 Pair - Cell\n";
    recursen--;
#endif
}
...
#ifdef ALLDEBUG2
    recursen++;
    dbg<<std::string(recursen,'.')<<"Start DirectP11Multi: "<<c3.MeanPos()<<" -- "<<p12.MeanPos()<<"   D3 = "<<p12.MeanD3()<<"  DM = "<<Dist(c3.MeanPos()-p12.MeanPos())<<std::endl;
#endif

    for(int i=0;i<p12.NBins();i++) {
        const PairCellData& pairdatai = p12.Data(i);
        if (pairdatai.w1w2 == 0.0) continue;
        std::complex<double> rm = c3.MeanPos() - pairdatai.pos;
        ...
        //dbg<<"multi  "<<j<<','<<k<<":  thisr = "<<pairdatai.d3<<", meanr = "<<datajk.meanr/datajk.weight<<std::endl;
    }

#ifdef ALLDEBUG2
    dbg<<std::string(recursen,'.')<<"Done DirectP11Multi\n";
    recursen--;
#endif
}
    ...
    const double dm, const std::complex<double>& rm) 
{
    ...
    static const double maxsingleu = sqrt(2.*b/(1.+sqrt(b/8)));

    double u = p12.MeanD3()/dm;
    if (u < maxsingleu && (c3.Size() + p12.AllSize())/dm < b) 
        DirectProcess11SingleU(data,p12,c3,rm,dm,u);
    else
        DirectProcess11MultiU(data,p12,c3);
}

void FinalizeProcess(std::vector<std::vector<EEEData> >& data, double vare)
{
