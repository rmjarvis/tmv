...
//#define SCH_U  // ifdef then use Schneider U, else Crittenden U
...
{
    Assert(false); // I don't know the functional form for this
}
    ...
    // functions: T+,T-,S+,S-
{
    ...
    return (2.4*acos(r/2.)*(2.-15.*rsq) - temp)/PI;
}
    ...
    //      + 3/40Pi r^3 sqrt(4-r^2) (r^6 - 14r^4 + 74r^2 - 196)
{
    ...
    return (18*rsq*acos(r/2.) + temp)/PI;
}
...
// modification: <M^3>(t,k2 t,k3 t) and <M^2M*>(t, k2 t, k3 t)
{
    ...
    if (k2 ==1. && k3 == 1.) {
        ...
        *T3 = ( SQR(q3*conj(q1q2))/kto6/24. - q3*conj(q1q2q3)/kto4/9. +
        ...
        double Z1 = ( k1sq * norm(s1) + k2sq * norm(s2) + k3sq * norm(s3) )
            /6./Theta2;
        double Z2 = ( k1sq * norm(s2) + k2sq * norm(s3) + k3sq * norm(s1) ) 
            /6./Theta2;
        double Z3 = ( k1sq * norm(s3) + k2sq * norm(s1) + k3sq * norm(s2) ) 
            /6./Theta2;
