...
// 
void DirectProcessV(
    NEEData& bin, const NCell& c1, const Cell& c2, const Cell& c3,
...
void Add(
    std::vector<std::vector<NEEData> >& data,
    const std::vector<std::vector<NEEData> >& temp, 
    const NCell& c1, const Cell& c2, const std::complex<double>& r3, double d3)
{
    ...
    const std::complex<double> e1 = c1.WE() * expm2iarg3;
    const std::complex<double> e2 = c2.WE() * expm2iarg3;
    const std::complex<double> e1e2 = e1*e2;
    const std::complex<double> e1e2c = e1*conj(e2);

    for(int ku = 0; ku < int(data.size()); ku++) {
        std::vector<NEEData>& dataku = data[ku];
        const std::vector<NEEData>& tempku = temp[ku];
        for(int kv = 0; kv < int(dataku.size()); kv++) {
            const NEEData& tempbin = tempku[kv];
...
int main()
{
    ...
    dbg<<"brightfield size = "<<brightfield->Size()<<", meanpos = "<<brightfield->MeanPos()<<std::endl;
    dbg<<"faintfield size = "<<faintfield->Size()<<", meanpos = "<<faintfield->MeanPos()<<std::endl;

    std::vector<std::vector<std::vector<NEEData> > > data(
        nrbins,
        std::vector<std::vector<NEEData> >(
            nubins,std::vector<NEEData>(nvbins)));
