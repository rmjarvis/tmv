...
inline NPairCellData::NPairCellData(
    ...
    const double d, const std::complex<double>& r) :
    NCellData( (c1.MeanPos()+c2.MeanPos())/2. ),
...
inline PairCellData::PairCellData(
    ...
    const double d, const std::complex<double>& r) :
    NPairCellData(c1,c2,d,r), w1w2(c1.Weight()*c2.Weight())
{
...
public:
    ...
    const DataType& Data(size_t i) const 
    ...
    const GenPairCell* Left() const { return left; }
    const GenPairCell* Right() const { return right; }
