...
class PRODMV : public VectorComposite<T>
{
  public:
    ...
    inline void AssignTo(const VectorView<T>& v0) const
    {
      ...
      if (transm2)
	MultMV(x1,m2->Transpose(),*v3,T(0),v0);
      else
...
{ 
  MultMV(T(1),m2.Transpose(),v1,T(0),v1);
  return v1;
}
...
{
  MultMV(CT(1),m2.Transpose(),v1,CT(0),v1);
  return v1;
}
...
{ 
  if (transm2)
    MultMV(pmv.GetX1(),pmv.GetM2().Transpose(),pmv.GetV3(),T(1),v);
  else
...
{ 
  if (transm2)
    MultMV(CT(pmv.GetX1()),pmv.GetM2().Transpose(),pmv.GetV3(),CT(1),v);
  else
