...
{
  public:
    PRODMV(const T _x, const GENMATRIX2<T1>& _m,
    ...
    inline const GENMATRIX2<T1>& GetM() const { return *m; }
...
template <class T> inline const VectorView<CT>& operator*=(
    const VectorView<CT>& v, const GENMATRIX2<T>& m)
{
...
{
  public:
    RQUOTVM(const T _x, const GenVector<T1>& _v, const GENMATRIX2<T2>& _m) :
      x(_x), v(&_v), m(&_m)
