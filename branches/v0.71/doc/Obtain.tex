...
\item \texttt{FINAL\_PREFIX=''} specifies a final installation directory if different from \texttt{PREFIX}.  This is used for installations (e.g. fink) that install everything in a temporary directory first, and then copies everthing to a final location only if the installation was successful.
...
\item \texttt{SINGLE\_BUILD\_THREAD=false} specifies whether to use only a single thread when building the code, rather than the default of ncpus.  Note that this only applied to the default case when you don't specify the number of threads with -jN.  The -jN flag always takes precedence.
\end{itemize}
