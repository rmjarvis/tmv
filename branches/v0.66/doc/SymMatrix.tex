...
tmv::HermMatrix<T,uplo,stor,index> h(size_t n)
\end{tmvcode}
...
tmv::HermMatrix<T,uplo,stor,index> h(size_t n, RT x)
\end{tmvcode}
Makes an \tt{n} $\times$ \tt{n} \tt{SymMatrix} or \tt{HermMatrix} 
...
In each case, the same compact format can be read back in the usual way:
\begin{tmvcode}
tmv::SymMatrix<T> s;
tmv::HermMatrix<T> h;
is >> s >> h;
