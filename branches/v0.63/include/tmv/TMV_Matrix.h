...
namespace tmv {
                      ...
                      private DivHelper<T>
    {
        ...
        typedef ConstMatrixView<RT> const_real_type;
    ...
    public:
        ...
        inline const_vec_type diag(int i, int j1, int j2) const
        {
            ...
            if (i >= 0) {
                TMVAssert(j1>=0 && j1-j2<=0 && 
                          j2<=TMV_MIN(int(colsize()),int(rowsize())-i));
                return const_vec_type(
                    cptr()+i*stepj()+j1*diagstep,j2-j1, diagstep,ct());
            } else {
                TMVAssert(j1>=0 && j1-j2<=0 && 
                          j2<=TMV_MIN(int(colsize())+i,int(rowsize())));
                return const_vec_type(
        ...
        inline RT norm2() const
        {
            ...
            TMV_Warning("Calling Matrix::norm2 without previously calling "
                        "divideUsing(SV)");
            return doNorm2();
        }
        ...
        inline RT condition() const
        {
            ...
            TMV_Warning("Calling Matrix::condition without previously calling "
                        "divideUsing(SV)");
            return doCondition();
        }
        ...
        // The rest of the deprecated versions are in BaseMatrix.
        ...
        { divideUsing(dt); }
        TMV_DEPRECATED(const LUDiv<T>& LUD() const)
        { return lud(); }
        TMV_DEPRECATED(const QRDiv<T>& QRD() const)
        { return qrd(); }
        TMV_DEPRECATED(const QRPDiv<T>& QRPD() const)
        { return qrpd(); }
        TMV_DEPRECATED(const SVDiv<T>& SVD() const)
        { return svd(); }
    ...
    inline bool GenMatrix<T>::SameAs(const BaseMatrix<T2>& m2) const
    { return isSameAs(m2); }

    template <class T> template <class T1> 
    inline void GenMatrix<T>::Inverse(const MatrixView<T1>& minv) const
    { makeInverse(minv); }

    template <class T> template <class T1, StorageType S, IndexStyle I> 
    inline void GenMatrix<T>::Inverse(Matrix<T1,S,I>& minv) const
    { makeInverse(minv); }

    template <class T> template <StorageType S, IndexStyle I> 
    inline void GenMatrix<T>::InverseATA(Matrix<T,S,I>& ata) const
    { makeInverseATA(ata); }
    ...
    class ConstMatrixView<T,FortranStyle> : public ConstMatrixView<T,CStyle>
    {
        ...
        typedef ConstMatrixView<RT,FortranStyle> const_real_type;
    ...
    public :
        ...
        {
            TMVAssert(hasSubVector(i,j,istep,jstep,s));
            return base::subVector(i-1,j-1,istep,jstep,s);
        }
    ...
    public:
        ...
        inline MatrixView(
            ...
            StorageType _stor, ConjType _ct, size_t _ls 
        ...
        { return setToIdentity(x); }
        TMV_DEPRECATED(const type& SwapRows(int i1, int i2) const)
        { return swapRows(i1,i2); }
        TMV_DEPRECATED(const type& SwapCols(int j1, int j2) const)
        { return swapCols(j1,j2); }
        TMV_DEPRECATED(const type& PermuteRows(
                const int* p, int i1, int i2) const)
        { return permuteRows(p,i1,i2); }
        TMV_DEPRECATED(const type& PermuteRows(const int* p) const)
        { return permuteRows(p); }
        TMV_DEPRECATED(const type& PermuteCols(
                const int* p, int j1, int j2) const)
        { return permuteCols(p,j1,j2); }
        TMV_DEPRECATED(const type& PermuteCols(const int* p) const)
        { return permuteCols(p); }
        TMV_DEPRECATED(const type& ReversePermuteRows(
                const int* p, int i1, int i2) const)
        { return reversePermuteRows(p,i1,i2); }
        TMV_DEPRECATED(const type& ReversePermuteRows(const int* p) const)
        { return reversePermuteRows(p); }
        TMV_DEPRECATED(const type& ReversePermuteCols(
                const int* p, int j1, int j2) const)
        { return reversePermuteCols(p,j1,j2); }
        TMV_DEPRECATED(const type& ReversePermuteCols(const int* p) const)
        ...
        { return lowerTri(dt); }
        TMV_DEPRECATED(view_type ColPair(int j1, int j2) const)
        { return colPair(j1,j2); }
        TMV_DEPRECATED(view_type RowPair(int i1, int i2) const)
        { return rowPair(i1,i2); }
    ...
    class MatrixView<T,FortranStyle> : public MatrixView<T,CStyle>
    {
        ...
        typedef MatrixView<RT,FortranStyle> real_type;
    ...
    public:
        ...
        inline view_type colPair(int j1, int j2) const
        {
            TMVAssert(j1 > 0 && j1 <= int(this->rowsize()));
            TMVAssert(j2 > 0 && j2 <= int(this->rowsize()));
            return c_type::colPair(j1-1,j2-1);
        }
        ...
        inline view_type rowPair(int i1, int i2) const
        {
            ...
            return c_type::rowPair(i1-1,i2-1);
        }
...
#define NEW_SIZE(cs,rs) \
        ...
        inline Matrix(size_t _colsize, size_t _rowsize, const T& x) :
            NEW_SIZE(_colsize,_rowsize)
        {
            TMVAssert(S==RowMajor || S==ColMajor);
            setAllTo(x);
...
#undef NEW_SIZE
        ...
        inline const_vec_type diag(int i, int j1, int j2) const
        {
            ...
            if (i >= 0) {
                TMVAssert(j2<=TMV_MIN(int(colsize()),int(rowsize())-i));
                return const_vec_type(
