...
\item
\begin{tmvcode}
tmv::SymMatrix<T,uplo,stor,index> s()
tmv::HermMatrix<T,uplo,stor,index> h()
\end{tmvcode}
...
\item 
\begin{tmvcode}
tmv::SymMatrix<T,uplo,stor,index> s(int n)
tmv::HermMatrix<T,uplo,stor,index> h(int n)
\end{tmvcode}
...
\item
\begin{tmvcode}
tmv::SymMatrix<T,uplo,stor,index> s(int n, T x)
tmv::HermMatrix<T,uplo,stor,index> h(int n, RT x)
\end{tmvcode}
Makes an \tt{n} $\times$ \tt{n} \tt{SymMatrix} or \tt{HermMatrix} 
...
tmv::HermMatrix<T,uplo,stor,index> h(const GenMatrix<T>& m)
tmv::SymMatrix<T,uplo,stor,index> s(const GenDiagMatrix<T>& d)
tmv::HermMatrix<T,uplo,stor,index> h(const GenDiagMatrix<T>& d)
\end{tmvcode}
...
tmv::HermMatrix<T,uplo,stor,index> h1(const GenSymMatrix<T2>& s2)
s1 = s2
\end{tmvcode}
...
tmv::SymMatrixView<T,index> h = 
      tmv::HermMatrixViewOf(MatrixView<T,index>& m, UpLoType uplo)
tmv::ConstSymMatrixView<T,index> h = 
