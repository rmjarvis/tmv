...
\item 
\begin{tmvcode}
tmv::SymMatrix<T,A> s(const GenMatrix<T>& m)
tmv::HermMatrix<T,A> h(const GenMatrix<T>& m)
tmv::SymMatrix<T,A> s(const GenDiagMatrix<T>& d)
tmv::HermMatrix<T,A> h(const GenDiagMatrix<T>& d)
\end{tmvcode}
Makes a \tt{SymMatrix} or \tt{HermMatrix} which copies the corresponding values of \tt{m}.

\item
\begin{tmvcode}
tmv::SymMatrix<T,A> s1(const GenSymMatrix<T2>& s2)
tmv::HermMatrix<T,A> h1(const GenSymMatrix<T2>& s2)
s1 = s2
\end{tmvcode}
