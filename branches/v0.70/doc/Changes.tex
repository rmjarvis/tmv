...
In addition, a \tt{BandMatrix} can be iterated in \tt{DiagMajor} order using \tt{m.diagmajor_begin()}.
...
\item 
Added warning for when the matrix multiplication routine catches a \tt{bad_alloc} exception and reverts to using a function that doesn't allocate memory.  Also added documenation for the Warning that had been output when the divide-and-conquer SVD algorithm has trouble converging.

\item
Changed the default warning behavior to be no output.  To turn on the warnings, the user needs to explicitly specify an \tt{ostream} with the \tt{tmv::WriteWarningsTo} function.
