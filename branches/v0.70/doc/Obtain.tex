...
\begin{itemize}
\item
\texttt{-DNDEBUG} or \texttt{-DTMV\_NDEBUG}
...
without turning it off for your own code.  It is exactly equivalent to \tt{-DNDEBUG} when compiling the library code, but it can be useful when compiling your own code if you 
...
will add some extra debugging statements that take $O(N)$ or $O(N^2)$ time rather than merely $O(1)$ time.  This can be useful to turn on when debugging your own code, but it is probably not ever necessary to use when compiling the library.
