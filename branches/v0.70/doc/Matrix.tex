...
\item 
\begin{tmvcode}
tmv::Matrix<T,stor,index> m(int nrows, int ncols)
\end{tmvcode}
Makes a \tt{Matrix} with \tt{nrows} rows and \tt{ncols} columns with 
\underline{uninitialized} values.
If extra debugging is turned on (with \tt{-DTMV\_EXTRA\_DEBUG}), then the values are in fact initialized to 888.  This should help you notice when you have neglected to initialize the \tt{Matrix} correctly.

\item
\begin{tmvcode}
tmv::Matrix<T,stor,index> m(int nrows, int ncols, T x)
\end{tmvcode}
...
However, for a regular \tt{Matrix} (i.e. not a \tt{MatrixView}) the iterators in the same direction as the matrices storage order will be a random-access iterator.  In fact, it will be a \tt{VIt<T,Unit,NonConj>} (c.f. \S\ref{VectorIterators}).
...
(Or if compiling with \tt{-DTMV\_EXTRA\_DEBUG}, the values are in fact set to 888.)
...
\index{Matrix!Methods!constLinearView}
\index{Matrix!Methods!canLinearize}
These return a view to the elements of a \tt{Matrix} as a single vector.  It is always allowed for an actual \tt{Matrix}.  For a \tt{MatrixView} (or \tt{ConstMatrixView}), it is only allowed if all of the elements in the view are in one contiguous block of memory.  The helper function \tt{m.canLinearize()} returns whether or not the first two methods will work.
...
m = ElemProd(m,m2);
\end{tmvcode}
The return value of the \tt{ElemProd} function is a \tt{MatrixComposite} object, so it can be used in more complicated arithmetic expressions just like any other arithmetic operator.
...
Performs the calculation $m = x \cdot m$.

\item
\begin{tmvcode}
MultMV<bool add>(T x, const GenMatrix<T1>& m, 
      const GenVector<T2>& v1, const VectorView<T>& v2)
\end{tmvcode}
\index{Matrix!Arithmetic!MultMV function}
Performs the calculation $v_2 =  [v_2 \;+\;] ~ x \cdot m \cdot v_1$ 
where the result is added to $v_2$ if \tt{add = true} and just assigned to it if \tt{add = false}.
...
Performs the calculation $m_2 = m_2 + x \cdot m_1$.
...
\end{tmvcode}
\index{Matrix!Arithmetic!AddMM function}
Performs the calculation $m_3 = x_1 \cdot m_1 + x_2 \cdot m_2$.
...
Performs the calculation $m_3 = [m_3 \;+\;] ~ x \cdot m_1 \cdot m_2$
where the result is added to $m_3$ if \tt{add = true} and just assigned to it if \tt{add = false}.
...
$m(i,j) = [m(i,j) \;+\;] ~ x \cdot v_1(i) \cdot v_2(j)$ for each element in the matrix
where the result is added to $m(i,j)$ if \tt{add = true} and just assigned to it if \tt{add = false}.

\item
\begin{tmvcode}
ElemMultMM<bool add>(T x, const GenMatrix<T1>& m1, 
      const GenMatrix<T2>& m2, const MatrixView<T>& m3)
\end{tmvcode}
\index{Matrix!Arithmetic!MultMM function}
Performs the calculation $m_3(i,j) = [m_3(i,j) \;+\;] ~ x \cdot m_1(i,j) \cdot m_2(i,j)$ 
where the result is added to $m_3(i,j)$ if \tt{add = true} and just assigned to it if \tt{add = false}.
