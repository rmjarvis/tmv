...
aren't the default, and you specify multiple values by using the \tt{|} 
operator.
...
already performed an SV decomposition of \tt{m}.  If you are using 
...
expensive, scaling as $O(N^3)$, rather than $O(N^2)$ like most other norms.
...
\index{Matrix!Singular value decomposition}
\index{Singular Value Decomposition!Matrix}
\tt{getU()}, \tt{getS()}, and \tt{getVt()} return $U$, $S$, and $V^\dagger$.
...
\tt{tmv::ColMajor}, either \tt{tmv::CStyle} or
...
\item
\begin{tmvcode}
tmv::SmallMatrix<T,M,N,A> m(const T* vv)
\end{tmvcode}
Makes an \tt{M} $\times$ \tt{N} \tt{SmallMatrix} with values copied from vv.

\item 
\begin{tmvcode}
tmv::SmallMatrix<T,M,N,A> m1(const GenMatrix<T>& m2)
\end{tmvcode}
