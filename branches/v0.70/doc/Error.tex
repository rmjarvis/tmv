...
that might not be 0's), but these are not enabled by default.  They are only turned on when using \tt{-DTMV\_EXTRA\_DEBUG}.
...
\begin{itemize}
\item The divide and conquer SVD algorithm had trouble converging on a 
...
\item A \tt{bad_alloc} was caught in the matrix multiplication algorithm, and
...
In the past, the default behavior had been to output warnings to std::cout,
