...
(e.g. \tt{double}, \tt{complex<double>}, \tt{int}, etc.) and \tt{A} is either
\tt{tmv::CStyle} or \tt{tmv::FortranStyle}.  The \tt{A} stands for Attributes.
A \tt{Vector} only has one available attribute, the indexing style, but 
the various matrix types will have other options as well which will be
specified with the Attributes template parameters.
In all of the constructors the
Attributes parameter may be omitted, in which case \tt{CStyle} is assumed.
