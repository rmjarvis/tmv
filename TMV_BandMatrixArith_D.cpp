...
namespace tmv {
      ...
      const T beta, const MatrixView<T>& C)
  {
	...
	MultXM(beta,C.Rows(i+1,C.colsize()));
	break;
      }
    }
  }
      ...
      const T beta, const MatrixView<T>& C)
  {
    ...
    for(size_t j=0;j<A.rowsize();++j) {
      Rank1Update(alpha,A.col(j,i1,i2),B.row(j),C.Rows(i1,i2));
      if (k>0) --k; else ++i1;
      if (i2<A.colsize()) ++i2;
      ...
      const T beta, const BandMatrixView<T>& C)
  {
	...
	if (++i < C.colsize()) 
	  MultXM(beta,C.SubBandMatrix(i,C.colsize(),j1,C.rowsize(),0,j2-j1-1));
	break;
      }
    }
  }
      ...
      const T beta, const BandMatrixView<T>& C)
  {
    ...
    MultXM(beta,C);
    for(size_t j=0;j<A.rowsize(); ++j) {
      Rank1Update(alpha,A.col(j,i1,i2),B.row(j,m1,m2),
	  C.SubMatrix(i1,i2,m1,m2));
      if (k>0) --k; else ++i1;
      if (i2<A.colsize()) ++i2;
      else if (i1==A.colsize()) break;
      if (n>0) --n; else ++m1;
      ...
      const T beta, const BandMatrixView<T>& C)
  {
	...
	if (++j < C.rowsize()) 
	  MultXM(beta,C.SubBandMatrix(i1,C.colsize(),j,C.rowsize(),i2-i1-1,0));
	break;
      }
    }
  }
