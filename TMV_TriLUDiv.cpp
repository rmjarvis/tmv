...
namespace tmv {
  ...
  template <class T> T UpperTriLUDiv<T>::Det() const
  {
    if (!donedet) {
  ...
  template <class T> T LowerTriLUDiv<T>::Det() const
  {
    if (!donedet) {
      if (!itsm->isunit()) det *= DiagMatrixViewOf(itsm->diag()).Det();
      donedet = true;
  ...
  template <class T> 
    UpperTriMatrix<T,NonUnitDiag,ColMajor> UpperTriLUDiv<T>::TInverse() const
    {
  ...
  template <class T> 
    LowerTriMatrix<T,NonUnitDiag,ColMajor> LowerTriLUDiv<T>::TInverse() const
    {
  ...
  template <class T> Matrix<T,ColMajor> UpperTriLUDiv<T>::InverseATA() const
  {
    UpperTriMatrix<T,NonUnitDiag,ColMajor> temp = TInverse();
    return temp*Adjoint(temp);
  }

  template <class T> Matrix<T,ColMajor> LowerTriLUDiv<T>::InverseATA() const
  {
	...
	const GenUpperTriMatrix<T1>& A, const VectorView<T2>& b)
  {
    ...
    if (A.isunit()) {
      if (N==1) return;
      for(int i=N-2,ip1=N-1,len=1; i>=0; --i,--ip1,++len) {
	T2 temp;
	if (A.isconj())
      ...
      const GenUpperTriMatrix<T1>& A, const VectorView<T2>& b)
  {
	...
	for(int j=N-1; j>0; --j,--bj) if (*bj != T2(0))
	  DoAddVV(-*bj,CVIt<T1,Unit,Conj>(A.col(j,0,j).begin()),
	      VIt<T2,Unit,NonConj>(b.begin()),j);
      else
	for(int j=N-1; j>0; --j,--bj) if (*bj != T2(0))
	  DoAddVV(-*bj,CVIt<T1,Unit,NonConj>(A.col(j,0,j).begin()),
	      VIt<T2,Unit,NonConj>(b.begin()),j);
    } else {
      CVIter<T1> Ajj = A.diag().begin()+N-1;
      for(int j=N-1; j>=0; --j,--bj,--Ajj) if (*bj != T2(0)) {
	...
	const GenLowerTriMatrix<T1>& A, const VectorView<T2>& b)
  {
    ...
    if (A.isunit()) {
      for(size_t i=i1+1,len=1;i<N;++i,++len,++bi) {
	T2 temp;
	if (A.isconj()) 
	...
	const GenLowerTriMatrix<T1>& A, const VectorView<T2>& b)
  {
    ...
    if (A.isunit()) {
      for(size_t j=i1,jp1=j+1,len=N-jp1;len>0;++j,++jp1,--len,++bj,++bjp1) {
	if (*bj != T2(0)) {
	  if (A.isconj())
	    DoAddVV(-*bj,CVIt<T1,Unit,Conj>(A.col(j,jp1,N).begin()),bjp1,len);
	  else
	    DoAddVV(-*bj,CVIt<T1,Unit,NonConj>(A.col(j,jp1,N).begin()),bjp1,
		len);
	}
      }
    } else {
      CVIter<T1> Ajj = A.diag().begin()+i1;
      for(size_t j=i1,jp1=j+1,len=N-jp1;len>0;++j,++jp1,--len,++Ajj,++bj,++bjp1) {
	if (*bj != T2(0)) {
...
#define InstFile "TMV_TriLUDiv.inst"
#include "TMV_Inst.h"
#undef InstFile
