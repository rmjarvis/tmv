...
//        The second version is included for easier conversion of code 
//        written for Matrix.  colsize must equal rowsize.
//
...
namespace tmv {
  ...
  template <class T> class GenDiagMatrix : public BaseMatrix<T>
  {
	...
	return ConstDiagMatrixView<T>(diag().Conjugate());
      ...
      inline void DInverse(DiagMatrix<T>& minv) const
      { DInverse(minv.View()); }

      inline DiagMatrix<T> DInverse() const
      { 
	DiagMatrix<T> minv(size());
	DInverse(minv.View());
	return minv;
      }

      inline DiagMatrix<T> Inverse() const
      { return DInverse(); }
	...
	DInverse(DiagMatrixViewOf(minv.diag()));
      ...
      {
	DiagMatrix<T> minv(size());
	DInverseATA(minv.View());
	return minv;
      }

      inline DiagMatrix<T> InverseATA() const
      { return DInverseATA(); }
    ...
    public GenDiagMatrix<T>
  {
    ...
    public:
      ...
      DiagMatrix(const GenDiagMatrix<T>& rhs) : itsdiag(rhs.diag()) {}
	...
	return *this; 
      ...
      inline DiagMatrix<T>& operator=(T x) { View() = x; return *this; }
...
//---------------------------------------------------------------------------
  ...
  { return ConstDiagMatrixView<T>(VectorViewOf(v,size,step)); }

  template <class T> inline DiagMatrixView<T> DiagMatrixViewOf(
      T* v, size_t size, int step=1)
  { return DiagMatrixView<T>(VectorViewOf(v,size,step)); }
  ...
  template <class T> inline DiagMatrix<T> InverseATA(const GenDiagMatrix<T>& m)
  { return m.DInverseATA(); }
  ...
  template <class T> inline string Type(const DiagMatrix<T>& m)
  { return string("DiagMatrix<")+Type(T())+">"; }
  template <class T> inline string Type(const GenDiagMatrix<T>& m)
  { return string("GenDiagMatrix<")+Type(T())+","+Text(m.diag().ct())+">"; }
  template <class T> inline string Type(const ConstDiagMatrixView<T>& m)
  { return string("ConstDiagMatrixView<")+Type(T())+","+Text(m.diag().ct())+">"; }
  template <class T> inline string Type(const DiagMatrixView<T>& m)
  { return string("DiagMatrixView<")+Type(T())+","+Text(m.diag().ct())+">"; }
