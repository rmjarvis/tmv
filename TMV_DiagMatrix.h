...
namespace tmv {
    ...
    public GenDiagMatrix<T>
  {
    public :
      ...
      ConstDiagMatrixView(const ConstDiagMatrixView<T,I>& rhs) :
    ...
    private :
      ...
      void operator=(const ConstDiagMatrixView<T,I>&) { TMVAssert(false); }
    ...
    public ConstDiagMatrixView<T,CStyle>
  {
    public :
      ...
      ConstDiagMatrixView(const ConstDiagMatrixView<T,FortranStyle>& rhs) :
    ...
    private :
      ...
      void operator=(const ConstDiagMatrixView<T,FortranStyle>&) 
      { TMVAssert(false); }
    ...
    public GenDiagMatrix<T>
  {
    ...
    public:
      ...
      DiagMatrixView(const DiagMatrixView<T,I>& rhs) :
	GenDiagMatrix<T>(rhs), itsdiag(rhs.diag()) {}

      explicit DiagMatrixView(const VectorView<T>& _diag) :
	itsdiag(_diag) {}

      template <IndexStyle I2> explicit DiagMatrixView(Vector<T,I2>& _diag) :
	itsdiag(_diag.View()) {}
    ...
    public DiagMatrixView<T,CStyle>
  {
    ...
    public:
      ...
      DiagMatrixView(const DiagMatrixView<T,FortranStyle>& rhs) :
	DiagMatrixView<T,CStyle>(rhs) {}

      DiagMatrixView(const DiagMatrixView<T,CStyle>& rhs) :
	DiagMatrixView<T,CStyle>(rhs) {}
    ...
    public GenDiagMatrix<T>
  {
    ...
    public:
      ...
      DiagMatrix(const DiagMatrix<T,I>& rhs) : itsdiag(rhs.diag()) {}
	...
	itsdiag(rhs.diag()) {}
      ...
      DiagMatrix(const DiagMatrixComposite<T>& mcomp) : itsdiag(mcomp.size())
      { mcomp.AssignTo(View()); }
	...
	if (i==j) return operator()(i);
      ...
      { return DiagMatrixView<T,I>(itsdiag); }
...
//---------------------------------------------------------------------------
  ...
  template <class T, IndexStyle I> inline string Type(const DiagMatrix<T,I>& m)
