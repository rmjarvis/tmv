...
namespace tmv {
  ...
  template <class T1, class T2, StepItType S2, ConjItType C2, StepItType S3> 
    ...
    { for(size_t i=size;i>0;--i,++it0,++it1) GivensMult(c,s,*it0,*it1); }
  ...
  //
  template <class T1, class T2> inline void GivensHermMult(
      RealType(T1) c, T1 s, T2& d0, T2& d1, T2& e0) 
  {
    // [ d0 e0* ] = [  c  s ] [ d0 e0* ] [ c  -s ]
    // [ e0 d1  ]   [ -s* c ] [ e0 d1  ] [ s*  c ]
    ...
    RealType(T1) s2 = NORM(s);
    RealType(T2) Rese0 = REAL(s*e0);
    T2 d1md0 = d1-d0;
    T2 dd = RealType(T1)(2)*c*Rese0 + s2*d1md0;
    d0 += dd;
    d1 -= dd;
    e0 += CONJ(s)*(c*d1md0 - RealType(T1)(2)*Rese0);
  }
  template <class T1, class T2> inline void GivensSymMult(
      RealType(T1) c, T1 s, T2& d0, T2& d1, T2& e0) 
  {
    // [ d0 e0 ] = [  c  s ] [ d0 e0 ] [ c -s* ]
    // [ e0 d1 ]   [ -s* c ] [ e0 d1 ] [ s  c  ]
    ...
    T2 se0 = s*e0;
    T2 d1md0 = d1-d0;
    T2 dd = RealType(T1)(2)*c*se0 + s2*d1md0;
    d0 += dd;
    d1 -= dd;
    if (IsReal(T1()))
      e0 += s*(c*d1md0 - RealType(T1)(2)*se0);
    else
      e0 += c*(s*d1-CONJ(s)*d0) - RealType(T1)(2)*CONJ(s)*se0;
  }
