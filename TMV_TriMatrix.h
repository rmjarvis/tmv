...
namespace tmv {
    ...
    public BaseMatrix<T>
  {
	...
	return ConstUpperTriMatrixView<RealType(T)>(
      ...
      void TInverse(const UpperTriMatrixView<T>& minv) const;

      inline UpperTriMatrix<T,NonUnitDiag,ColMajor> TInverse() const
      { 
	UpperTriMatrix<T,NonUnitDiag,ColMajor> minv(size());
	TInverse(minv.View());
	return minv;
      }

      template <DiagType D, StorageType S> inline void TInverse(
	  UpperTriMatrix<T,D,S>& minv) const
      { TInverse(minv.View()); }

      inline void Inverse(const MatrixView<T>& minv) const
      {
	TInverse(UpperTriMatrixViewOf(minv));
	if (size() > 0) LowerTriMatrixViewOf(minv).OffDiag().Zero();
      }
    ...
    public BaseMatrix<T>
  {
	...
	return ConstLowerTriMatrixView<RealType(T)>(
      ...
      void TInverse(const LowerTriMatrixView<T>& minv) const;

      inline LowerTriMatrix<T,NonUnitDiag,ColMajor> TInverse() const
      { 
	LowerTriMatrix<T,NonUnitDiag,ColMajor> minv(size());
	TInverse(minv.View());
	return minv;
      }
      ...
      inline void Inverse(const MatrixView<T>& minv) const
      {
	TInverse(LowerTriMatrixViewOf(minv));
	if (size() > 0) UpperTriMatrixViewOf(minv).OffDiag().Zero();
      }
