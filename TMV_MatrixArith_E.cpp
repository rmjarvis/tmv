...
namespace tmv {
      ...
      const GenVector<Ty>& y, const MatrixView<T>& A)
  {
    ...
    for (size_t i=0;i<A.colsize();++i,++xit) {
      ...
      if (IMAG(ax) == RealType(T)(0)) AddVV(REAL(ax),y,A.row(i));
      else AddVV(ax,y,A.row(i));
    }
  }
      ...
      const GenVector<Ty>& y, const MatrixView<T>& A)
  {
    ...
    CVIter<Ty> yit = y.begin();
    if (alpha == T(1))
      for (size_t j=0;j<A.rowsize();++j,++yit) AddVV(*yit,x,A.col(j));
    else if (IMAG(alpha) == RealType(T)(0))
      for (size_t j=0;j<A.rowsize();++j,++yit) 
	AddVV(*yit*REAL(alpha),x,A.col(j));
    else
      for (size_t j=0;j<A.rowsize();++j,++yit) AddVV(*yit*alpha,x,A.col(j));
  }
