...
namespace tmv {
  ...
  // in v, except for the first element.
  ...
  // (For the first column, this means that y is the first element.)
  ...
  // H z = z - beta v vt z
  template <class T1, class T2> inline void Householder_LDiv(
      const GenVector<T1>& v, T1 beta, const VectorView<T2>& z)
    // z <- H^-1 z = Ht z = (I - beta* v vt) z
  { Householder_LMult(v,CONJ(beta),z); }
  template <class T1, class T2> inline void Householder_RMult(
      const GenVector<T1>& v, T1 beta, const VectorView<T2>& z)
    // z <- z H = z H = z (I - beta v vt) 
    // zT = (I - beta v* vT) zT
  { Householder_LMult(v.Conjugate(),beta,z); }
  template <class T1, class T2> inline void Householder_RDiv(
      const GenVector<T1>& v, T1 beta, const VectorView<T2>& z)
    // z <- z H^-1 = z Ht = z (I - beta* v vt) 
    // zT = (I - beta* v* vT) zT
  { Householder_LMult(v.Conjugate(),CONJ(beta),z); }
  ...
  template <class T1, class T2> inline void Householder_LDiv(
      const GenVector<T1>& v, T1 beta, const MatrixView<T2>& M)
  { Householder_LMult(v,CONJ(beta),M); }
  template <class T1, class T2> inline void Householder_RMult(
      const GenVector<T1>& v, T1 beta, const MatrixView<T2>& M)
  { Householder_LMult(v.Conjugate(),beta,M.Transpose()); }
  template <class T1, class T2> inline void Householder_RDiv(
      const GenVector<T1>& v, T1 beta, const MatrixView<T2>& M)
  { Householder_LMult(v.Conjugate(),CONJ(beta),M.Transpose()); }
  ...
  template <class T1, class T2> inline void Householder_LRDiv(
      const GenVector<T1>& v, T1 beta, const SymMatrixView<T2>& M)
  { Householder_LRMult(v,CONJ(beta),M); }
