...
namespace tmv {
  ...
  template <class T> T Householder_Reflect(const VectorView<T>& v, T& det)
  {
    // Finds the Householder matrix H which rotates v into (y,0,0,0..0).  
    ...
    // Determine normx = |x|
    VectorView<T> v1 = v.SubVector(1,v.size());
    RealType(T) normsqx = NormSq(v1);
    const T x0 = v(0);
  ...
  template <class T> T Householder_Reflect(const MatrixView<T>& m, T& det)
  {
    ...
    T beta = Householder_Reflect(m.col(0),det);
    if (beta != T(0)) 
      Householder_LMult(m.col(0,1,m.colsize()),beta,m.Cols(1,m.rowsize()));
    return beta;
  }
      ...
      const GenVector<T1>& v, T1 beta, const MatrixView<T2>& m)
  {
    ...
    else if (m.rowsize() == 1) Householder_LMult(v,beta,m.col(0));
    else if (beta != T1(0)) {
      ...
      const GenVector<T1>& v, T1 beta, const SymMatrixView<T2>& m)
  {
	...
	mv = m*vv.Conjugate();
      ...
      Rank2Update(T2(-beta),vv,mv,m);
      if (IsComplex(T2()) && m.isherm()) {
	// imag part is 0 - make it exact.
        betasqvtmv = REAL(betasqvtmv);
	m += betasqvtmv * (vv^vv.Conjugate());
      } else {
	m += betasqvtmv * (vv^vv);
      }
    }
  ...
  template <class T, class T2> void BlockHouseholder_RMult(
      const GenMatrix<T>& Y, const GenUpperTriMatrix<T>& Z,
      const MatrixView<T2>& m)
  {
    // The routine finds m <- m (I - YZYt) 

    TMVAssert(Z.size() == Y.rowsize());
    TMVAssert(Y.rowsize() > 0);
    TMVAssert(Y.colsize() > 0);
    TMVAssert(m.rowsize() == Y.colsize());
    
    size_t M = Y.colsize();
    size_t N = Y.rowsize();

    if (m.iscm()) {
      Matrix<T2,ColMajor> mYZ = m.Cols(0,N) * 
	LowerTriMatrixViewOf(Y.Rows(0,N),UnitDiag);
      mYZ += m.Cols(N,M) * Y.Rows(N,M);
      mYZ *= Z;
      m.Cols(N,M) -= mYZ * Y.Rows(N,M).Adjoint();
      mYZ *= LowerTriMatrixViewOf(Y.Rows(0,N),UnitDiag).Adjoint();
      m.Cols(0,N) -= mYZ;
    } else {
      Matrix<T2,RowMajor> mYZ = m.Cols(0,N) * 
	LowerTriMatrixViewOf(Y.Rows(0,N),UnitDiag);
      mYZ += m.Cols(N,M) * Y.Rows(N,M);
      mYZ *= Z;
      m.Cols(N,M) -= mYZ * Y.Rows(N,M).Adjoint();
  ...
  template <class T, class T2> void BlockHouseholder_RDiv(
      const GenMatrix<T>& Y, const GenUpperTriMatrix<T>& Z,
      const MatrixView<T2>& m)
  {
    // The routine finds m <- m (I - YZYt)^-1
    // = m (I - YZtYt)
    ...
    if (m.iscm()) {
      Matrix<T2,ColMajor> mYZt = m.Cols(0,N) * 
	LowerTriMatrixViewOf(Y.Rows(0,N),UnitDiag);
      mYZt += m.Cols(N,M) * Y.Rows(N,M);
      mYZt *= Z.Adjoint();
      m.Cols(N,M) -= mYZt * Y.Rows(N,M).Adjoint();
      mYZt *= LowerTriMatrixViewOf(Y.Rows(0,N),UnitDiag).Adjoint();
      m.Cols(0,N) -= mYZt;
    } else {
      Matrix<T2,RowMajor> mYZt = m.Cols(0,N) * 
	LowerTriMatrixViewOf(Y.Rows(0,N),UnitDiag);
      mYZt += m.Cols(N,M) * Y.Rows(N,M);
      mYZt *= Z.Adjoint();
      m.Cols(N,M) -= mYZt * Y.Rows(N,M).Adjoint();
