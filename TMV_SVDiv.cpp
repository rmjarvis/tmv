...
#undef APTR
      ...
      const BaseMatrix<T>& m, ostream* fout) const
  {
    ...
    RealType(T) nm = Norm(usv-(istrans ? mm->Transpose() : mm->View()));
    ...
    return nm < Condition()*mm->colsize()*Epsilon<T>();
  }
      ...
      const VectorView<RealType(T)>& E, T& det)
  {
    ...
    B.diag(1) = E;
    Matrix<T> AA = U*B*V;
    if (Norm(A0-AA) > 0.001*max(RealType(T)(1),Norm(A0))) {
  ...
  template <class TUV, class T> void ReduceUnredBidiagonal_DC(
      const MatrixView<TUV>* U, const VectorView<T>& D,
      const VectorView<T>& E, const MatrixView<TUV>* V)
  {
    // Reduce the superdiagonal elements of unreduced Bidiagonal Matrix B 
  ...
  template <class TUV, class T> void ReduceUnredBidiagonal(
      ...
      const VectorView<T>& E, const MatrixView<TUV>* V)
  {
    ReduceUnredBidiagonal_QR(U,D,E,V);
  }
      ...
      const VectorView<RealType(T)>& E, const MatrixView<T>* V, bool SetUV)
  {
    ...
    //if (U) cerr<<"U = "<<Type(*U)<<"  "<<U->colsize()<<"  "<<U->rowsize()<<endl;

    //if (V) cerr<<"V = "<<Type(*V)<<"  "<<V->colsize()<<"  "<<V->rowsize()<<endl;
      ...
      const VectorView<RealType(T)>& S, T& det)
  {
    ...
    if (M > 5*N/3) {
      ...
      *UV2 = UpperTriMatrixViewOf(UV);
      ...
      //gettimeofday(&tp,0);
      //double t3 = tp.tv_sec + tp.tv_usec/1.e6;
      //cerr<<"Second SV_Decomp: "<<t3-t2<<" seconds\n";
      ...
      //gettimeofday(&tp,0);
      //double t2 = tp.tv_sec + tp.tv_usec/1.e6;
      //cerr<<"Decomp from Bidiagonal: "<<t2-t1b<<" seconds\n";
    }
    //gettimeofday(&tp,0);
    //double tx = tp.tv_sec + tp.tv_usec/1.e6;
    //cerr<<"Total time: "<<tx-t0<<" seconds\n";
