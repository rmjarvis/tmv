...
#define RefType(T) typename VIter<T>::reference
  template <class T> inline T& REF(T* vi, ConjItType )
  { return *vi; }
    ...
    public ReadError
  {
    public :
      ...
      inline VectorReadError(size_t _i, const GenVector<T>& _v, istream& _is) :
	ReadError("Vector"),
	i(_i), v(new Vector<T>(_v)), exp(0), got(0), s(_v.size()),
	is(_is), iseof(_is.eof()), isbad(_is.bad()) {}
      inline VectorReadError(istream& _is) :
	ReadError("Vector"),
	...
	is(_is), iseof(_is.eof()), isbad(_is.bad()) {}
      inline VectorReadError(const GenVector<T>& _v, istream& _is, size_t _s) :
	ReadError("Vector"),
	i(0), v(new Vector<T>(_v)), exp(0), got(0), s(_s),
	is(_is), iseof(_is.eof()), isbad(_is.bad()) {}

      inline VectorReadError(const VectorReadError<T>& rhs) :
	i(rhs.i), v(rhs.v), exp(rhs.exp), got(rhs.got), s(rhs.s),
	is(rhs.is), iseof(rhs.iseof), isbad(rhs.isbad) {}
      virtual inline ~VectorReadError() throw() {}
  ...
  template <class T> class GenVector
  {
    public:
      ...
      inline GenVector(ConjItType ct) : itsct(ct) {}
      inline GenVector(const GenVector<T>& rhs) : itsct(rhs.itsct) {}
	...
	return cref(i); 
      ...
      { return const_iterator(cptr(),step(),itsct); }
    ...
    public GenVector<T>
  {
	...
	itsv(rhs.itsv), itssize(rhs.itssize), itsstep(rhs.itsstep)
      ...
      inline VectorView(T* inv, size_t insize, int instep, ConjItType inct 
	  PARAMFIRSTLAST(T) ) :
	GenVector<T>(inct), itsv(inv), itssize(insize), itsstep(instep)
	...
	return ref(i); 
      ...
      { return VIter<T>(ptr(),step(),GenVector<T>::ct() FIRSTLAST ); }
      inline iterator end() const 
      { return begin() + size(); }
      inline reverse_iterator rbegin() const 
      {
	return VIter<T>(ptr()+step()*(size()-1),-step(),GenVector<T>::ct() 
	  FIRSTLAST ); 
      }
      ...
      inline const VectorView<T,I>& Zero() const 
      {
	...
	return VectorView<T,I>(ptr(),size(),step(),
	    ConjOf(T,GenVector<T>::ct()) FIRSTLAST ); 
      }
	...
	return VectorView<RealType(T),I>(
      ...
      { 
	TMVAssert(IsComplex(T()));
	TMVAssert(GenVector<T>::ct() == NonConj);
	return VectorView<RealType(T),I>(
...
#define NEW_SIZE(n) \
	...
	SetAllTo(val); 
      ...
      explicit inline Vector(const valarray<T>& vv) : NEW_SIZE(vv.size())
      { 
	T* p = itsv.get();
	...
	memmove(itsv.get(),vv,n*sizeof(T)); 
      ...
      explicit inline Vector(const vector<T>& vv) : NEW_SIZE(vv.size())
      {
	T* p = itsv.get();
	for(size_t i=0;i<size();++i,++p) *p = vv[i]; 
      }
	...
	memmove(itsv.get(),rhs.cptr(),itssize*sizeof(T)); 
      ...
      inline Vector(const GenVector<T>& rhs) : NEW_SIZE(rhs.size())
      { 
	...
	Copy(rhs,View()); 
      ...
      explicit inline Vector(const VectorComposite<T>& vcomp) :
	NEW_SIZE(vcomp.size())
      { 
	vcomp.AssignTo(View()); 
      }
...
#undef NEW_SIZE
  ...
  template <class T> inline void DoCopy(
      const GenVector<complex<T> >&, const VectorView<T>&)
  { TMVAssert(FALSE); }
  template <class T, class T1> inline void Copy(
      const GenVector<T1>& v1, const VectorView<T>& v2)
  { 
  ...
  template <class T, IndexStyle I> inline string Type(const Vector<T,I>& )
  ...
  template <class T, IndexStyle I> inline string Type(
      const ConstVectorView<T,I>& v)
  { 
    return string("ConstVectorView<")+Type(T())+","+Text(I)+","+
      Text(v.ct())+">"; 
  }
  template <class T, IndexStyle I> inline string Type(const VectorView<T,I>& v)
  { 
