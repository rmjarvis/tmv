...
namespace tmv {
  ...
  template <bool cd, class T, class Td> void DoDiagLDivEq(
      const GenDiagMatrix<Td>& d, const VectorView<T>& v)
  {
	...
	if (*di == Td(0)) tmv_error("Division by 0 in DiagMatrix /");
...
#ifdef XDEBUG
    Vector<T> v1 = d*v;
    if (Norm(v1-v0) > 0.001*max(RealType(T)(1),Norm(v0))) {
      cerr<<"DiagLDivEq v: \n";
  ...
  template <bool rm, bool cd, class T, class Td> void RowDiagLDivEq(
  ...
  template <bool cm, bool ca, class T, class Ta> extern void DoColMultEqMM(
      const GenDiagMatrix<Ta>& A, const MatrixView<T>& B);

  template <bool cm, bool cd, class T, class Td> void ColDiagLDivEq(
      const GenDiagMatrix<Td>& d, const MatrixView<T>& m)
  {
    TMVAssert(d.size() == m.colsize());
    TMVAssert(m.rowsize() > 0);
...
#ifdef XDEBUG
    Matrix<T> m1 = d*m;
    if (Norm(m1-m0) > 0.001*max(RealType(T)(1),Norm(m0))) {
      cerr<<"DiagLDivEq m: \n";
      cerr<<"d = "<<Type(d)<<"  "<<d<<endl;
