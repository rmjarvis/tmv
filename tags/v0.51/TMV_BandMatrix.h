...
namespace tmv {
    ...
    public BaseMatrix<T>
  {
	...
	return BandMatrixView<T>(const_cast<T*>(cptr()),colsize(),rowsize(),
	    nlo(),nhi(),stepi(),stepj(),diagstep(),stor(),itsct,linsize
...
#undef NEW_SIZE
	...
	return BandMatrixView<T>(ptr(),rowsize(),colsize(),nhi(),nlo(),
      ...
      {
	return VectorView<T>(itsm1,ls(),1,NonConj FIRSTLAST );
      }
...
//---------------------------------------------------------------------------
  ...
  { return string("BandMatrix<")+Type(T())+","+Text(S)+">"; }
  ...
  template <class T> inline string Type(const ConstBandMatrixView<T>& m)
  { 
    return string("ConstBandMatrixView<")+Type(T())+","+Text(m.ct())+
      ","+Text(m.stor())+">"; 
  }
  template <class T> inline string Type(const BandMatrixView<T>& m)
  {
    return string("BandMatrixView<")+Type(T())+","+Text(m.ct())+
      ","+Text(m.stor())+">"; 
  }
