...
namespace tmv {
  ...
  template <bool b0, bool cx, bool ca, bool cm, class T, class Ta, class Tx> 
	...
	const VectorView<T>& y)
    {
      ...
      for(size_t j=N; j>0; --j,++xj,A0j+=sj) 
	// y += *xj * A.col(j)
	if (*xj != Tx(0)) {
	  const Ta* Aij = A0j;
	  T* yi = y0;
...
#ifdef BLAS
  // MKL gemv is not very accurate for large matrices
  // And for small matrices, it's not faster, so just skip blas in this case.
  // (Also, it is slower for non-unit steps.)
#ifndef MKL 
  ...
  { 
    ...
    if (Norm(y-y2) > 0.001*max(RealType(T)(1),
	  abs(alpha)*Norm(A)*Norm(x)+abs(beta)*Norm(y0))) {
      cerr<<"MultMV: alpha,beta = "<<alpha<<"  "<<beta<<endl;
      cerr<<"A = "<<Type(A);
      if (A.rowsize() < 30 && A.colsize() < 30) cerr<<"  "<<A;
      ...
      cerr<<endl<<"xptr = "<<x.cptr()<<", yptr = "<<y.cptr()<<endl;
      if (y.size() < 200) {
	cerr<<"--> y = "<<y<<endl;
	cerr<<"y2 = "<<y2<<endl;
      } else {
