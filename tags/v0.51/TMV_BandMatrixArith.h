...
namespace tmv {
    ...
    public GenBandMatrix<T>
  {
	...
	GenBandMatrix<T>(s,NonConj,s==DiagMajor?0:1), 
      ...
      virtual ~BandMatrixComposite() { if (itsm1) delete [] itsm1; };
    ...
    public BandMatrixComposite<T> 
  {
	...
	if (m0.SameStorageAs(*m2)) {
	  m0 = *m2;
	  AddMM(x3,*m4,x1,m0);
	} else if (m0.SameStorageAs(*m4) || x1 != T(1)) {
	  m0 = *m4;
	  AddMM(x1,*m2,x3,m0);
	} else {
	  m0 = *m2;
  ...
  template <class T, class T2, class T3> class ProdBB : 
    public BandMatrixComposite<T>
  {
    public:
  ...
  template <class T, class T2, class T3>
	...
	const BandMatrixView<T>& m, const ProdBB<T,T2,T3>& pmm)
    {
