...
//    implies that their diagonal elements are real.  Most routines
...
//    SymMatrix<T,uplo,stor>(size_t colsize, size_t rowsize)
//        Makes a Symmetric Matrix with column size = row size = n
//        with _uninitialized_ values.
//        For the second version, colsize must equal rowsize - it is 
//        there for easier conversion from routines written for Matrix.
//
//    SymMatrix<T,uplo,stor>(size_t n, T x)
//    SymMatrix<T,uplo,stor>(size_t colsize, size_t rowsize, T x)
...
#define UTransOf(U) (U==Upper ? Lower : Upper)
    ...
    public BaseMatrix<T>
  {
	...
	return SymMatrixView<T>(const_cast<T*>(cptr()),size(),
      ...
      { return UpperTri().MaxAbsElement(); }

      // MJ: The inverse is also symmetric.  Consider making an SInverse
      // function which uses this fact.  Need a SymDivider class like for Tri.
      
      using BaseMatrix<T>::Inverse;
      using BaseMatrix<T>::InverseATA;

      template <StorageType S> inline void Inverse(
	  Matrix<T,S>& minv) const
      { Inverse(minv.View()); }

      inline Matrix<T,ColMajor> Inverse() const
      {
	Matrix<T,ColMajor> minv(size(),size());
	Inverse(minv.View());
	return minv;
      }
	...
	if (uplo() == Upper)
      ...
      {
	TMVAssert(dt == CH || dt == LU || dt == SV || dt == SVF || dt == SVS);
    ...
    public GenSymMatrix<T>
  {
	...
	UpperTri().SetAllTo(x); return *this; 
      ...
      { UpperTri().ConjugateSelf(); return *this; }
...
#define NEW_SIZE(s) GenSymMatrix<T>(Sym,U,S,NonConj), itslen((s)*(s)), \
	...
	SetAllTo(T(888));
      ...
      SymMatrix(size_t cs, size_t rs) : NEW_SIZE(cs) 
      {
	TMVAssert(cs == rs);
	TMVAssert(S==RowMajor || S==ColMajor);
#ifdef TMVDEBUG
	...
	SetAllTo(x);
      ...
      SymMatrix(size_t cs, size_t rs, const T x) : NEW_SIZE(cs) 
      { 
	TMVAssert(cs == rs);
	TMVAssert(S==RowMajor || S==ColMajor);
	SetAllTo(x);
	...
	for(size_t i=0;i<itslen;++i) *vi=vv[i];
      ...
      SymMatrix(size_t cs, size_t rs, const valarray<T>& vv) : NEW_SIZE(cs)
      {
	TMVAssert(cs == rs);
	TMVAssert(S==RowMajor || S==ColMajor);
	TMVAssert(vv.size() == itslen);
	...
	memmove(itsm,vv,itslen*sizeof(T));
      ...
      SymMatrix(size_t cs, size_t rs, const T* vv) : NEW_SIZE(cs)
      {
	TMVAssert(cs == rs);
	TMVAssert(S==RowMajor || S==ColMajor);
	memmove(itsm,vv,itslen*sizeof(T));
	...
	for(size_t i=itslen;i>0;--i) *vi = *vvi;
      ...
      SymMatrix(size_t cs, size_t rs, const vector<T>& vv) : NEW_SIZE(cs)
      {
	TMVAssert(cs == rs);
	TMVAssert(S==RowMajor || S==ColMajor);
	TMVAssert(vv.size() == itslen);
...
#undef NEW_SIZE
	...
	return *this;
      ...
      { UpperTri() = m2.UpperTri(); return *this; }
...
#define NEW_SIZE(s) GenSymMatrix<T>(Herm,U,S,NonConj), itslen((s)*(s)), \
	...
	SetAllTo(RealType(T)(888));
      ...
      HermMatrix(size_t cs, size_t rs) : NEW_SIZE(cs) 
      { 
	TMVAssert(cs == rs);
	TMVAssert(S==RowMajor || S==ColMajor);
#ifdef TMVDEBUG
	...
	SetAllTo(x);
      ...
      HermMatrix(size_t cs, size_t rs, const RealType(T) x) : NEW_SIZE(cs) 
      { 
	TMVAssert(cs == rs);
	TMVAssert(S==RowMajor || S==ColMajor);
	SetAllTo(x);
	...
	for(size_t i=0;i<itslen;++i) *vi=vv[i];
      ...
      HermMatrix(size_t cs, size_t rs, const valarray<T>& vv) : NEW_SIZE(cs)
      {
	TMVAssert(cs == rs);
	TMVAssert(S==RowMajor || S==ColMajor);
	TMVAssert(vv.size() == itslen);
	...
	memmove(itsm,vv,itslen*sizeof(T));
      ...
      HermMatrix(size_t cs, size_t rs, const T* vv) : NEW_SIZE(cs)
      {
	TMVAssert(cs == rs);
	TMVAssert(S==RowMajor || S==ColMajor);
	memmove(itsm,vv,itslen*sizeof(T));
	...
	for(size_t i=itslen;i>0;--i) *vi = *vvi;
      ...
      HermMatrix(size_t cs, size_t rs, const vector<T>& vv) : NEW_SIZE(cs)
      {
	TMVAssert(cs == rs);
	TMVAssert(S==RowMajor || S==ColMajor);
	TMVAssert(vv.size() == itslen);
...
#undef NEW_SIZE
	...
	return VectorView<T>(ptr()+i*(U==Upper?stepj():stepi())+j1*ds,j2-j1,
      ...
      { return SetAllTo(0); }
      inline HermMatrix<T,U,S>& SetAllTo(RealType(T) x) 
      { UpperTri().SetAllTo(x); return *this; }
