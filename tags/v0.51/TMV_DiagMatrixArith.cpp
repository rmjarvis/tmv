...
namespace tmv {
  ...
  template <bool rm, bool ca, class T, class Ta, class Tb>
    inline void RowAddMultEqMM(
	const GenDiagMatrix<Ta>& A, const GenMatrix<Tb>& B,
	const MatrixView<T>& C)
    {
      if (B.isconj()) DoRowAddMultEqMM<rm,ca,true>(A,B,C);
      else DoRowAddMultEqMM<rm,ca,false>(A,B,C);
    }

  template <bool cm, bool ca, bool cb, class T, class Ta, class Tb> 
    void DoColAddMultEqMM(
	const GenDiagMatrix<Ta>& A, const GenMatrix<Tb>& B,
	const MatrixView<T>& C)
    {
      // C += A * B 
  ...
  template <bool cm, bool ca, class T, class Ta, class Tb> 
    inline void ColAddMultEqMM(
	const GenDiagMatrix<Ta>& A, const GenMatrix<Tb>& B,
	const MatrixView<T>& C)
    { 
      if (A.diag().step() == 1)
	if (B.isconj())
	  DoColAddMultEqMM<cm,ca,true>(A,B,C);
	else
	  DoColAddMultEqMM<cm,ca,false>(A,B,C);
      else {
	DiagMatrix<Ta> AA = A;
	if (B.isconj())
	  DoColAddMultEqMM<cm,ca,true>(AA,B,C);
	else
	  DoColAddMultEqMM<cm,ca,false>(AA,B,C);
      }
    }
    ...
    // C += alpha * A * B
  {
	...
	if (IMAG(alpha) == RealType(T)(0)) {
	  DiagMatrix<Ta> AA = REAL(alpha)*A;
	  if (B.isrm() && C.isrm()) 
	    RowAddMultEqMM<true,false>(AA,B,C);
	  else if (B.iscm() && C.iscm()) 
	    ColAddMultEqMM<true,false>(AA,B,C);
	  else if (B.colsize() > B.rowsize()) 
	    ColAddMultEqMM<false,false>(AA,B,C);
	  else 
	    RowAddMultEqMM<false,false>(AA,B,C);
	} else {
	  DiagMatrix<T> AA = alpha*A;
	  if (B.isrm() && C.isrm()) 
	    RowAddMultEqMM<true,false>(AA,B,C);
        ...
        if (B.isrm() && C.isrm()) 
	  RowAddMultEqMM<true,true>(A,B,C);
	else if (B.iscm() && C.iscm()) 
	  ColAddMultEqMM<true,true>(A,B,C);
	else if (B.colsize() > B.rowsize()) 
	  ColAddMultEqMM<false,true>(A,B,C);
	else 
	  RowAddMultEqMM<false,true>(A,B,C);
      } else {
        if (B.isrm() && C.isrm()) 
	  RowAddMultEqMM<true,false>(A,B,C);
	else if (B.iscm() && C.iscm()) 
	  ColAddMultEqMM<true,false>(A,B,C);
	else if (B.colsize() > B.rowsize()) 
	  ColAddMultEqMM<false,false>(A,B,C);
	else 
	  RowAddMultEqMM<false,false>(A,B,C);
      }
    }
