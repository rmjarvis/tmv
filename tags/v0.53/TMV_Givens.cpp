...
namespace tmv {
  ...
  // GivensMult Scalars
  //
  template <class Tg, class T> inline void GivensMult(
  ...
  template <class Tg, class T> inline void GivensMult(
      RealType(Tg) c, Tg s, ConjRef<T> x, T& y)
  {
    T xx = c*x+s*y;
    y = c*y-tmv::CONJ(s)*x;
    x = xx;
  ...
  template <class Tg, class T> inline void GivensMult(
      RealType(Tg) c, Tg s, T& x, ConjRef<T> y)
  {
    T xx = c*x+s*y;
    y = c*y-tmv::CONJ(s)*x;
    x = xx;
  ...
  template <class Tg, class T> inline void GivensMult(
      RealType(Tg) c, Tg s, VarConjRef<T> x, T& y)
  {
    T xx = c*x+s*y;
    y = c*y-tmv::CONJ(s)*x;
    x = xx;
  ...
  template <class Tg, class T> inline void GivensMult(
      RealType(Tg) c, Tg s, T& x, VarConjRef<T> y)
  {
    T xx = c*x+s*y;
    y = c*y-tmv::CONJ(s)*x;
    x = xx;
  ...
  { 
    ...
    TMVAssert(v2.ct()==NonConj);
    cblas_drot(v1.size(),v1.ptr(),v1.step(),v2.ptr(),v2.step(),c,s); 
  }
  template <> inline void BlasGivensMult(
      double c, double s,
  ...
  { 
    ...
    TMVAssert(v2.ct()==NonConj);
    if (v1.isconj()) NonBlasGivensMult<true>(c,s,v1,v2);
    else cblas_zdrot(v1.size(),v1.ptr(),v1.step(),v2.ptr(),v2.step(),c,s); 
  }
#ifndef NOFLOAT
  template <> inline void BlasGivensMult(
      float c, float s,
  ...
  { 
    ...
    TMVAssert(v1.ct()==NonConj);
    TMVAssert(v2.ct()==NonConj);
    cblas_srot(v1.size(),v1.ptr(),v1.step(),v2.ptr(),v2.step(),c,s); 
  }
  template <> inline void BlasGivensMult(
      float c, float s,
  ...
  { 
    ...
    TMVAssert(v2.ct()==NonConj);
    if (v1.isconj()) NonBlasGivensMult<true>(c,s,v1,v2);
    else cblas_csrot(v1.size(),v1.ptr(),v1.step(),v2.ptr(),v2.step(),c,s); 
  }
#endif
#ifdef LAP
  template <> inline void BlasGivensMult(
      ...
      const VectorView<complex<double> >& v2)
  {
    ...
    if (v1.isconj()) NonBlasGivensMult<true>(c,s,v1,v2);
      ...
      int s1 = v2.step();
      double cc = c;
      zrot(&n,LAP_Complex(v1.ptr()),&s0,LAP_Complex(v2.ptr()),&s1,
	  &cc,LAP_Complex(&s)); 
    }
  }
#ifndef NOFLOAT
      ...
      const VectorView<complex<float> >& v2)
  {
    ...
    if (v1.isconj()) NonBlasGivensMult<true>(c,s,v1,v2);
      ...
      int s1 = v2.step();
      float cc = c;
      crot(&n,LAP_Complex(v1.ptr()),&s0,LAP_Complex(v2.ptr()),&s1,
	  &cc,LAP_Complex(&s)); 
    }
  }
#endif
...
#ifdef BLAS
    if (v2.step() > 0 && v2.step() > 0 && v1.ct() == NonConj)
  ...
  //
  template <class Tg, class T> inline void GivensHermMult(
  ...
  template <class Tg, class T> inline void GivensSymMult(
      RealType(Tg) c, Tg s, T& d0, T& d1, T& e0) 
  {
    // [ d0 e0 ] = [  c  s ] [ d0 e0 ] [ c -s* ]
    // [ e0 d1 ]   [ -s* c ] [ e0 d1 ] [ s  c  ]
