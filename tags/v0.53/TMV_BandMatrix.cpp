...
namespace tmv {
  ...
  { 
    ...
    if (m.iscm()) {
      ...
      norm = dlangb(&cc,&n,&kl,&ku,const_cast<double*>(m.cptr()-m.nhi()),
	  &lda,work);
      if (work) delete[] work;
    } else {
      norm = dlangt(&cc,&n,const_cast<double*>(m.cptr()+m.stepi()),
	  const_cast<double*>(m.cptr()),
	  const_cast<double*>(m.cptr()+m.stepj()));
    }
    return norm;
  }
  ...
  { 
    ...
    if (m.iscm()) {
      ...
      int lda = m.stepj()+1;
      double* work = (c == 'I' ? new double[n] : 0);
      norm = zlangb(&cc,&n,&kl,&ku,LAP_Complex(m.cptr()-m.nhi()),
	  &lda,work);
      if (work) delete[] work;
    } else {
      norm = zlangt(&cc,&n,LAP_Complex(m.cptr()+m.stepi()),
	  LAP_Complex(m.cptr()),LAP_Complex(m.cptr()+m.stepj()));
    }
    return norm;
  }
#ifndef NOFLOAT
  ...
  { 
    ...
    if (m.iscm()) {
      ...
      norm = slangb(&cc,&n,&kl,&ku,const_cast<float*>(m.cptr()-m.nhi()),
	  &lda,work);
      if (work) delete[] work;
    } else {
      norm = slangt(&cc,&n,const_cast<float*>(m.cptr()+m.stepi()),
	  const_cast<float*>(m.cptr()),
	  const_cast<float*>(m.cptr()+m.stepj()));
    }
    return norm;
  }
  ...
  { 
    ...
    if (m.iscm()) {
      ...
      int lda = m.stepj()+1;
      float* work = (c == 'I' ? new float[n] : 0);
      norm = clangb(&cc,&n,&kl,&ku,LAP_Complex(m.cptr()-m.nhi()),
	  &lda,work);
      if (work) delete[] work;
    } else {
      norm = clangt(&cc,&n,LAP_Complex(m.cptr()+m.stepi()),
	  LAP_Complex(m.cptr()),LAP_Complex(m.cptr()+m.stepj()));
    }
    return norm;
  }
  ...
  template <class T, IndexStyle I> void BandMatrixView<T,I>::Read(istream& is) const
  {
    char paren;
    size_t j1=0;
    size_t len=nhi()+1;
  ...
  template <class T, StorageType S, IndexStyle I> istream& operator>>(istream& is,
      BandMatrix<T,S,I>*& m)
  { 
    char b;
    is >> b;
    if (!is || b != 'B') 
      tmv_error("reading B in BandMatrix::Read");
    size_t cs,rs;
    int lo,hi;
    is >> cs >> rs >> lo >> hi;
    if (!is) 
      tmv_error("reading size in BandMatrix::Read");
    m = new BandMatrix<T,S,I>(cs,rs,lo,hi);
    m->View().Read(is); 
    return is;
  }
  ...
  { 
    ...
    is >> cs;
    if (!is)
      tmv_error("reading colsize in BandMatrix::Read");
    if (cs != m.colsize())
      tmv_error("colsize does not match in BandMatrix::Read");
    is >> rs;
    if (!is)
      tmv_error("reading rowsize in BandMatrix::Read");
    if (rs != m.rowsize())
      tmv_error("rowsize does not match in BandMatrix::Read");
    is >> lo;
    if (!is)
      tmv_error("reading nlo in BandMatrix::Read");
    if (lo != m.nlo())
      tmv_error("nlo does not match in BandMatrix::Read");
    is >> hi;
    if (!is)
      tmv_error("reading nhi in BandMatrix::Read");
    if (hi != m.nhi())
      tmv_error("nhi does not match in BandMatrix::Read");
