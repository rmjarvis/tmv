...
namespace tmv {
    ...
    public BaseMatrix<T>
  {
    ...
    public:
      ...
      GenBandMatrix(StorageType s, ConjItType c, size_t ls) :
	...
	return linsize > 0; 
      ...
      virtual ConstVectorView<T> ConstLinearView() const
      {
	TMVAssert(!isdm());
    ...
    public GenBandMatrix<T>
  {
    public :
      ...
      ConstBandMatrixView(const ConstBandMatrixView<T,I>& rhs) :
	...
	GenBandMatrix<T>(rhs.stor(),rhs.ct(),0), itsm(rhs.cptr()),
    ...
    public ConstBandMatrixView<T,CStyle>
  {
    public :
      ...
      ConstBandMatrixView(const ConstBandMatrixView<T,FortranStyle>& rhs) :
	...
	ConstBandMatrixView<T,CStyle>(rhs) {}
      ...
      ConstBandMatrixView(const GenBandMatrix<T>& rhs, int _lo, int _hi) :
	ConstBandMatrixView<T,CStyle>(rhs,_lo,_hi) {}
    ...
    public GenBandMatrix<T>
  {
    ...
    public:
      ...
      BandMatrixView(const BandMatrixView<T,I>& rhs) : 
	GenBandMatrix<T>(rhs), itsm(rhs.itsm), 
	itscs(rhs.itscs), itsrs(rhs.itsrs),
	itsnlo(rhs.itsnlo), itsnhi(rhs.itsnhi),
	itssi(rhs.itssi), itssj(rhs.itssj), itssd(rhs.itssd)
      ...
      BandMatrixView(const BandMatrixView<T,I>& rhs, int _lo, int _hi) :
	GenBandMatrix<T>(rhs.stor(),rhs.ct(),0), itsm(rhs.ptr()),
	itscs(rhs.itscs), itsrs(rhs.itsrs),
    ...
    public BandMatrixView<T,CStyle>
  {
    ...
    public:
      ...
      BandMatrixView(const BandMatrixView<T,FortranStyle>& rhs) : 
	BandMatrixView<T,CStyle>(rhs) {}

      BandMatrixView(const BandMatrixView<T,CStyle>& rhs) : 
	BandMatrixView<T,CStyle>(rhs) {}

      BandMatrixView(const BandMatrixView<T>& rhs, int _lo, int _hi) :
	BandMatrixView<T,CStyle>(rhs,_lo,_hi) {}
	...
	{ BandMatrixView<T,CStyle>::operator=(m2); return *this; }
      ...
      inline const BandMatrixView<T,FortranStyle>& operator=(
	  const BandMatrixComposite<T>& mcomp) const
      { BandMatrixView<T,CStyle>::operator=(mcomp); return *this; }
...
#define NEW_SIZE(cs,rs,lo,hi) \
	...
	Copy(BandMatrixViewOf(rhs,lo,hi),View());
      ...
      BandMatrix(const BandMatrix<T,S,I>& rhs) :
	...
	itsm(S==DiagMajor ? itsm1-itsnlo*itssi : itsm1)
      ...
      {
	TMVAssert(S == RowMajor || S == ColMajor || S == DiagMajor);
	memmove(itsm1,rhs.itsm1,ls()*sizeof(T));
      }

      template <IndexStyle I2> BandMatrix(const BandMatrix<T,S,I2>& rhs) :
	GenBandMatrix<T>(S,NonConj,rhs.ls()), itsm1(new T[ls()]),
	itscs(rhs.itscs), itsrs(rhs.itsrs),
	itsnlo(rhs.itsnlo), itsnhi(rhs.itsnhi),
	itssi(rhs.itssi), itssj(rhs.itssj), itsds(rhs.itsds),
	itsm(S==DiagMajor ? itsm1-itsnlo*itssi : itsm1)
      ...
      {
	TMVAssert(S == RowMajor || S == ColMajor || S == DiagMajor);
	memmove(itsm1,rhs.start_mem(),ls()*sizeof(T));
      }
...
#undef NEW_SIZE
	...
	TMVAssert(nhi() >= m2.nhi());
	if (&m2 != this) {
	  if (nlo() > m2.nlo() || nhi() > m2.nhi()) Zero();
	  if (S == DiagMajor)
	    if (nlo() > m2.nlo())
	      memmove(itsm+m2.nlo()*stepi(),m2.itsm1,m2.ls()*sizeof(T));
	    else 
	      memmove(itsm1,m2.itsm1,m2.ls()*sizeof(T));
	...
	return ConstBandMatrixView<T,I>(cptr(),rowsize(),colsize(),nhi(),nlo(),
      ...
      {
	return ConstVectorView<T>(itsm1,ls(),1,NonConj);
      }
	...
	return BandMatrixView<T,I>(ptr(),rowsize(),colsize(),nhi(),nlo(),
      ...
      {
	return VectorView<T,I>(itsm1,ls(),1,NonConj FIRSTLAST );
      }
