...
namespace tmv {
  ...
  template <class T, class T1, class T2> class SumBsB : 
    public BandMatrixComposite<T> 
  {
	...
	TMVAssert(m1.rowsize() == m2.size()); 
      ...
      inline int nlo() const { return std::max(m1.nlo(),m2.nlo()); }
      inline int nhi() const { return std::max(m1.nhi(),m2.nhi()); }
...
#undef PRODXM2
  ...
  template <class T, class T1, class T2> class ProdBsB : 
    public BandMatrixComposite<T>
  {
    public:
	...
	x(_x), m1(_m1), m2(_m2)
      ...
      { return std::min(int(colsize()-1),m1.nlo()+m2.nlo()); }
      inline int nhi() const 
      { return std::min(int(rowsize()-1),m1.nhi()+m2.nhi()); }
      inline StorageType stor() const
      {
  ...
  template <class T, class T1, class T2> class ProdsBB : 
    public BandMatrixComposite<T>
  {
    public:
