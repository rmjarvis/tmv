...
namespace tmv {
  ...
  template <class T, class Ta, class Tb> void AddMM(
      ...
      const T beta, const GenMatrix<Tb>& B,
  ...
  template <class T> class MatrixComposite :
    public GenMatrix<T>
  {
    public:
  ...
  template <class T> inline void MultXM(const T alpha, const MatrixView<CT>& A)
  { MultXM(CT(alpha),A); }
  ...
  template <class T> inline void AddMM(const CT alpha,
      const GenMatrix<CT>& A, const CT beta, const GenMatrix<T>& B,
  ...
  template <bool add, class T, class Tx, class Ty> inline void Rank1Update(
      const T alpha, const GenVector<Tx>& x, const GenVector<Ty>& y, 
      const MatrixView<CT>& A)
  ...
  template <class T> inline void AddElementProd(
      const CT alpha, const GenMatrix<CT>& A, const GenMatrix<T>& B,
      const MatrixView<CT>& C)
  { AddElementProd(alpha,B,A,C); }

  // Specialize disallowed complex combinations:
  template <bool add, class T, class Ta, class Tb> inline void MultMV(
      const CT , const GenMatrix<Ta>& , const GenVector<Tb>& ,
      const VectorView<T>& )
  { TMVAssert(FALSE); }

  template <class T> inline void MultXM(const CT , const MatrixView<T>& )
  { TMVAssert(FALSE); }

  template <class T, class Ta> inline void AddMM(
      const CT , const GenMatrix<Ta>& , const MatrixView<T>& )
  { TMVAssert(FALSE); }
  template <class T, class Ta, class Tb> inline void AddMM(
  ...
  template <bool add, class T, class Ta, class Tb> inline void Rank1Update(
