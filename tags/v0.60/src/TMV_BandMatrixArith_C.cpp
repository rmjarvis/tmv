...
namespace tmv {
  ...
  {
#ifdef XDEBUG
    //cerr<<"Band AddMM: alpha = "<<alpha<<endl;
	...
	if (SameStorage(A,C)) {
	  if (SameStorage(B,C)) {
	    if (A.isrm()) {
	      ...
	      C = beta*B;
	      DoAddMM(alpha,tempA,BandMatrixViewOf(C,A.nlo(),A.nhi()));
	    } else if (A.iscm()) {
	  ...
	  C = beta*B;
	  DoAddMM(alpha,A,BandMatrixViewOf(C,A.nlo(),A.nhi()));
	}
      }
    }
#ifdef XDEBUG
