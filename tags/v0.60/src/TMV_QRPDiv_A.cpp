...
#define QRP_BLOCKSIZE 32
      ...
      const VectorView<T>& beta, size_t* P, T& det)
  {
    ...
    Matrix<T,RowMajor> ZYtA(std::min(size_t(QRP_BLOCKSIZE),M),N);
    ...
    for(size_t j1=0;j1<N;) {
      size_t j2 = std::min(N,j1+QRP_BLOCKSIZE);

      for(size_t j=j1,jmj1=0; j<j2; ++j,++jmj1, ++bj) {
	size_t jpiv;
	RealType(T) maxnormsq = colnormsq.SubVector(j,N).MaxElement(&jpiv);
      ...
      const MatrixView<T>& A, const VectorView<T>& beta, size_t* P, T& det)
  {
	...
	size_t j2 = std::min(j3,j1+QRP_BLOCKSIZE);
	//cerr<<"j2 = "<<j2<<endl;
	TMVAssert(j1 < j2);
    ...
    Matrix<T> AA = Q*UpperTriMatrixViewOf(A);
    AA.ReversePermuteCols(P);
    if (Norm(AA-A0) > 0.001*std::max(RealType(T)(1),Norm(A0))) {
      ...
      const VectorView<double>& beta, size_t* P, double& det)
  {
    ...
    for(size_t i=0;i<beta.size();++i,++bi) {
      if (det) {
	if (*bi != 0.) det = -det;
	if (P[i] != i) det = -det;
      }
  ...
  template <> inline void LapQRP_Decompose(
      ...
      const VectorView<std::complex<double> >& beta, size_t* P,
      std::complex<double>& det)
  {
    ...
    for(size_t i=0;i<A.rowsize();++i,Aii+=Ads) {
  ...
  template <> inline void LapQRP_Decompose(
      ...
      const VectorView<std::complex<float> >& beta, size_t* P,
