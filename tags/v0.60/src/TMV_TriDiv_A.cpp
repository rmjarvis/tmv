...
namespace tmv {
  ...
  template <bool rm, class T, class Ta> inline void RowTri_LDivEq(
      const GenUpperTriMatrix<Ta>& A, const VectorView<T>& b)
  {
    if (A.isconj())
      if (A.isunit())
  ...
  template <bool cm, class T, class Ta> inline void ColTri_LDivEq(
      const GenUpperTriMatrix<Ta>& A, const VectorView<T>& b)
  {
    if (A.isconj())
      if (A.isunit())
