...
#define QR_BLOCKSIZE 64
      ...
      const MatrixView<T>& m)
  {
    ...
    const size_t M = Q.colsize();
    const size_t N = Q.rowsize();
    UpperTriMatrix<T1,NonUnitDiag,ColMajor> BaseZ(std::min(size_t(QR_BLOCKSIZE),N));
  ...
  template <> inline void LapQ_RDivEq(
      const GenMatrix<std::complex<double> >& Q,
      const GenVector<std::complex<double> >& beta,
      const MatrixView<std::complex<double> >& x)
  {
      ...
      const GenMatrix<T2>& m, const MatrixView<T>& x, size_t N1)
  {
    ...
    UpperTriMatrixViewOf(QRx).SubTriMatrix(0,N1).LDivEq(x.Rows(0,N1));
      ...
      const MatrixView<T>& m, size_t N1)
  {
    ...
    UpperTriMatrixViewOf(QRx).SubTriMatrix(0,N1).LDivEq(m.Rows(0,N1));;
      ...
      const GenMatrix<T2>& m, const MatrixView<T>& x, size_t N1)
  {
    ...
    UpperTriMatrixViewOf(QRx).SubTriMatrix(0,N1).RDivEq(x.Cols(0,N1));;
      ...
      const MatrixView<T>& m, size_t N1)
  {
    ...
    UpperTriMatrixViewOf(QRx).SubTriMatrix(0,N1).RDivEq(m.Cols(0,N1));
    Q_RDivEq(QRx,beta,m);
  }
