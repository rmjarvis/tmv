...
namespace tmv {
  ...
  {
    //cerr<<"Start Composite cptr\n";
    if (!itsm.get()) {
      itsm.reset(new T[this->size()*this->size()]);
      UpperTriMatrixViewOf(itsm.get(),this->size(),this->dt(),
	  this->stor()) = *this;
    }
    //cerr<<"made itsm: "<<itsm.get()<<std::endl;
    return itsm.get();
  }
  ...
  template <class T> const T* LowerTriMatrixComposite<T>::cptr() const
  {
    if (!itsm.get()) {
      itsm.reset(new T[this->size()*this->size()]);
      LowerTriMatrixViewOf(itsm.get(),this->size(),this->dt(),
	  this->stor()) = *this;
    }
    return itsm.get();
  }
  ...
  template <bool cm, bool ca, bool ua, class T, class Ta> 
	...
	const GenLowerTriMatrix<Ta>& A, const VectorView<T>& x)
    {
      ...
      for(size_t jj=N-1,len=1;jj>0;--jj,++len,--xj,Ajj-=ds) if (*xj!=T(0)) {
	// j = N-2..0, jj = j+1
