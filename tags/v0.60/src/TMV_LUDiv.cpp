...
#undef APTR
    ...
    pimpl(new LUDiv_Impl(A,inplace)) 
  {
    ...
    LU_Decompose(pimpl->LUx,pimpl->P.get(),pimpl->det);
  }

  template <class T> LUDiv<T>::~LUDiv() { delete pimpl; }
...
#define CT std::complex<T>
  ...
  template <class T> inline void LU_RDivEq(
      const GenMatrix<CT>& , const size_t* , const MatrixView<T>& )
  { TMVAssert(FALSE); }
  template <class T> inline void LU_Inverse(
      const GenMatrix<CT>& , const size_t* , const MatrixView<T>& )
  { TMVAssert(FALSE); }
#undef CT
  ...
  template <class T> const size_t* LUDiv<T>::GetP() const 
  { return pimpl->P.get(); }

  template <class T> std::string LUDiv<T>::Type() const
  { return std::string("LUDiv<") + tmv::Type(T()) + ">"; }

  template <class T> DivType LUDiv<T>::GetDivType() const 
  { return LU; }
