...
#define CT TMV_ComplexType(T)
    ...
    static RT NonLapNormF(const GenSymBandMatrix<T>& m)
    {
        ...
        if (mmax == RT(0)) return RT(0);
            ...
            // TMV_Epsilon is a pure power of 2, so no rounding errors from rescaling.
            RT scale = inveps;
            mmax *= scale;
        ...
        const T* m, size_t s, int nlo, UpLoType uplo, StorageType stor)
    {
        ...
        if (stor == DiagMajor) 
            return ConstSymBandMatrixView<T>(
                m + (uplo==Upper ? 0 : nlo*(s-1)),
                s,nlo,-int(s)+1,int(s),1,Sym,uplo,DiagMajor,NonConj);
        else if (stor == RowMajor)
            return ConstSymBandMatrixView<T>(
                m,s,nlo,nlo,1,nlo+1,Sym,uplo,RowMajor,NonConj);
        else
            return ConstSymBandMatrixView<T>(
                m,s,nlo,1,nlo,nlo+1,Sym,uplo,ColMajor,NonConj);
    }
        ...
        T* m, size_t s, int nlo, UpLoType uplo, StorageType stor)
    {
        ...
        if (stor == DiagMajor) 
            return SymBandMatrixView<T>(
                m + (uplo==Upper ? 0 : nlo*(s-1)),
                s,nlo,-int(s)+1,int(s),1,Sym,uplo,DiagMajor,NonConj
                TMV_FIRSTLAST1(m,m+BandStorageLength(DiagMajor,s,s,nlo,nlo)));
        else if (stor == RowMajor)
            return SymBandMatrixView<T>(
                m,s,nlo,nlo,1,nlo+1,Sym,uplo,RowMajor,NonConj
                TMV_FIRSTLAST1(m,m+BandStorageLength(RowMajor,s,s,nlo,nlo)));
        else
            return SymBandMatrixView<T>(
                m,s,nlo,1,nlo,nlo+1,Sym,uplo,ColMajor,NonConj
                TMV_FIRSTLAST1(m,m+BandStorageLength(ColMajor,s,s,nlo,nlo)));
    }
        ...
        const T* m, size_t s, int nlo, UpLoType uplo, StorageType stor)
    {
        ...
        if (stor == DiagMajor) 
            ...
            return ConstSymBandMatrixView<T>(
                m,s,nlo,nlo,1,nlo+1,Herm,uplo,RowMajor,NonConj);
        else
            return ConstSymBandMatrixView<T>(
                m,s,nlo,1,nlo,nlo+1,Herm,uplo,ColMajor,NonConj);
    }
