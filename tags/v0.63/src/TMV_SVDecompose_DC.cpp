        ...
        const T* zsq, const T normsqz, T* diff, T* sum)
    {
        ...
        for(int iter = 0; iter < TMV_MAXITER; iter++) {
            ...
            if (iter == TMV_MAXITER-1) {
                std::cout<<"Warning - Unable to find appropriate initial "
                    "guess in FindDCSingularValue\n"<<
                    "Unsuccessful after "<<TMV_MAXITER<<" iterations.\n";
                exit(1);
            }
#endif
        }
...
#pragma omp critical
        ...
        MVP<T> U, const VectorView<RT>& D, const VectorView<RT>& E, MVP<T> V)
    {
        ...
        if (E.size() > 0)
            SV_DecomposeFromBidiagonal_QR(U,D,E,V);
        ...
        bool UisI, bool VisI)
    {
        ...
        // So I always use QR for the S only calculation.
            ...
            if (U) {
                ...
                SV_DecomposeFromBidiagonal_DC<RT>(
                    U1.view(),D1,
                    E1.subVector(0,K-1),V1.rowRange(0,K),true,false);
                if (UisI) U->subMatrix(0,K,0,K) = U1;
                else U->colRange(0,K) *= U1;
            } else {
                SV_DecomposeFromBidiagonal_DC<RT>(
                    0,D1,E1.subVector(0,K-1),
                    V1.rowRange(0,K),false,false);
            }
            ...
            if (N > 1) {
                ...
                if (V) V->rowRange(0,N).permuteRows(P.get());
                dbgcout<<"After sort\n";
                dbgcout<<"DN = "<<D.subVector(0,N)<<endl;
                dbgcout<<"zN = "<<z.subVector(0,N)<<endl;
#ifdef XDEBUG
                ...
                if (i_firstswap < N-1) {
                    ...
                    if (V) V->rowRange(i_firstswap,N).permuteRows(P.get());
                }
                ...
                if (U || V) {
                    ...
                    if (V) {
                        ...
                        for(int j=0;j<N;j++) {
                            ...
                            dbgcout<<"Norm(y.row("<<j<<")) => "<<
                                Norm(W.col(j))<<endl;
                        }
                        // V = Y * V
                        dbgcout<<"Before V = Y * V\n";
                        dbgcout<<"Norm(VVt-1) (Rows 0-N) = "<<
                            Norm(V->rowRange(0,N)*V->rowRange(0,N).adjoint()
                                 - T(1))<<endl;
        ...
        if (U && V) {
            ...
            if (Norm(A2-A0) > 0.001*normA0 || 
                ...
                Vector<RT> E_QR = E0;
                SmallProblem<T>(
                    U_QR.view(),D_QR.view(),E_QR.view(),V_QR.view());
