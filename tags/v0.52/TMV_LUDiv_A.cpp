...
#define RecursiveLU
      ...
      const MatrixView<double>& A, size_t* P, double& det)
  {
...
#ifdef CLAP
    clapack_dgetrf(CblasColMajor,A.colsize(),A.rowsize(),A.ptr(),
	A.stepj(),(int*)P);
#else
    int m = A.colsize();
    int n = A.rowsize();
    int lda = A.stepj();
    int info;
    dgetrf(&m,&n,A.ptr(),&lda,(int*)P,&info);
    if (info < 0) tmv_error("dgetrf returned info < 0");
    for(size_t i=0;i<A.colsize();i++) --P[i];
#endif
    for(size_t i=0;i<A.colsize();i++) if (P[i]!=i) det = -det;
  }
  template <> inline void LapLU_Decompose(
      ...
      complex<double>& det)
  {
    ...
    TMVAssert(A.ct()==NonConj);
#ifdef CLAP
    clapack_zgetrf(CblasColMajor,A.colsize(),A.rowsize(),A.ptr(),
	A.stepj(),(int*)P);
#else
    int m = A.colsize();
    int n = A.rowsize();
    int lda = A.stepj();
    int info;
    zgetrf(&m,&n,LAP_Complex(A.ptr()),&lda,(int*)P,&info);
      ...
      const MatrixView<float>& A, size_t* P, float& det)
  {
    ...
    TMVAssert(A.ct()==NonConj);
#ifdef CLAP
    clapack_sgetrf(CblasColMajor,A.colsize(),A.rowsize(),A.ptr(),
	A.stepj(),(int*)P);
#else
    int m = A.colsize();
    int n = A.rowsize();
    int lda = A.stepj();
    int info;
    sgetrf(&m,&n,A.ptr(),&lda,(int*)P,&info);
      ...
      complex<float>& det)
  {
    ...
    TMVAssert(A.ct()==NonConj);
#ifdef CLAP
    clapack_cgetrf(CblasColMajor,A.colsize(),A.rowsize(),A.ptr(),
	A.stepj(),(int*)P);
#else
    int m = A.colsize();
    int n = A.rowsize();
    int lda = A.stepj();
    int info;
    cgetrf(&m,&n,LAP_Complex(A.ptr()),&lda,(int*)P,&info);
