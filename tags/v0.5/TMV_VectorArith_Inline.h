...
namespace tmv {
      ...
      const T1 x, VIt<T2,S2,NonConj> vit, size_t size)
  {
    TMVAssert(x!=T1(0));
    //TMVAssert(x!=T1(1)); 
    // Should check for x==1 in calling routine, but for division,
    // sometimes 1/x rounds to 1 even when x != 1, so it's not worth
    // being a stickler about it, since x=1 isn't _wrong_ here.
    TMVAssert(size>0);
    for(size_t i=size;i>0;--i,++vit) *vit *= x;
  }
#ifdef BLAS
  ...
  template <class T1, class T2, StepItType S2, ConjItType C2, class T3, StepItType S3>
	...
	VIt<T3,S3,NonConj> v3it, size_t size)
    {
      ...
      TMVAssert(x1 != T1(0));
      if (x1 == T1(1))        
	for(size_t i=size;i>0;--i,++v2it,++v3it) (*v3it) += (*v2it);
      else if (x1 == T1(-1))
	for(size_t i=size;i>0;--i,++v2it,++v3it) (*v3it) -= (*v2it);
      else
	for(size_t i=size;i>0;--i,++v2it,++v3it) (*v3it) += x1 * (*v2it);
    }
  template <class T1, class T2, StepItType S2, ConjItType C2, class T3, StepItType S3>
  ...
  template <class T, StepItType S1, class T2, StepItType S2, ConjItType C2> 
	...
	CVIt<T2,S2,C2> v2it, size_t size)
    {
      TMVAssert(size>0);
      res=T(0);
      for(size_t i=size;i>0;--i,++v1it,++v2it) res += (*v1it) * (*v2it);
    }
  template <class T, StepItType S1, class T2, StepItType S2, ConjItType C2> 
  ...
  template <class T, StepItType S1, ConjItType C1, StepItType S2, ConjItType C2>
    ...
    { T x; DoMultVV(x,v1it,v2it,size); res = x; }
  ...
  template <class T1, class T2, StepItType S2, ConjItType C2, class T3, StepItType S3, ConjItType C3, class T4, StepItType S4>
    ...
    // zi += alpha * xi * yi 
    {
      ...
      TMVAssert(size>0);
      if (alpha == T1(1))
	for(size_t i=size;i>0;--i,++xit,++yit,++zit) *zit += (*xit) * (*yit);
      else if (alpha == T1(-1))
	for(size_t i=size;i>0;--i,++xit,++yit,++zit) *zit -= (*xit) * (*yit);
      else 
	for(size_t i=size;i>0;--i,++xit,++yit,++zit) 
	  *zit += alpha * (*xit) * (*yit);
    }

  template <class T1, class T2, StepItType S2, ConjItType C2, class T3, StepItType S3>
    inline void DoElementProd(const T1 alpha, CVIt<T2,S2,C2> xit,
	VIt<T3,S3,NonConj> yit, size_t size)
    // yi = xi * yi
    {
      TMVAssert(size>0);
      if (alpha == T1(1))
	for(size_t i=size;i>0;--i,++xit,++yit) *yit *= (*xit);
      else if (alpha == T1(-1))
	for(size_t i=size;i>0;--i,++xit,++yit) *yit *= -(*xit);
      else 
	for(size_t i=size;i>0;--i,++xit,++yit) *yit *= alpha * (*xit);
    }
