...
//
//        For each of these (Inverse, InverseATA), when calling as a method
...
//    Likewise:
//    QRD(), QRPD(), SVD(), SVFD() return the corresponding
//    Divider classes.
//    (SVFD() is appropriate for SVF, SVS, SVU, and SVV.)
//
...
namespace tmv {
    ...
    public BaseMatrix<T>
  {
	...
	return *dynamic_cast<const SVDiv<T>*>(GetDiv());
      ...
      inline const SVFDiv<T>& SVFD() const
      {
	TMVAssert(GetDiv());
	TMVAssert(dynamic_cast<const SVFDiv<T>*>(GetDiv()));
	return *dynamic_cast<const SVFDiv<T>*>(GetDiv());
      }
...
#define NEW_SIZE(cs,rs) GenMatrix<T>(S,NonConj), \
...
#undef NEW_SIZE
	...
	return ConstMatrixView<T>(cptr(),colsize(),rowsize(),
      ...
      { 
	return ConstMatrixView<T>(cptr(),rowsize(),colsize(),
	    stepj(),stepi(),TransOf(S),NonConj);
      }
  
      inline ConstMatrixView<T> Conjugate() const
      { 
	return ConstMatrixView<T>(cptr(),colsize(),rowsize(),
	    stepi(),stepj(),S,ConjOf(T,NonConj));
      }
	...
	return MatrixView<T>(ptr(),colsize(),rowsize(),
      ...
      { 
	return MatrixView<T>(ptr(),rowsize(),colsize(),
	    stepj(),stepi(),TransOf(S),NonConj FIRSTLAST);
      }
  
      inline MatrixView<T> Conjugate()
      { 
	return MatrixView<T>(ptr(),colsize(),rowsize(),
	    stepi(),stepj(),S,ConjOf(T,NonConj) FIRSTLAST);
      }
...
//---------------------------------------------------------------------------
      ...
      const GenVector<T>& v1, const GenVector<T>& v2)
  {
    ...
    Rank1Update(T(1),v1,v2,temp.View());
    return temp;
  }
      ...
      const GenMatrix<T1>& m1, const MatrixView<T2>& m2)
  {
    ...
    if (m1.iscm() && m2.iscm()) {
      ...
      T2* m2ptr = m2.ptr();
      if (m1.isconj())
	for(size_t k=m2.rowsize();k>0;--k,m1ptr+=m1.stepj(),m2ptr+=m2.stepj()) 
	  DoCopy(CVIt<T1,Unit,Conj>(m1ptr,1),
	      VIt<T2,Unit,NonConj>(m2ptr,1),size);
      else
	for(size_t k=m2.rowsize();k>0;--k,m1ptr+=m1.stepj(),m2ptr+=m2.stepj()) 
