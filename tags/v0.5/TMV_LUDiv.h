...
namespace tmv {
  ...
  extern bool RecursiveLU;

  template <class T> void LU_Decompose(const MatrixView<T>& A, 
      size_t* P, T& det);
...
#define CT complex<T>
  template <class T> inline void LU_LDivEq(
      const GenMatrix<CT>& LUx, const size_t* P, 
...
#undef CT
    ...
    virtual public Divider<T> 
  {
	...
	if (istrans) LU_RDivEq(LUx,P,m.Transpose()); 
	else LU_LDivEq(LUx,P,m); 
      } 

      template <class T1> inline void DoRDivEq(const MatrixView<T1>& m) const 
      {
	TMVAssert(LUx.colsize() == m.rowsize());
	if (istrans) LU_LDivEq(LUx,P,m.Transpose()); 
	else LU_RDivEq(LUx,P,m); 
      } 
