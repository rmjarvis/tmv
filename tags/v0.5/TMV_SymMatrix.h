...
//        (This is guaranteed, for example, if all the upper square 
...
#define UTransOf(U) (U==Upper ? Lower : Upper)
    ...
    public GenSymMatrix<T>
  {
	...
	return SymMatrixView<T>(ptr(),size(),stepi(),stepj(),
      ...
      {
	return SymMatrixView<T>(ptr(),size(),stepj(),stepi(),
	    sym(),UTransOf(uplo()),TransOf(stor()),ConjOf(T,ct()) FIRSTLAST);
      }
...
#define NEW_SIZE(s) GenSymMatrix<T>(Herm,U,S,NonConj), \
      itslen((s)*(s)), itsm(new T[itslen]), itss(s)  \
      DEFFIRSTLAST(itsm,itsm+itslen)
...
//---------------------------------------------------------------------------
  ...
  template <class T> bool operator==(
      const GenSymMatrix<T>& m1, const GenSymMatrix<T>& m2)
  ...
  template <class T, UpLoType U, StorageType S> inline std::string Type(
  ...
  template <class T> inline std::string Type(const ConstSymMatrixView<T>& m)
  {
    return std::string("ConstSymMatrixView<")+Type(T())+","+Text(m.sym())+","
      +Text(m.uplo())+","+Text(m.ct())+","+Text(m.stor())+">"; 
  }

  template <class T> inline std::string Type(const SymMatrixView<T>& m)
  {
