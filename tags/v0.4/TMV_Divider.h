...
// m.DivideUsing(ALG)
// where ALG is LU, QR, QRP, SV, SVF or CH  for the algorithms above.
//
...
#define CT ComplexType(T)
  ...
  // This is defined in TMV_Base.h
  //enum DivType { XXX, LU, QR, QRP, SV, SVF, SVS, CH };
  ...
  template <class T> class Divider 
  {
    ...
    public :
      ...
      // MJ: Move decomposition checks as CheckDecomp method
      // MJ: Also, make Inverse as void Inverse(m);
      // MJ: Will want x/M as a MatrixComposite for this.
...
#undef CT
  ...
  template <class T> class MetaDivider : virtual public Divider<T>
  {
	...
	trans(rhs.trans), conj(rhs.conj), itsm(rhs.itsm) { }
      ...
      {
	return (SV_Type(itsm->GetDivType()) ||
...
#include "TMV_AuxAllDiv.h"
  ...
  {
    switch (dt) {
      case tmv::XXX : return "XXX";
      case tmv::LU : return "LU";
      case tmv::QR : return "QR";
      case tmv::QRP : return "QRP";
      case tmv::SV : return "SV";
      case tmv::SVF : return "SVF";
      case tmv::SVS : return "SVS";
      case tmv::CH : return "CH";
      default : return "unknown DT";
    }
  }
