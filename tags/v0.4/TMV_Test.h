    ...
    const MT& m, tmv::DivType dt) 
{
  ...
  switch (dt) {
	...
	cout<<"P = "<<tmv::Matrix<double>(m.LUD().LU_GetP())<<endl;
	cout<<"Q = "<<tmv::Matrix<double>(m.LUD().LU_GetQ())<<endl;
#endif
	...
	cout<<"P = "<<tmv::Matrix<double>(m.QRD().QR_GetP())<<endl;
#endif
	if (m.QRD().QR_IsTrans()) {
