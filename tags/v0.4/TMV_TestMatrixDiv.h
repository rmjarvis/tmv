    ...
    const Matrix<T2,S2>& m2, string label)
{
  ...
  b.SetDiv();
  m2.SetDiv();
  //cerr <<"b.dt = "<<Text(b.GetDivType())<<" = "<<b.GetDiv()->Type()<<endl;
  //cerr <<"m.dt = "<<Text(m2.GetDivType())<<" = "<<m2.GetDiv()->Type()<<endl;
  ...
  double eps = EPS*Norm(m1)*Norm(m2)*Norm(m2inv);
  ...
  if (CanLDiv(a,b)) {
    ...
    //cerr<<"m1/b = "<<m1/b<<endl;
    //cerr<<"a/b = "<<a/b<<endl;
    //cerr<<"Norm(diff) = "<<Norm(a/b-m1/m2)<<" <?  "<<eps<<endl;
    Assert(Norm((a/b)-(m1/m2)) <= eps,label+" a/b");
    //cerr<<"Norm(diff) = "<<Norm(a/m2-m1/m2)<<" <?  "<<eps<<endl;
    Assert(Norm((a/m2)-(m1/m2)) <= eps,label+" a/m");
    //cerr<<"Norm(diff) = "<<Norm(m1/b-m1/m2)<<" <?  "<<eps<<endl;
...
#endif
  m2.DivideUsing(dt);
  DoTestMatrixDivArithMM1b(a,b,m1,m2,label+"(Meta)");
  b.ReSetDiv();
    ...
    const Matrix<T2,S2>& m2, string label)
{
  ...
  if (CanRDivEq(m1,b)) {
    ...
    //cerr<<"m4 = "<<m4<<endl;
    //cerr<<"m3*b = "<<m3*b<<endl;
    //cerr<<"m4*m2 = "<<m4*m2<<endl;
    //cerr<<"Norm(m3-m4) = "<<Norm(m3-m4)<<endl;
    //cerr<<"EPS*Norm(m3)*Norm(m2) = "<<EPS*Norm(m3)*Norm(m2)<<endl;
    Assert(Norm(m3-m4) <= eps,label+" m%=b");
  }
#ifdef SHOWSTARTDONE
  cout<<"Done MM2b"<<endl;
...
#endif
  m2.DivideUsing(dt);
  DoTestMatrixDivArithMM2b(a,b,m1,m2,label+"(Meta)");
  b.ReSetDiv();
    ...
    const Vector<T>& v, const SM& b, const Matrix<T2,S2>& m, string label)
{
  ...
  double eps = EPS*Norm(v)*Norm(m)*Norm(minv);
...
#endif
  m.DivideUsing(dt);
  DoTestMatrixDivArithMV1b(v,b,m,label+"(Meta)");
  b.ReSetDiv();
...
#endif
  m.DivideUsing(dt);
  DoTestMatrixDivArithMV2b(v,b,m,label+"(Meta)");
  b.ReSetDiv();
    ...
    const SM1& a, const Matrix<T,S>& m, T2 x, string label)
{
  ...
  double normm = Norm(m);
  double eps = EPS*Norm(m)*Norm(minv);
  ...
  //cerr<<"x/a = "<<x/a<<endl;
  //cerr<<"x/m = "<<x/m<<endl;
  //cerr<<"a*(x/a) = "<<a*(x/a)<<endl;
  //cerr<<"m*(x/m) = "<<m*(x/m)<<endl;
  //cerr<<"Norm(diff) = "<<Norm((x/a)-(x/m))<<endl;
  //cerr<<"eps*x = "<<eps*abs(x)<<endl;
  Assert(Norm((x/a)-(x/m)) <= eps*abs(x),label+" x/a");
  //cerr<<"x%a = "<<x%a<<endl;
  //cerr<<"x%m = "<<x%m<<endl;
  //cerr<<"x%a*a = "<<(x%a)*a<<endl;
  //cerr<<"x%m*m = "<<(x%m)*m<<endl;
  //cerr<<"Norm(diff) = "<<Norm((x%a)-(x%m))<<endl;
  Assert(Norm((x%a)-(x%m)) <= eps*abs(x),label+" x%a");
  //cerr<<"a = "<<Type(a)<<"  "<<a<<endl;
  //cerr<<"1/a = "<<T2(1)/a<<endl;
  //cerr<<"a*(1/a) = "<<a*(T2(1)/a)<<endl;
  //cerr<<"(a*(1/a))*a = "<<(a*(T2(1)/a))*a<<endl;
  //cerr<<"(a*(1/a))*a-a = "<<(a*(T2(1)/a))*a-a<<endl;
  //cerr<<Norm(a*((T2(1)/a)*a)-a)<<"  "<<eps*normm<<endl;
  //cerr<<Norm((a*(T2(1)/a))*a-a)<<"  "<<eps*normm<<endl;
  //cerr<<Norm(a*((T2(1)%a)*a)-a)<<"  "<<eps*normm<<endl;
  //cerr<<Norm((a*(T2(1)%a))*a-a)<<"  "<<eps*normm<<endl;
  //cerr<<Norm((T2(1)/a)*a-Transpose((T2(1)/a)*a))<<"  "<<eps<<endl;
  //cerr<<Norm((T2(1)%a)*a-Transpose((T2(1)%a)*a))<<"  "<<eps<<endl;
  //cerr<<Norm(a*(T2(1)/a)-Transpose(a*(T2(1)/a)))<<"  "<<eps<<endl;
  //cerr<<Norm(a*(T2(1)%a)-Transpose(a*(T2(1)%a)))<<"  "<<eps<<endl;
...
#endif
  m.DivideUsing(dt);
  DoTestMatrixDivArithMXb(a,m,x,label+"(Meta)");
  a.ReSetDiv();
...
template <class T, class SM1, class SM2, class CSM1, class CSM2> void TestMatrixDivArith(
    DivType dt, SM1& a, SM2& b, CSM1& ca, CSM2& cb, string label)
{
#ifdef SHOWSTARTDONE
