...
template <class T, class T2, class T3> class PRODMM : 
  public MatrixComposite<T>
{
  public:
  ...
  public MatrixComposite<T>
{
  public:
	...
	if (m0.isrm()) {
	  Matrix<T,RowMajor> temp(m0.colsize(),m0.rowsize(),m0.stor());
	  m2->RDiv(*m1,temp.QuickView());
	  m0 = temp;
	} else {
	  Matrix<T,ColMajor> temp(m0.colsize(),m0.rowsize(),m0.stor());
	  m2->RDiv(*m1,temp.QuickView());
	  m0 = temp;
	}
      } else {
