...
namespace tmv {
  ...
  template <class T> 
  inline void QR_Update(const UpperTriMatrixView<T>& R, const VectorView<T>& z)
  ...
  // Remove a single row
  template <class T> 
  inline void QR_Downdate(const UpperTriMatrixView<T>& R,
...
#include "tmv/TMV_AuxAllDiv.h"
  ...
  private :
    ...
    inline QRDiv(const QRDiv<T>&) : pimpl(0)
    { TMVAssert(FALSE); }
    inline QRDiv<T>& operator=(const QRDiv<T>&)
