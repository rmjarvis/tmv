...
namespace tmv {
  ...
  void AddVV(const T x1, const GenVector<T1>& v1,
  ...
  void ElementProd(const T alpha, const GenVector<Tx>& x,
      const VectorView<T>& y);
  template <class T, class Tx, class Ty> 
  void AddElementProd(const T alpha, const GenVector<Tx>& x,
  ...
  { MultXV(CT(x),v2); }
  template <class T, class T1> 
  inline void MultXV(const T x,
      const GenVector<T1>& v1, const VectorView<CT>& v2)
  { MultXV(CT(x),v1,v2); }

  template <class T, class T1> 
  inline void AddVV(const T x, 
      const GenVector<T1>& v1, const VectorView<CT>& v2)
  { AddVV(CT(x),v1,v2); }
  template <class T, class T1, class T2> 
  inline void AddVV(const T x1, const GenVector<T1>& v1,
      const T x2, const GenVector<T2>& v2, const VectorView<CT>& v3)
  { AddVV(CT(x1),v1,CT(x2),v2,v3); }
  template <class T> 
  inline void AddVV(const CT x1, const GenVector<CT>& v1,
  ...
  template <class T, class Tx> 
  inline void ElementProd(const T alpha, const GenVector<Tx>& x,
      const VectorView<CT>& y)
  { ElementProd(CT(alpha),x,y); }
  template <class T, class Tx, class Ty> 
  inline void AddElementProd(const T alpha, const GenVector<Tx>& x,
      const GenVector<Ty>& y, const VectorView<CT>& z)
  { AddElementProd(CT(alpha),x,y,z); }
  template <class T> 
  inline void AddElementProd(const CT alpha, const GenVector<CT>& x,
  ...
  inline void MultXV(const CT ,
      const GenVector<Ta>& , const VectorView<T>& )
  { TMVAssert(FALSE); }

  template <class T, class T1> 
  inline void AddVV(const CT , 
      const GenVector<T1>& , const VectorView<T>& )
  { TMVAssert(FALSE); }
  template <class T, class Ta, class Tb> 
  inline void AddVV(const CT , const GenVector<Ta>& ,
