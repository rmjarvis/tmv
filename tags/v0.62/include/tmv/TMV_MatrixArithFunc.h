...
namespace tmv {
  ...
  void MultMV(const T alpha, const GenMatrix<Ta>& A, const GenVector<Tx>& x,
  ...
  void AddMM(const T alpha, const GenMatrix<Ta>& A,
      const T beta, const GenMatrix<Tb>& B, const MatrixView<T>& C);

  // C (+)= alpha * A * B
  template <bool add, class T, class Ta, class Tb> 
  void MultMM(const T alpha, const GenMatrix<Ta>& A, const GenMatrix<Tb>& B,
      const MatrixView<T>& C);

  // A (+)= alpha * x * yT
  template <bool add, class T, class Tx, class Ty> 
  void Rank1Update(const T alpha, const GenVector<Tx>& x, const GenVector<Ty>& y, 
      const MatrixView<T>& A);

  template <class T, class Ta> 
  void ElementProd(const T alpha, const GenMatrix<Ta>& A,
      const MatrixView<T>& B);

  template <class T, class Ta, class Tb> 
  void AddElementProd(const T alpha, const GenMatrix<Ta>& A,
  ...
  template <bool add, class T, class Ta, class Tx> 
  inline void MultMV(const T alpha, const GenMatrix<Ta>& A,
  ...
  template <class T> 
  inline void AddMM(const CT alpha, const GenMatrix<CT>& A,
  ...
  template <class T, class Ta> 
  inline void ElementProd(const T alpha, const GenMatrix<Ta>& A,
      const MatrixView<CT>& B)
  { ElementProd(CT(alpha),A,B); }

  template <class T, class Ta, class Tb> 
  inline void AddElementProd(const T alpha, const GenMatrix<Ta>& A,
      const GenMatrix<Tb>& B, const MatrixView<CT>& C)
  { AddElementProd(CT(alpha),A,B,C); }

  template <class T> 
  inline void AddElementProd(const CT alpha, const GenMatrix<CT>& A,
      const GenMatrix<T>& B, const MatrixView<CT>& C)
  { AddElementProd(alpha,B,A,C); }

  // Specialize disallowed complex combinations:
  template <bool add, class T, class Ta, class Tb> 
  inline void MultMV(const CT , const GenMatrix<Ta>& ,
  ...
  template <bool add, class T, class Ta, class Tb> 
  inline void MultMM(const CT , const GenMatrix<Ta>& , const GenMatrix<Tb>& ,
      const MatrixView<T>& )
  { TMVAssert(FALSE); }

  template <bool add, class T, class Ta, class Tb> 
  inline void Rank1Update(const CT , const GenVector<Ta>& ,
