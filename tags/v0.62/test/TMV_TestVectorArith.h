    ...
    IFTEMP1(V0& temp) IFTEMP1(CV0& ctemp) const V1& a, const V2& b, std::string label)
{
  ...
  Assert(Norm(VEC(ComplexType(T),IFTEMP(ctemp=)a-z*b)-(v1-z*v2)) <= x*eps,label+" a-z*b");
  Assert(Norm(VEC(ComplexType(T),IFTEMP(ctemp=)a+z*b)-(v1+z*v2)) <= x*eps,label+" a+z*b");
  Assert(Norm(VEC(ComplexType(T),IFTEMP(ctemp=)z*a-b)-(z*v1-v2)) <= x*eps,label+" z*a-b");
  Assert(Norm(VEC(ComplexType(T),IFTEMP(ctemp=)z*a+b)-(z*v1+v2)) <= x*eps,label+" z*a+b");
  Assert(Norm(VEC(ComplexType(T),IFTEMP(ctemp=)z*a-z*b)-(z*v1-z*v2)) <= x*eps,label+" z*a-z*b");
  Assert(Norm(VEC(ComplexType(T),IFTEMP(ctemp=)z*a+z*b)-(z*v1+z*v2)) <= x*eps,label+" z*a+z*b");

  Assert(Norm(VEC(ComplexType(T),IFTEMP(ctemp=)z*a-x*b)-(z*v1-x*v2)) <= x*eps,label+" z*a-x*b");
  Assert(Norm(VEC(ComplexType(T),IFTEMP(ctemp=)z*a+x*b)-(z*v1+x*v2)) <= x*eps,label+" z*a+x*b");
  Assert(Norm(VEC(ComplexType(T),IFTEMP(ctemp=)x*a-z*b)-(x*v1-z*v2)) <= x*eps,label+" x*a-z*b");
  Assert(Norm(VEC(ComplexType(T),IFTEMP(ctemp=)x*a+z*b)-(x*v1+z*v2)) <= x*eps,label+" x*a+z*b");
