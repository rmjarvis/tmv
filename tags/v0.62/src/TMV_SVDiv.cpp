...
#define RT RealType(T)
  ...
  template <class T, class Tm, class Tx> void SV_LDiv(
      ...
      const GenMatrix<Tm>& m, const MatrixView<Tx>& x)
  {
    ...
    TMVAssert(kmax <= int(V.rowsize())); // = K
    TMVAssert(kmax <= int(U.colsize()));
#ifdef XDEBUG
    Matrix<T> A = U * SS * V;
    Matrix<Tm> m0(m);
#endif

    Matrix<Tx,RowMajor> m2 = U.Adjoint().Rows(0,kmax) * m; // KxR
    ...
    Matrix<Tx> mm = A*x;
    if (U.IsSquare() && kmax==int(SS.size()) && Norm(m0-mm) > 0.001 * Norm(A)*Norm(m0)) {
      cerr<<"SV_LDiv\n";
      cerr<<"U = "<<U<<endl;
      cerr<<"S = "<<SS.diag()<<endl;
      cerr<<"V = "<<V<<endl;
  ...
  template <class T, class Tm, class Tx> void SV_RDiv(
      const GenMatrix<T>& U, const GenDiagMatrix<RT>& SS, 
      const GenMatrix<T>& V, int kmax,
      const GenMatrix<Tm>& m, const MatrixView<Tx>& x) 
  {
    ...
    TMVAssert(kmax <= int(U.colsize())); // = K
    TMVAssert(kmax <= int(V.rowsize()));
#ifdef XDEBUG
    Matrix<T> A = U * SS * V;
    Matrix<Tm> m0(m);
#endif

    Matrix<Tx,RowMajor> m2 = m * V.Adjoint().Cols(0,kmax); // = RxK
    m2 %= SS.SubDiagMatrix(0,kmax);
