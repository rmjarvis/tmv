...
namespace tmv {
  ...
  template <class T> 
  void LU_Decompose(const BandMatrixView<T>& LUx, int* p,
      T& signdet, int Anhi);
  // Decompose A (input as LUx) into L * U

  template <class T, class T1> 
  void TriLDivEq(const GenBandMatrix<T1>& A, const MatrixView<T>& B,
      DiagType dt);
  template <class T, class T1> 
  void TriLDivEq(const GenBandMatrix<T1>& A, const VectorView<T>& v,
      DiagType dt);
  // Solve A X = B  where A is upper or lower band triangular

  template <class T, class T1> 
  void LU_PackedPL_Unpack(const GenBandMatrix<T1>& LUx, const int* p,
      const LowerTriMatrixView<T>& m);

  template <class T, class T1> 
  void LU_PackedPL_LDivEq(const GenBandMatrix<T1>& LUx, const int* p,
      const MatrixView<T>& m);
  template <class T, class T1> 
  void LU_PackedPL_RDivEq(const GenBandMatrix<T1>& LUx, const int* p,
      const MatrixView<T>& m);

  template <class T, class T1> 
  void LU_LDivEq(const GenBandMatrix<T1>& LUx, const int* p,
      const MatrixView<T>& m);
  template <class T, class T1> 
  void LU_RDivEq(const GenBandMatrix<T1>& LUx, const int* p,
      const MatrixView<T>& m);

  template <class T, class T1> 
  void LU_Inverse(const GenBandMatrix<T1>& LUx, const int* p,
      const MatrixView<T>& m);

  template <class T> 
  void Tri_Inverse(const UpperTriMatrixView<T>& U, int nhi);

#ifndef NOTHROW
  template <class T> 
  class SingularBandLU :
    public Singular
  {
  public:
