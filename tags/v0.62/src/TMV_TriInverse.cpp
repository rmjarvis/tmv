...
#define TRI_DIV_BLOCKSIZE 64
  ...
  template <class T> static inline void NonLapInverse(const UpperTriMatrixView<T>& U)
  {
#ifndef NOTHROW
    try {
#endif
...
#ifdef ALAP
  template <class T> static inline void LapInverse(const UpperTriMatrixView<T>& m)
  { NonLapInverse(m); }
#ifdef INST_DOUBLE
