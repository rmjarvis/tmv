...
namespace tmv {
  ...
  template <class T> 
  inline T MAXABS(std::complex<T> x)
  ...
  template <class T> 
  Givens<T> Givens_Rotate(T& x, T& y);
  ...
  void GivensMult(RealType(Tg) c, Tg s,
      T& x, T& y);
  template <class Tg, class T> 
  void GivensMult(RealType(Tg) c, Tg s,
      T& x, ConjRef<T> y);
  ...
  //     [ e0 d1  ]    [ -s* c ] [ e0 d1  ] [ s*  c ]
  template <class Tg, class T> 
  void GivensHermMult(RealType(Tg) c, Tg s,
      T& d0, T& d1, T& e0);

  // Do: [ d0 e0 ] <- [  c  s ] [ d0 e0 ] [ c -s* ]
  //     [ e0 d1 ]    [ -s* c ] [ e0 d1 ] [ s  c  ]
  template <class Tg, class T> 
  void GivensSymMult(RealType(Tg) c, Tg s,
      T& d0, T& d1, T& e0);
  ...
  public:
    ...
    // Constructors
    inline Givens(const RealType(T)& _c, const T& _s) : 
      c(_c), s(_s) {}
