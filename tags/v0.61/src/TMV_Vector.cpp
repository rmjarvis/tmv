...
#define RT RealType(T)
  ...
  {
    static const RT save = SQRT(Epsilon<T>());
    return save;
  }
  ...
  template <class T, IndexStyle I> 
    ...
    {
      if (step() == 1) memset(ptr(),0,size()*sizeof(T));
      else SetAllTo(0);
      return *this;
    }
  ...
  template <class T, IndexStyle I> 
	...
	*ptr() = x;
	...
	if (x == T(0)) memset(ptr(),0,size()*sizeof(T));
	else {
	  const int N = size();
	  int N1 = 64/sizeof(T);
	  if (N1 < 4) N1 = 4;
	  ...
	  if (N > N1) {
	    ...
	    while (2*N1 <= N) {
#ifdef TMVFLDEBUG
	      TMVAssert(p+N1 >= first);
	      TMVAssert(p+2*N1 <= last);
#endif
	      memmove(p+N1,p,N1*sizeof(T));
	      N1 *= 2;
	    }
	    if (N1 < N) {
#ifdef TMVFLDEBUG
  ...
  {
    const int N = size();
    if (x == T(0)) memset(ptr(),0,N*sizeof(T));
    else {
      int N1 = 64/sizeof(T);
      if (N1 < 4) N1 = 4;
      if (N1 > N) N1 = N;
      ...
      int i1, int i2)
  {
    ...
    TMVAssert(i2 < int(size()));
    if (i1 != i2) 
      if (I==CStyle) SWAP(*(ptr()+i1),*(ptr()+i2));
      else SWAP(*(ptr()+i1-1),*(ptr()+i2-1));
      return *this;
  }
  ...
  template <class T> void GenVector<T>::Write(std::ostream& os) const
  {
    ...
    if (this->isconj()) 
      if (s == 1) 
	for(int i=size();i>0;--i,++p) os << " " << CONJ(*p) << " ";
      else 
	for(int i=size();i>0;--i,p+=s) os << " " << CONJ(*p) << " ";
    else 
      if (step() == 1) 
	for(int i=size();i>0;--i,++p) os << " " << *p << " ";
      else
	for(int i=size();i>0;--i,p+=s) os << " " << *p << " ";
    os << ")";
  }
