...
namespace tmv {
  ...
  template <class T> struct BandQRDiv<T>::BandQRDiv_Impl
  {
    ...
    private :
      BandQRDiv_Impl(const BandQRDiv_Impl&) :
	istrans(false), inplace(false), Aptr1(0), Aptr(0),
	QRx(BandMatrixViewOf(Aptr,0,0,0,0,ColMajor)),
	Qbeta(0), logdet(0), signdet(0), donedet(false)
      { TMVAssert(FALSE); }
      BandQRDiv_Impl& operator=(const BandQRDiv_Impl&)
...
#undef QRX
  ...
  template <class T> BandQRDiv<T>::~BandQRDiv() { delete pimpl; pimpl=0; }
      ...
      const MatrixView<T>& minv) const
  {
    ...
    UpperTriMatrixView<T> Rinv = UpperTriMatrixViewOf(minv);
  ...
  template <class T> Matrix<T> BandQRDiv<T>::GetQ() const 
  {
    ...
    temp = pimpl->QRx;
    GetQFromBandQR(temp.View(),pimpl->Qbeta.View(),pimpl->QRx.nlo());
    return temp;
  }
