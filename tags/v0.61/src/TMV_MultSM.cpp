...
#define SYM_MM_BLOCKSIZE2 32
      ...
      const int beta, const MatrixView<std::complex<double> >& C)
  {
    ...
    if (IMAG(alpha) == double(0)) {
      SymMatrix<double,Lower,ColMajor> A1 = A.Real();
	...
	LowerTriMatrixView<double> L = A1.LowerTri();
	L = A.LowerTri().Imag();
	// A.Imag() = L - LT
	if (A.LowerTri().isconj() != C.isconj()) {
      ...
      if (C.isconj()) {
	if (beta == 0) C.Conjugate().Imag() = C1;
	else C.Conjugate().Imag() += C1;
      } else {
	if (beta == 0) C.Imag() = C1;
	else C.Imag() += C1;
      }
    } else {
      SymMatrix<double,Lower,ColMajor> Ar = A.Real();
      SymMatrix<double,Lower,ColMajor> Ai(A.size());
      LowerTriMatrixView<double> L = Ai.LowerTri();
      Matrix<double,ColMajor> C1 = REAL(alpha)*Ar*B;
      if (A.issym()) {
	Ai = A.Imag();
	if (C.isconj()) C1 += IMAG(alpha)*Ai*B;
	...
	if (A.LowerTri().isconj() != C.isconj()) {
      ...
      if (A.issym()) {
	if (C.isconj()) C1 = -REAL(alpha)*Ai*B;
  ...
  template <> void BlasMultMM(
      const std::complex<double> alpha,
      const GenSymMatrix<double>& A, const GenMatrix<double>& B,
      const int beta, const MatrixView<std::complex<double> >& C)
  {
    TMVAssert(A.size() == C.colsize());
    TMVAssert(A.size() == B.colsize());
      ...
      const int beta, const MatrixView<std::complex<float> >& C)
  {
    ...
    if (IMAG(alpha) == float(0)) {
      SymMatrix<float,Lower,ColMajor> A1 = A.Real();
	...
	LowerTriMatrixView<float> L = A1.LowerTri();
	L = A.LowerTri().Imag();
	// A.Imag() = L - LT
	if (A.LowerTri().isconj() != C.isconj()) {
	  C1 = -REAL(alpha)*L*B;
	...
	if (beta == 0) C.Imag() = C1;
      ...
      SymMatrix<float,Lower,ColMajor> Ar = A.Real();
      SymMatrix<float,Lower,ColMajor> Ai(A.size());
      LowerTriMatrixView<float> L = Ai.LowerTri();
  ...
  template <> void BlasMultMM(
      const std::complex<float> alpha,
      const GenSymMatrix<float>& A, const GenMatrix<float>& B,
      const int beta, const MatrixView<std::complex<float> >& C)
  {
    TMVAssert(A.size() == C.colsize());
    TMVAssert(A.size() == B.colsize());
