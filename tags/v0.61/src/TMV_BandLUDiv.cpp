...
namespace tmv {
  ...
  template <class T, class T1> void LU_PackedPL_Unpack(
  ...
  template <class T, class T1> void LU_PackedPL_LDivEq(
      ...
      const int* p, const MatrixView<T>& m) 
  {
    ...
    PL0 = T(1);
    LU_PackedPL_Unpack(LUx,p,LowerTriMatrixViewOf(PL0,UnitDiag));
...
#ifdef XDEBUG
    RealType(T) kappa = Norm(PL0)*Norm(PL0.Inverse());
    RealType(T) normdiff = Norm(m2-m);
...
#ifdef XDEBUG
    RealType(T) kappa = Norm(PLU)*Norm(PLU.Inverse());
    RealType(T) normdiff = Norm(m2-m);
    if (normdiff > 0.0001*kappa*Norm(m2)) {
      cerr<<"LU_PackedPL_LDivEq\n";
      cerr<<"LUx = "<<LUx<<endl;
  ...
  template <class T, class T1> void LU_PackedPL_RDivEq(
      const GenBandMatrix<T1>& LUx,
      const int* p, const MatrixView<T>& m) 
  {
#ifdef XDEBUG
  ...
  template <class T, class T1> static inline void LapTriDiagLU_LDivEq(
      const GenBandMatrix<T1>& LUx, const int* P, const MatrixView<T>& m)
  { NonLapLU_LDivEq(LUx,P,m); }
  template <class T, class T1> static inline void LapLU_RDivEq(
      const GenBandMatrix<T1>& LUx, const int* P, const MatrixView<T>& m)
  { NonLapLU_RDivEq(LUx,P,m); }
  template <class T, class T1> static inline void LapTriDiagLU_RDivEq(
      const GenBandMatrix<T1>& LUx, const int* P, const MatrixView<T>& m)
  { NonLapLU_RDivEq(LUx,P,m); }
#ifdef INST_DOUBLE
  ...
  template <> void LapTriDiagLU_LDivEq(
      const GenBandMatrix<double>& LUx,
      const int* P, const MatrixView<double>& m) 
  {
    TMVAssert(m.colsize() == LUx.colsize());
  ...
  template <> void LapTriDiagLU_RDivEq(
      const GenBandMatrix<double>& LUx,
      const int* P, const MatrixView<double>& m) 
  {
    TMVAssert(m.rowsize() == LUx.colsize());
