...
namespace tmv {
  ...
  template <class T> struct SymLDLDiv<T>::SymLDLDiv_Impl
  {
    ...
    private :
      SymLDLDiv_Impl(const SymLDLDiv_Impl&) :
	inplace(false), Aptr1(0), Aptr(0), 
	LLx(SymMatrixViewOf(Aptr,0,Lower,ColMajor)), 
	xD(0), P(0), logdet(0), signdet(1)
      { TMVAssert(FALSE); }
      SymLDLDiv_Impl& operator=(const SymLDLDiv_Impl&)
...
#undef LLX
  ...
  { return LowerTriMatrixViewOf(pimpl->LLx.LowerTri(),UnitDiag); }
