...
#undef Sj
  ...
  if (S == tmv::RowMajor) {
    const T qar[] = { T(0), T(-1), T(-2),
                      T(2), T(1), T(0) };
    tmv::SmallMatrix<T,2,3,S> q(qar);
    for(int i=0;i<2;i++) for(int j=0;j<3;j++) {
      Assert(q(i,j) == T(2*i-j),"Create SmallMatrix from T*");
    }
    std::vector<T> qv(6);
    for(int i=0;i<6;i++) qv[i] = qar[i];
    tmv::SmallMatrix<T,2,3,S> q2(qv);
    for(int i=0;i<2;i++) for(int j=0;j<3;j++) {
      Assert(q(i,j) == T(2*i-j),"Create SmallMatrix from vector");
    }
  } else {
    const T qar[] = { T(0), T(2),
                      T(-1), T(1),
  ...
  //system("rm tmvtest_smallmatrix_io.dat");
#endif
}
