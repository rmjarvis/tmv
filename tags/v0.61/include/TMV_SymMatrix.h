...
#undef NEW_SIZE
	...
	i = std::abs(i);
	const int ds = stepi()+stepj();
	return ConstVectorView<T,I>(cptr()+i*(U==Upper?stepj():stepi())+j1*ds,
	    j2-j1,ds,NonConj);
      }
	...
	i = std::abs(i);
	const int ds = stepi()+stepj();
	return VectorView<T,I>(ptr()+i*(U==Upper?stepj():stepi())+j1*ds,
      ...
      {
	memset(itsm.get(),0,size()*size()*sizeof(T));
	return *this;
      }
	...
	return ConstSymMatrixView<T,I>(cptr()+i1*(stepi()+stepj()),i2-i1,
	...
	return ConstSymMatrixView<T,I>(cptr()+i1*(stepi()+stepj()),
...
#undef NEW_SIZE
	...
	TMVAssert(View().OKSubSymMatrix(i1,i2,1));
	if (I==FortranStyle) { --i1; }
	return SymMatrixView<T,I>(ptr()+i1*(stepi()+stepj()),
