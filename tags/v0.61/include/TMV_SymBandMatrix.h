...
namespace tmv {
    ...
    private DivHelper<T>
  {
	...
	TMVAssert(i>=-nlo() && i<=nlo());
	TMVAssert(i>=-int(size()) && i<=int(size())); 
	TMVAssert(j1>=0 && j1<=j2 && j2<=int(size())-std::abs(i));
	if (i>=0)
	  if (uplo()==Upper)
	...
	m2.diag() = diag();
	if (!issym()) {
	  // Need this complication because v.Imag is invalid if v isconj.
	  if (m2.diag().isconj()) m2.diag().Conjugate().Imag().Zero();
    ...
    public ConstSymBandMatrixView<T,CStyle>
  {
	...
	return GenSymBandMatrix<T>::operator()(i-1,j-1);
      ...
      { 
	TMVAssert(i>0 && i<=int(this->size()));
	TMVAssert(j1>0 && j1<=j2 && j2<=int(this->size()));
	...
	TMVAssert(j1>0 && j1<=j2 && j2<=int(this->size())-std::abs(i));
...
#undef NEW_SIZE
	...
	{ 
	  ...
	  if ((U==Upper && i<=j1) || (U==Lower && j2<=i+1))
	    ...
	    return ConstVectorView<T,I>(cptr()+i*stepj()+j1*stepi(),
		j2-j1,stepi(),NonConj); 
	}
	...
	inline ConstVectorView<T,I> col(int j, int i1, int i2) const
	{
	  ...
	  if ((U==Upper && i2<=j+1) || (U==Lower && j<=i1))
	    ...
	    return ConstVectorView<T,I>(cptr()+i1*stepj()+j*stepi(),
		i2-i1,stepj(),NonConj); 
	}
	...
	inline ConstVectorView<T,I> diag(int i, int j1, int j2) const
	{
	  ...
	  TMVAssert(i>=-int(size()) && i<=int(size())); 
	  if (I==FortranStyle) { 
	    TMVAssert(j1>0 && j1<=j2 && j2<=int(size())-std::abs(i)); --j1;
	  } else {
	    TMVAssert(j1>=0 && j1<=j2 && j2<=int(size())-std::abs(i));
	  }
	  i = std::abs(i);
	  return ConstVectorView<T,I>(
	      cptr()+i*(U==Upper?stepj():stepi())+j1*diagstep(),
	...
	{ 
	  ...
	  if ((U==Upper && i<=j1) || (U==Lower && j2<=i+1))
	    ...
	    return VectorView<T,I>(ptr()+i*stepj()+j1*stepi(),
	...
	inline VectorView<T,I> col(int j, int i1, int i2)
	{
	  ...
	  if ((U==Upper && i2<=j+1) || (U==Lower && j<=i1))
	    ...
	    return VectorView<T,I>(ptr()+i1*stepj()+j*stepi(),
	...
	inline VectorView<T,I> diag(int i, int j1, int j2) 
	{
	  ...
	  return VectorView<T,I>(
	      ptr()+i*(U==Upper?stepj():stepi())+j1*diagstep(),
	...
	{
	  memset(itsm1.get(),0,linsize*sizeof(T));
	  return *this;
	}
	    ...
	    int i1, int i2, int j1, int j2) const
	{
	  ...
	  if ((U==Upper && i2-1<=j1) || (U==Lower && j2-1<=i1))
	    ...
	    return ConstMatrixView<T,I>(cptr()+i1*stepj()+j1*stepi(),
	...
	inline ConstSymMatrixView<T,I> SubSymMatrix(int i1, int i2) const
	{
	  ...
	  return ConstSymMatrixView<T,I>(cptr()+i1*diagstep(),i2-i1,
	    ...
	    int istep) const
	{
	  ...
	  return ConstSymMatrixView<T,I>(cptr()+i1*diagstep(),
	    ...
	    int i1, int i2, int newnlo) const
	{
	  ...
	  if (I==FortranStyle) { --i1; }
	  return ConstSymBandMatrixView<T,I>(cptr()+i1*diagstep(),i2-i1,
	    ...
	    int i1, int i2, int newnlo, int istep) const
	{
	  ...
	  return ConstSymBandMatrixView<T,I>(
	      cptr()+i1*diagstep(), (i2-i1)/istep, newnlo,
	      istep*stepi(),istep*stepj(),istep*diagstep(),
	...
	{
	  TMVAssert(newnlo>=0 && newnlo <= nlo());
	  return ConstSymBandMatrixView<T,I>(cptr(),size(),newnlo,
	...
	inline MatrixView<T,I> SubMatrix(int i1, int i2, int j1, int j2)
	{
	  ...
	  if ((U==Upper && i2-1<=j1) || (U==Lower && j2-1<=i1))
	    ...
	    return MatrixView<T,I>(ptr()+i1*stepj()+j1*stepi(),
	...
	inline SymMatrixView<T,I> SubSymMatrix(int i1, int i2)
	{
	  TMVAssert(View().OKSubSymMatrix(i1,i2,1));
	  if (I==FortranStyle) { --i1; }
	  return SymMatrixView<T,I>(ptr()+i1*diagstep(),i2-i1,
	...
	inline SymMatrixView<T,I> SubSymMatrix(int i1, int i2, int istep) 
	{
	  TMVAssert(View().OKSubSymMatrix(i1,i2,istep));
	  if (I==FortranStyle) { --i1; i2+=istep-1; }
	  return SymMatrixView<T,I>(ptr()+i1*diagstep(),(i2-i1)/istep,
	    ...
	    int i1, int i2, int newnlo, int istep)
	{
	  TMVAssert(View().OKSubSymBandMatrix(i1,i2,newnlo,istep));
	  if (I==FortranStyle) { --i1; i2+=istep-1; }
	  return SymBandMatrixView<T,I>(ptr()+i1*diagstep(),(i2-i1)/istep,
