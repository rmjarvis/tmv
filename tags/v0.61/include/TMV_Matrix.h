...
namespace tmv {
    ...
    public GenMatrix<T>
  {
	...
	return *this;
      ...
      const MatrixView<T,I>& PermuteRows(const int* p, 
	  int i1, int i2) const;
...
#define NEW_SIZE(cs,rs) \
	...
	for(int i=ls();i>0;--i,++vi,++vvi) *vi = *vvi;
      }
...
#undef NEW_SIZE
	...
	return ConstVectorView<T,I>(cptr()+j*stepj(),
	...
	if (i >= 0) {
      ...
      {
	if (I == FortranStyle) { TMVAssert(j1 > 0 && j1<=j2); --j1; }
	else { TMVAssert( j1>=0 && j1<=j2); }
	TMVAssert(i>=-int(colsize()) && i<=int(rowsize()));
	...
	return ConstVectorView<T,I>(cptr()+i*stepi()+j*stepj(),s,
	...
	return ConstMatrixView<T,I>(cptr()+j1*stepj(),colsize(),2,
	...
	return ConstMatrixView<T,I>(cptr()+i1*stepi(),2,rowsize(),
	...
	return ConstMatrixView<T,I>(cptr()+j1*stepj(),colsize(),j2-j1,
	...
	return ConstMatrixView<T,I>(cptr()+i1*stepi(),i2-i1,rowsize(),
	...
	TMVAssert(i>=0 && i<int(colsize()));
	TMVAssert(j>=0 && j<int(rowsize()));
	T*const mi = ptr() + i*stepi() + j*stepj();
#ifdef TMVFLDEBUG
	TMVAssert(mi >= first);
	TMVAssert(mi < last);
#endif
...
//---------------------------------------------------------------------------
  ...
  template <class T, class T1> inline void DoCopyDiffType(
  ...
  template <class T1, class T2> inline void CopyNonConj(
      const GenMatrix<T1>& m1, const MatrixView<T2>& m2)
  {
