...
namespace tmv {
      ...
      const T3 beta, const MatrixView<T4>& B)
  {
    ...
    for(size_t i=0;i<A.colsize();++i,Aptr+=A.stepi(),Bptr+=B.stepi()) {
      if (beta != T3(1)) 
	DoMultXV(beta,VIt<T4,Unit,NonConj>(Bptr,1),B.rowsize());
      if (A.isconj())
	DoAddVV(alpha,CVIt<T2,Unit,Conj>(Aptr,1),
	    VIt<T4,Unit,NonConj>(Bptr,1),B.rowsize());
      else
      ...
      const T beta, const MatrixView<T>& B)
  {
    ...
    if (beta != T(1)) B *= beta;
    for(size_t i=0;i<A.colsize();++i) B.row(i) += alpha * A.row(i);
  }
      ...
      const T3 beta, const MatrixView<T4>& B)
  {
    ...
    for(size_t j=0;j<A.rowsize();++j,Aptr+=A.stepj(),Bptr+=B.stepj()) {
      if (beta != T3(1)) 
	DoMultXV(beta,VIt<T4,Unit,NonConj>(Bptr,1),B.colsize());
      if (A.isconj())
	DoAddVV(alpha,CVIt<T2,Unit,Conj>(Aptr,1),
	    VIt<T4,Unit,NonConj>(Bptr,1),B.colsize());
      else
	DoAddVV(alpha,CVIt<T2,Unit,NonConj>(Aptr,1),
	    VIt<T4,Unit,NonConj>(Bptr,1),B.colsize());
    }
      ...
      const T beta, const MatrixView<T>& B)
  {
    ...
    if (beta != T(1)) B *= beta;
    for(size_t j=0;j<A.rowsize();++j) B.col(j) += alpha * A.col(j);
  }
  ...
  { 
    ...
    if (B.isconj()) 
      DoAddMM(CONJ(alpha),A.Conjugate(),CONJ(beta),B.Conjugate());
