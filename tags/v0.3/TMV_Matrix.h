...
//          (SV is default for non-square matrices)
...
//        SVD for its divisions (default for non-square m) and
//        SVD has been set (through SetDiv() or implicitly through
//        matrix division or Inverse().
//
//    m.NewTranspose()
//    m.NewConjugate()
//    m.NewAdjoint()
...
//        Transpose, Conjugate, Adjoint, Inverse, or itself respectively.
//
// Operators:
...
//    m.DivideUsing(dt)
//    where dt is tmv::LU, tmv::SV, tmv::QR, or tmv::QRP
//
...
namespace tmv {
    ...
    public BaseMatrix<T>
  {
	...
	BaseMatrix<T>(rhs), itsstor(rhs.itsstor), itsct(rhs.itsct) {}
      ...
      // MJ: Try these as virtual to speed up when GenMatrix is 
      // reall a full Matrix
	...
	return ConstMatrixView<T>(cptr(),rowsize(),colsize(),
      ...
      {
	TMVAssert(stepi()==stepj()*rowsize() || stepj()==stepi()*colsize());
	...
	if (isrm()) return new Matrix<T,RowMajor>(*this); 
      ...
      {
	TMVAssert(dt == tmv::LU || dt == tmv::SV || dt == tmv::QR ||
	    dt == tmv::QRP);
