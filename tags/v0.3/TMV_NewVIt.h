...
#define SETFIRSTLAST(f,l)
  ...
  template <class T> ConjItType ConjOf(ConjItType C)
  { 
    if (IsReal(T())) return NonConj;
    else return C == NonConj ? Conj : NonConj;
  }
  ...
  { 
	...
	return *p; 
      ...
      { return VIter<T>(p+n*s,s FIRSTLAST ); }
      inline VIter<T> operator-(size_t n) const 
      { return VIter<T>(p-n*s,s FIRSTLAST ); }
  ...
  { 
    public :
      ...
      CVIter() : p(0), s(0) {}
      CVIter(const T* inp, int instep ) : p(inp), s(instep) {}
      CVIter(const CVIter<T>& rhs) : p(rhs.p), s(rhs.s) {}
      CVIter(const VIter<T>& rhs) : p(rhs.GetP()), s(rhs.s) {}
	...
	return *this; 
      ...
      { return CVIter<T>(p+n*s,s FIRSTLAST ); }
      inline CVIter<T> operator-(size_t n) const 
      { return CVIter<T>(p-n*s,s FIRSTLAST ); }
  ...
  { 
	...
	p(rhs.p), s(1) DEFFIRSTLAST(rhs.first,rhs.last) {}
      VIt(const VIter<T>& rhs) : 
	p(rhs.p), s(rhs.step()) DEFFIRSTLAST(rhs.first,rhs.last) {}
  ...
  { 
    public :
      ...
      CVIt(const CVIt<T,Unit,C>& rhs) : p(rhs.p), s(1) {}
  ...
  template <class T> class ConjRef
  {
    public:
      ...
      { 
	if (&val == &x2) {
	  tmv_error("Trying to swap element with its conjugate in place.");
	}
  ...
  template <class T> inline RealType(T) imag(ConjRef<T> x) 
  { return x.imag(); }
  template <class T> inline void swap(ConjRef<complex<T> > x1,
      ConjRef<complex<T> > x2)
...
#ifdef TMVFLDEBUG
    public :
      const complex<T>*const first;
      const complex<T>*const last;
#endif
  };
  ...
  { 
    public :
      ...
      CVIt(const VIt<complex<T>,Unit,Conj>& rhs) : p(rhs.p) {}
  ...
  { 
	...
	return ConjRef<complex<T> >(*p);
      ...
      inline VIt<complex<T>,Step,Conj>& operator++() 
      { p += s; return *this; }
      inline void operator++(int) { p += s; }
      inline VIt<complex<T>,Step,Conj>& operator--() 
      { p -= s; return *this; }
  ...
  { 
	...
	p = rhs.GetP(); return *this; 
      ...
      inline CVIt<complex<T>,Step,Conj>& operator++() 
      { p += s; return *this; }
      inline void operator++(int) { p += s; }
      inline CVIt<complex<T>,Step,Conj>& operator--() 
      { p -= s; return *this; }
  ...
  template <class T> inline RealType(T) imag(VarConjRef<T> x) 
  { return x.imag(); }
  template <class T> inline void swap(VarConjRef<complex<T> > x1,
      VarConjRef<complex<T> > x2)
  { return x1.SwapWith(x2); }
  template <class T> inline void swap(complex<T>& x1,
      VarConjRef<complex<T> > x2)
  { return x2.SwapWith(x1); }
  ...
  { 
    public :
      ...
      VIter(complex<T>* inp, int instep, ConjItType inc PARAMFIRSTLAST(T) ) : 
	...
	return VarConjRef<complex<T> >(*p,c);
      ...
      { return VIter<complex<T> >(s==1?p+n:p+n*s,s FIRSTLAST ); }
      inline VIter<complex<T> > operator-(size_t n) const 
      { return VIter<complex<T> >(s==1?p-n:p-n*s,s FIRSTLAST ); }
  ...
  { 
	...
	return *this; 
      ...
      { return CVIter<complex<T> >(s==1?p+n:p+n*s,s FIRSTLAST ); }
      inline CVIter<complex<T> > operator-(size_t n) const 
      { return CVIter<complex<T> >(s==1?p-n:p-n*s,s FIRSTLAST ); }
  ...
  inline std::string Text(StepItType s)
  { return s == Unit ? "Unit" : "Step"; }

  inline std::string Text(ConjItType c)
