...
namespace tmv {
  ...
  template <class T> void NonLapGetQFromQR(
...
#ifdef LAP
  template <class T> void LapGetQFromQR(
      const MatrixView<T>& Q, const GenVector<T>& Qbeta)
  ...
  template <class T> void NonLapQR_Decompose(
...
#ifdef LAP
  template <class T> void LapQR_Decompose(
      const MatrixView<T>& QRx,
      const VectorView<T>& Qbeta, T& det)
  { NonLapQR_Decompose(QRx,Qbeta,det); }
  template <> void LapQR_Decompose(const MatrixView<double>& QRx,
      const VectorView<double>& Qbeta, double& det)
  {
    ...
    for(size_t i=0;i<Qbeta.size();++i) if (Qbeta(i) != 0.) det = -det;
  }
...
#ifndef NOFLOAT
  template <> void LapQR_Decompose(const MatrixView<float>& QRx,
...
#ifdef XLAP
  template <class T> void LapQRP_Decompose(
      const MatrixView<T>& QRx,
      const VectorView<T>& Qbeta, Permutation& P, T& det)
  { NonLapQR_Decompose(QRx,Qbeta,det); }
  template <> void LapQRP_Decompose(const MatrixView<double>& QRx,
...
#ifndef NOFLOAT
  template <> void LapQRP_Decompose(const MatrixView<float>& QRx,
...
#endif
  template <class T> void NewLapQRP_Decompose(
      const MatrixView<T>& QRx,
      const VectorView<T>& Qbeta, Permutation& P, T& det)
  { NonLapQR_Decompose(QRx,Qbeta,det); }
  template <> void NewLapQRP_Decompose(const MatrixView<double>& QRx,
      const VectorView<double>& Qbeta, Permutation& P, double& det)
  {
    TMVAssert(QRx.colsize() >= QRx.rowsize());
    TMVAssert(QRx.rowsize() == Qbeta.size());
...
#ifndef NOFLOAT
  template <> void NewLapQRP_Decompose(const MatrixView<float>& QRx,
      const VectorView<float>& Qbeta, Permutation& P, float& det)
  {
    TMVAssert(QRx.colsize() >= QRx.rowsize());
    TMVAssert(QRx.rowsize() == Qbeta.size());
  ...
  template <class T1, class T2> void NonLapQ_LDivEq(
...
#ifdef LAP
  template <class T1, class T2> void LapQ_LDivEq(
      const GenMatrix<T1>& Q, const GenVector<T1>& Qbeta,
      const MatrixView<T2>& m)
  ...
  template <class T1, class T2> void NonLapQ_RDivEq(
      const GenMatrix<T1>& Q, const GenVector<T1>& Qbeta,
      const MatrixView<T2>& m)
  {
    TMVAssert(Q.colsize() >= Q.rowsize());
...
#ifdef LAP
  template <class T1, class T2> void LapQ_RDivEq(
      const GenMatrix<T1>& Q, const GenVector<T1>& Qbeta,
      const MatrixView<T2>& m)
  ...
  void LapQ_RDivEq(const GenMatrix<complex<double> >& Q,
      const GenVector<complex<double> >& Qbeta,
      const MatrixView<complex<double> >& x)
  {
    TMVAssert(Q.colsize() >= Q.rowsize());
  ...
  void LapQ_RDivEq(const GenMatrix<complex<float> >& Q,
      const GenVector<complex<float> >& Qbeta,
      const MatrixView<complex<float> >& x)
  {
    TMVAssert(Q.colsize() >= Q.rowsize());
