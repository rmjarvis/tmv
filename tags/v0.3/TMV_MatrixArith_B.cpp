	...
	if (A.isrm()) {
	  ...
	  if (IMAG(alpha) == RealType(T)(0)) 
	    for(size_t i=0;i<A.colsize();++i,Aptr+=A.stepi()) 
	      DoMultXV(REAL(alpha),VIt<T,Unit,NonConj>(Aptr,1),A.rowsize());
	  else 
	    for(size_t i=0;i<A.colsize();++i,Aptr+=A.stepi()) 
	      DoMultXV(alpha,VIt<T,Unit,NonConj>(Aptr,1),A.rowsize());
	}
	else if (A.iscm()) {
	  T* Aptr = A.ptr();
	  if (IMAG(alpha) == RealType(T)(0)) 
	    for(size_t j=0;j<A.rowsize();++j,Aptr+=A.stepj()) 
	      DoMultXV(REAL(alpha),VIt<T,Unit,NonConj>(Aptr,1),A.colsize());
	  else 
	    for(size_t j=0;j<A.rowsize();++j,Aptr+=A.stepj()) 
	      DoMultXV(alpha,VIt<T,Unit,NonConj>(Aptr,1),A.colsize());
	}
	else if (A.colsize() < A.rowsize()) {
	  for(size_t i=0;i<A.colsize();++i) A.row(i) *= alpha;
	}
	else {
	  for(size_t i=0;i<A.colsize();++i) A.col(i) *= alpha;
	}
      }
    }
  }
