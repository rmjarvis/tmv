...
//    This form of division is used for LU and QR specifications.
//    SV does a full singular value decomposition.
...
namespace tmv {
    ...
    public BaseMatrix<T>
  {
	...
	if (itsdiag == UnitDiag) m2.diag().SetAllTo(T2(1));
...
#define NEW_SIZE(s) GenLowerTriMatrix<T>(D,S,NonConj), \
	...
	if (D==D2) Copy(rhs.QuickTranspose(),QuickTranspose());
	  ...
	  Copy(rhs.OffDiag().QuickTranspose(),OffDiag().QuickTranspose());
	  if (D==NonUnitDiag && D2==UnitDiag) diag().SetAllTo(T(1));
	}
      }
...
#undef NEW_SIZE
	...
	TMVAssert(!(m2.dt()==NonUnitDiag && D==UnitDiag));
	if (m2.dt()==UnitDiag && D==NonUnitDiag) {
	  Copy(m2.OffDiag().QuickTranspose(),OffDiag().QuickTranspose());
	  diag().SetAllTo(T(1));
	} else 
...
#define NEW_SIZE(s) GenLowerTriMatrix<T>(D,ColMajor,NonConj), \
	...
	if (D==D2) Copy(rhs.QuickTranspose(),QuickTranspose());
	  ...
	  Copy(rhs.QuickTranspose().OffDiag(),QuickTranspose().OffDiag());
	  if (D==NonUnitDiag && D2==UnitDiag) diag().SetAllTo(T(1));
	}
      }
