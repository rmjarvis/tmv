...
namespace tmv {
  ...
  // A = alpha * A
  template <class T> void MultXM(const T alpha,
      const UpperTriMatrixView<T>& A);
  template <class T> inline void MultXM(const T alpha,
  ...
  { AddMM(alpha,A,UpperTriMatrixViewOf(B)); }
  template <class T, class Ta> inline void AddMM(
      const T alpha, const GenLowerTriMatrix<Ta>& A,
      const MatrixView<T>& B)
  { AddMM(alpha,A.Transpose(),LowerTriMatrixViewOf(B).Transpose()); }
  // C = alpha * A + beta * B
