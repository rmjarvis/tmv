...
namespace tmv {
    ...
    static void Scale(const T& x, BaseMatrix_Mutable<M>& m)
    ...
    public:
        ...
        size_t colsize() const { return m.colsize(); }
        size_t rowsize() const { return m.rowsize(); }
...
#define CCT ConjRef<CT>
    ...
    // m *= x
    template <class M>
    static void MultEq(BaseMatrix_Mutable<M>& m, const int x)
    { Scale(RT(x),m.mat()); }

    template <class M>
    static void MultEq(BaseMatrix_Mutable<M>& m, const RT x)
    { Scale(x,m.mat()); }

    template <class M>
    static void MultEq(BaseMatrix_Mutable<M>& m, const CT x)
    { Scale(x,m.mat()); }

    template <class M>
    static void MultEq(BaseMatrix_Mutable<M>& m, const CCT x)
    { Scale(CT(x),m.mat()); }

    template <class M, class T>
    static void MultEq(BaseMatrix_Mutable<M>& m, const Scaling<0,T>& x)
    { Scale(x,m.mat()); }

    template <class M, class T>
    static void MultEq(BaseMatrix_Mutable<M>& m, const Scaling<1,T>& x)
    {}
    ...
    // m /= x
    template <class M>
    static void DivEq(BaseMatrix_Mutable<M>& m, const int x)
    { Scale(RT(1)/RT(x),m.mat()); }

    template <class M>
    static void DivEq(BaseMatrix_Mutable<M>& m, const RT x)
    { Scale(RT(1)/x,m.mat()); }

    template <class M>
    static void DivEq(BaseMatrix_Mutable<M>& m, const CT x)
    { Scale(RT(1)/x,m.mat()); }

    template <class M>
    static void DivEq(BaseMatrix_Mutable<M>& m, const CCT x)
    { Scale(RT(1)/CT(x),m.mat()); }

    template <class M, class T>
    static void DivEq(BaseMatrix_Mutable<M>& m, const Scaling<0,T>& x)
    { Scale(RT(1)/T(x),m.mat()); }

    template <class M, class T>
    static void DivEq(BaseMatrix_Mutable<M>& m, const Scaling<1,T>& x)
    {}
    ...
    // -m
    template <class M>
    static ProdXM<-1,RT,M> operator-(const BaseMatrix<M>& m)
    { return ProdXM<-1,RT,M>(RT(-1),m); }

    // x * m
    template <class M>
    static ProdXM<0,RT,M> operator*(const int x, const BaseMatrix<M>& m)
    { return ProdXM<0,RT,M>(RT(x),m); }

    template <class M>
    static ProdXM<0,RT,M> operator*(const RT x, const BaseMatrix<M>& m)
    { return ProdXM<0,RT,M>(x,m); }

    template <class M>
    static ProdXM<0,CT,M> operator*(const CT x, const BaseMatrix<M>& m)
    { return ProdXM<0,CT,M>(x,m); }

    template <class M>
    static ProdXM<0,CT,M> operator*(const CCT x, const BaseMatrix<M>& m)
    { return CT(x)*m; }

    template <class M, int ix, class T>
    static ProdXM<ix,T,M> operator*(const Scaling<ix,T>& x, 
                                    const BaseMatrix<M>& m)
    { return ProdXM<ix,T,M>(T(x),m); }

    // m * x
    template <class M>
    static ProdXM<0,RT,M> operator*(const BaseMatrix<M>& m, const int x)
    { return RT(x)*m; }

    template <class M>
    static ProdXM<0,RT,M> operator*(const BaseMatrix<M>& m, const RT x)
    { return x*m; }

    template <class M>
    static ProdXM<0,CT,M> operator*(const BaseMatrix<M>& m, const CT x)
    { return x*m; }

    template <class M>
    static ProdXM<0,CT,M> operator*(const BaseMatrix<M>& m, const CCT x)
    { return CT(x)*m; }

    template <class M, int ix, class T>
    static ProdXM<ix,T,M> operator*(const BaseMatrix<M>& m,
                                    const Scaling<ix,T>& x)
    { return ProdXM<ix,T,M>(T(x),m); }
...
#define CCT ConjRef<CT>
    ...
    // -(x*m)
    template <int ix, class T, class M>
    static ProdXM<-ix,T,M> operator-(const ProdXM<ix,T,M>& pxm)
    { return ProdXM<-ix,T,M>(-pxm.getX(),pxm.getM()); }

    // x*(x*m)
    template <int ix, class T, class M>
    static ProdXM<0,T,M> operator*(const int x, const ProdXM<ix,T,M>& pxm)
    { return ProdXM<0,T,M>(RT(x)*pxm.getX(),pxm.getM()); }

    template <int ix, class T, class M>
    static ProdXM<0,T,M> operator*(const RT x, const ProdXM<ix,T,M>& pxm)
    { return ProdXM<0,T,M>(x*pxm.getX(),pxm.getM()); }

    template <int ix, class T, class M>
    static ProdXM<0,CT,M> operator*(const CT x, const ProdXM<ix,T,M>& pxm)
    { return ProdXM<0,CT,M>(x*pxm.getX(),pxm.getM()); }

    template <int ix, class T, class M>
    static ProdXM<0,CT,M> operator*(const CCT x, const ProdXM<ix,T,M>& pxm)
    { return ProdXM<0,CT,M>(x*pxm.getX(),pxm.getM()); }

    template <int ix1, class T1, int ix, class T, class M>
    static ProdXM<ix*ix1,typename Traits2<T1,T>::type,M> operator*(
    ...
    // (x*m)*x
    template <int ix, class T, class M>
    static ProdXM<0,T,M> operator*(const ProdXM<ix,T,M>& pxm, const int x)
    { return ProdXM<0,T,M>(RT(x)*pxm.getX(),pxm.getM()); }

    template <int ix, class T, class M>
    static ProdXM<0,T,M> operator*(const ProdXM<ix,T,M>& pxm, const RT x)
    { return ProdXM<0,T,M>(x*pxm.getX(),pxm.getM()); }

    template <int ix, class T, class M>
    static ProdXM<0,CT,M> operator*(const ProdXM<ix,T,M>& pxm, const CT x)
    { return ProdXM<0,CT,M>(x*pxm.getX(),pxm.getM()); }

    template <int ix, class T, class M>
    static ProdXM<0,CT,M> operator*(const ProdXM<ix,T,M>& pxm, const CCT x)
    { return ProdXM<0,CT,M>(x*pxm.getX(),pxm.getM()); }
    ...
    // (x*m)/x
    template <int ix, class T, class M>
    static ProdXM<0,RT,M> operator/(const ProdXM<ix,T,M>& pxm, const int x)
    { return ProdXM<0,RT,M>(pxm.getX()/RT(x),pxm.getM()); }

    template <int ix, class T, class M>
    static ProdXM<0,RT,M> operator/(const ProdXM<ix,T,M>& pxm, const RT x)
...
#endif
    template <int ix, class T, class M>
    static std::string TMV_Text(const ProdXM<ix,T,M>& pxm)
    {
        std::ostringstream s;
