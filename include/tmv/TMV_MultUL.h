...
namespace tmv {
    ...
    struct MultUL_Helper<17,s,add,ix,T,M1,M2,M3> 
    {
        ...
        struct Helper2<0,dummy> // s == UNKNOWN
        {
                ...
                const Scaling<ix,T>& x, const M1& m1, const M2& m2, M3& m3)
            {
                ...
                const int xx = UNKNOWN;
                if (N < TMV_Q2) {
                    MultUL_Helper<algo2,xx,add,ix,T,M1,M2,M3>::call(
                        x,m1,m2,m3);
                } else {
                    // [ C00 C01 ] = [ A00 A01 ] [ B00  0  ]
                    // [ C10 C11 ]   [  0  A11 ] [ B10 B11 ]
                    ...
                    // C00 (+)= x A00 B00
                    MultUL_Helper<17,xx,add,ix,T,M1st,M2st,M3sm>::call(
                        x,A00,B00,C00);

                    // C00 += A01 B10
                    MultMM_Helper<-2,xx,xx,xx,true,ix,T,M1sm,M2sm,M3sm>::call(
                        x,A01,B10,C00);
                    ...
                    // C10 (+)= A11 B10
                    MultUM_Helper<-2,xx,xx,add,ix,T,M1st,M2sm,M3sm>::call(
                        x,A11,B10,C10);

                    // C01 (+)= A01 B11
                    MultUM_Helper<-2,xx,xx,add,ix,T,M2stt,M1smt,M3smt>::
        ...
        struct Helper2<1,dummy> // s < TMV_Q2
        {
                ...
                const Scaling<ix,T>& x, const M1& m1, const M2& m2, M3& m3)
            {
                ...
                // nops = 1/6 n(n+1)(2n+1)
                const int nops = 
                    IntTraits2<IntTraits2<s,sp1>::prod,twosp1>::prod / 6;
                const bool unroll = 
                    s > 20 ? false :
                    nops > TMV_Q1 ? false :
                    s <= 10;
        ...
        struct Helper2<2,dummy> // s >= TMV_Q2
        {
                ...
                const Scaling<ix,T>& x, const M1& m1, const M2& m2, M3& m3)
            {
                ...
                // C00 (+)= x A00 B00
                MultUL_Helper<17,Nx,add,ix,T,M1st,M2st,M3sm>::call(
                    x,A00,B00,C00);

                // C00 += A01 B10
                MultMM_Helper<-2,Nx,Nx,Ny,true,ix,T,M1sm,M2sm,M3sm>::call(
                    x,A01,B10,C00);

                // C10 (+)= A11 B10
                MultUM_Helper<-2,Ny,Nx,add,ix,T,M1st,M2sm,M3sm>::call(
                    x,A11,B10,C10);

                // C01 (+)= A01 B11
                MultUM_Helper<-2,Ny,Nx,add,ix,T,M2stt,M1smt,M3smt>::
                    call(x,B11t,A01t,C01t);

                // C11 (+)= x A11 B11
                MultUL_Helper<17,Ny,add,ix,T,M1st,M2st,M3sm>::call(
                    x,A11,B11,C11);
            }
        };
        static void call(
    ...
    struct MultUL_Helper<27,s,add,ix,T,M1,M2,M3> 
    {
        ...
        struct Helper2<0,dummy> // s == UNKNOWN
        {
                ...
                const Scaling<ix,T>& x, const M1& m1, const M2& m2, M3& m3)
            {
                ...
                if (N < TMV_Q2) {
                    ...
                    // C11 (+)= x A11 B11
                    MultUL_Helper<27,xx,add,ix,T,M1st,M2st,M3sm>::call(
                        x,A11,B11,C11);
        ...
        struct Helper2<2,dummy> // s >= TMV_Q2
        {
                ...
                const Scaling<ix,T>& x, const M1& m1, const M2& m2, M3& m3)
            {
                ...
                // C11 (+)= x A11 B11
                MultUL_Helper<27,Ny,add,ix,T,M1st,M2st,M3sm>::call(
                    x,A11,B11,C11);

                // C11 += A10 B01
                MultMM_Helper<-2,Ny,Ny,Nx,true,ix,T,M1sm,M2sm,M3sm>::call(
                    x,A10,B01,C11);

                // C01 (+)= A00 B01
                MultUM_Helper<-2,Nx,Ny,add,ix,T,M1st,M2sm,M3sm>::call(
                    x,A00,B01,C01);

                // C10 (+)= A10 B00
                MultUM_Helper<-2,Nx,Ny,add,ix,T,M2stt,M1smt,M3smt>::
                    call(x,B00t,A10t,C10t);

                // C00 (+)= x A00 B00
                MultUL_Helper<27,Nx,add,ix,T,M1st,M2st,M3sm>::call(
                    x,A00,B00,C00);
            }
        };
        static void call(
    ...
    struct MultUL_Helper<-3,s,add,ix,T,M1,M2,M3> 
    {
            ...
            const Scaling<ix,T>& x, const M1& m1, const M2& m2, M3& m3)
        {
            ...
            // nops = 1/6 n(n+1)(2n+1)
            const int nops = 
                IntTraits2<IntTraits2<s2,s2p1>::prod,twos2p1>::prod / 6;
            const bool unroll = 
                s == UNKNOWN ? false :
                nops > TMV_Q1 ? false :
                s <= 10;
    ...
    struct MultUL_Helper<94,s,add,ix,T,M1,M2,M3>
    {
        ...
        {
            Maybe<!add>::zero(m3);
            typedef typename M3::uppertri_type M3u;
            M3u m3u = m3.upperTri();
            MultUU_Helper<-2,s,add,ix,T,M1,M2,M3u>::call(x,m1,m2,m3u);
        }
    };
    ...
    struct MultUL_Helper<95,s,add,ix,T,M1,M2,M3>
    {
        ...
        {
            Maybe<!add>::zero(m3);
            typedef typename M3::lowertri_type M3l;
            M3l m3l = m3.lowerTri();
            MultUU_Helper<-2,s,add,ix,T,M1,M2,M3l>::call(x,m1,m2,m3l);
        }
    };
    ...
    // algo 940: U*U, so convert to MultUU call (-1, not -2)
    template <int s, bool add, 
              int ix, class T, class M1, class M2, class M3>
    struct MultUL_Helper<940,s,add,ix,T,M1,M2,M3>
    {
            ...
            const Scaling<ix,T>& x, const M1& m1, const M2& m2, M3& m3)
        {
            typedef typename M3::uppertri_type M3u;
            M3u m3u = m3.upperTri();
            MultUU_Helper<-1,s,add,ix,T,M1,M2,M3u>::call(x,m1,m2,m3u);
            typedef typename M3::lowertri_type::offdiag_type M3lo;
            M3lo m3lo = m3.lowerTri().offDiag();
            Maybe<!add>::zero(m3lo);
        }
    };

    // algo 950: L*L, so convert to MultUU call (-1, not -2)
    template <int s, bool add, 
              int ix, class T, class M1, class M2, class M3>
    struct MultUL_Helper<950,s,add,ix,T,M1,M2,M3>
    {
            ...
            const Scaling<ix,T>& x, const M1& m1, const M2& m2, M3& m3)
        {
            typedef typename M3::lowertri_type M3l;
            M3l m3l = m3.lowerTri();
            MultUU_Helper<-1,s,add,ix,T,M1,M2,M3l>::call(x,m1,m2,m3l);
            typedef typename M3::uppertri_type::offdiag_type M3uo;
            M3uo m3uo = m3.upperTri().offDiag();
            Maybe<!add>::zero(m3uo);
        }
    };
    ...
    struct MultUL_Helper<99,s,add,ix,T,M1,M2,M3>
    {
            ...
            const Scaling<ix,T>& x, const M1& m1, const M2& m2, M3& m3)
        {
            ...
            } else if ( !s2 && ( ExactSameStorage(m1,m3) || 
