...
#define TMV_TRANSPOSEM_UNROLL 0
    ...
    struct TransposeSelf_Helper<11,s,M1>
    {
        ...
        {
            const ptrdiff_t n = (s == Unknown ? m.colsize() : s);
    ...
    struct TransposeSelf_Helper<12,s,M1>
    {
        static void call(M1& m)
        {
            ...
            const ptrdiff_t step1 = m.stepi();
            const ptrdiff_t step2 = m.stepj();
            for(ptrdiff_t i=1;i<n;++i) {
    ...
    struct TransposeSelf_Helper<14,s,M1>
    {
        static void call(M1& m)
        {
            ptrdiff_t n = s == Unknown ? m.colsize() : s;
            if (n <= 1) return;
            typedef typename M1::row_type Mr;
            typedef typename M1::col_type Mc;
            typedef typename Mr::iterator IT1;
            ...
            const ptrdiff_t step = m.stepi() + m.stepj();
            for(--n;n;--n) {
    ...
    struct TransposeSelf_Helper<-3,s,M1>
    {
        static TMV_INLINE void call(M1& m)
        {
            ...
            // nops = n(n-1)/2
            const ptrdiff_t sm1 = IntTraits<s>::Sm1;
            const ptrdiff_t nops = IntTraits2<s,sm1>::safeprod / 2;
            const bool unroll = 
                s >= 8 ? false :
                s == Unknown ? false :
