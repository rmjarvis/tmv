...
namespace tmv {
  ...
  public :
    ...
    inline reference operator[](int n) const { return reference(p[n*step()]); }
  ...
  template <class T> inline T TMV_CONJ(const ConjRef<T>& x) { return x.conj(); }
  template <class T> inline RealType(T) TMV_NORM(const ConjRef<T>& x) 
  { return TMV_NORM(x.conj()); }
  template <class T> inline RealType(T) TMV_ABS(const ConjRef<T>& x) 
  ...
  template <class T> inline RealType(T) TMV_REAL(const ConjRef<T>& x) 
  { return x.real(); }
  template <class T> inline RealType(T) TMV_IMAG(const ConjRef<T>& x) 
  { return x.imag(); }
  ...
  template <class T, int S, bool C> 
  inline std::string TypeText(VIt<T,S,C> it)
  {
    std::ostringstream s;
    s << "VIt<" << TypeText(T());
    s << ","<<IntTraits<S>::text();
  ...
  template <class T, int S, bool C> 
  inline std::string TypeText(CVIt<T,S,C> it)
  {
    std::ostringstream s;
    s << "CVIt<" << TypeText(T());
    s << ","<<IntTraits<S>::text();
