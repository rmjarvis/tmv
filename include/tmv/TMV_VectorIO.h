...
namespace tmv {
  ...
  public :
    ...
    inline VectorReadError(std::istream& _is) throw() :
      ...
      is(_is), iseof(_is.eof()), isbad(_is.bad()) {}
    inline VectorReadError(int _i, const BaseVector<V>& _v, std::istream& _is,
  ...
  inline std::istream& operator>>(std::istream& is, auto_ptr<Vector<T,I> >& v)
  {
    size_t n;
    is >> n;
    if (!is)
