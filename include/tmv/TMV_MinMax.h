  ...
  struct MinMaxElement_Helper<1,comp,max,V>
  {
    ...
    static inline RT call(const V& v, int*const ibest)
    {
      ...
      if (n) do {
        ...
        // if statements to be true and then backtracks if it turned out
        // to be false.  So you always want to test for the most likely
        // choice first.  In this case, it is more likely that value
        // is not the new best.
...
L2:
  ...
  template <class V>
  struct MinMaxElement_Helper<8,ABS_COMP,true,V>
  {
    typedef typename V::value_type VT;
    typedef typename V::real_type RT;
    typedef typename V::const_iterator IT;
    ...
    static inline RT call(const V& v, int*const ibest)
    {
      ...
      if (n) do {
        ...
        if (TMV_ABS(real(value)) + TMV_ABS(imag(value)) < best) continue;
        else { 
          Component<ABS_COMP,VT>::applyf(value);
  ...
  template <class V>
  struct MinMaxElement_Helper<8,ABS_COMP,false,V>
  {
    typedef typename V::value_type VT;
    typedef typename V::real_type RT;
    typedef typename V::const_iterator IT;
...
#if TMV_OPT >= 2
    if (!imax) 
      return MinMaxElement_Helper<algo1,REAL_COMP,true,V>::call(v.vec(),imax);
    else 
#endif
      return MinMaxElement_Helper<algo2,REAL_COMP,true,V>::call(v.vec(),imax);
  }
      ...
      const BaseVector_Calc<V>& v, int*const imax)
  {
    ...
    enum { algo2 = (
        ...
        1 ) };
#ifdef PRINTALGO
    std::cout<<"InlineMaxAbsElement: algo = "<<algo1<<" "<<algo2<<std::endl;
#endif

#if TMV_OPT >= 2
    if (!imax) 
      return MinMaxElement_Helper<algo1,ABS_COMP,true,V>::call(v.vec(),imax);
    else 
#endif
      return MinMaxElement_Helper<algo2,ABS_COMP,true,V>::call(v.vec(),imax);
  }
      ...
      const BaseVector_Calc<V>& v, int*const imax)
  {
    ...
    enum { algo2 = (
        ...
        1 ) };
#ifdef PRINTALGO
    std::cout<<"InlineMaxAbs2Element: algo = "<<algo1<<" "<<algo2<<std::endl;
#endif

#if TMV_OPT >= 2
    if (!imax) 
      return MinMaxElement_Helper<algo1,ABS2_COMP,true,V>::call(v.vec(),imax);
    else 
#endif
      return MinMaxElement_Helper<algo2,ABS2_COMP,true,V>::call(v.vec(),imax);
  }

  // Defined in TMV_Vector.cpp
  template <class T>
  RealType(T) InstMaxAbs2Element(
      const ConstVectorView<T>& v, int*const imax); // BLAS amax

  template <class T>
  inline RealType(T) InstMaxAbs2Element(
...
#if TMV_OPT >= 2
    if (!imin) 
      return MinMaxElement_Helper<algo1,REAL_COMP,false,V>::call(v.vec(),imin);
    else 
#endif
      return MinMaxElement_Helper<algo2,REAL_COMP,false,V>::call(v.vec(),imin);
  }
      ...
      const BaseVector_Calc<V>& v, int*const imin)
  {
    ...
    enum { algo2 = (
        ...
        1 ) };
#ifdef PRINTALGO
    std::cout<<"InlineMinAbsElement: algo = "<<algo1<<" "<<algo2<<std::endl;
#endif

#if TMV_OPT >= 2
    if (!imin) 
      return MinMaxElement_Helper<algo1,ABS_COMP,false,V>::call(v.vec(),imin);
    else 
#endif
      return MinMaxElement_Helper<algo2,ABS_COMP,false,V>::call(v.vec(),imin);
  }

  // Defined in TMV_Vector.cpp
  template <class T>
  RealType(T) InstMinAbsElement(const ConstVectorView<T>& v,
      int*const imin); // BLAS amin

  template <class T>
  inline RealType(T) InstMinAbsElement(
      ...
      const BaseVector_Calc<V>& v, int*const imin)
  {
    ...
    enum { algo2 = (
        ...
        1 ) };
#ifdef PRINTALGO
    std::cout<<"InlineMinAbs2Element: algo = "<<algo1<<" "<<algo2<<std::endl;
#endif

#if TMV_OPT >= 2
    if (!imin) 
      return MinMaxElement_Helper<algo1,ABS2_COMP,false,V>::call(v.vec(),imin);
    else 
#endif
      return MinMaxElement_Helper<algo2,ABS2_COMP,false,V>::call(v.vec(),imin);
  }

  // Defined in TMV_Vector.cpp
  template <class T>
  RealType(T) InstMinAbs2Element(const ConstVectorView<T>& v,
      int*const imin); // BLAS amin

  template <class T>
  inline RealType(T) InstMinAbs2Element(
      const ConstVectorView<T,UNKNOWN,true>& v, int*const imin)
