...
namespace tmv {
  ...
  public:
    ...
    template <class T2> 
    inline bool SameAs(const GenBandMatrix<T2>& ) const
    { return false; }
    ...
    RealType(T) Norm2() const
    {
      ...
      TMV_Warning("Calling BandMatrix::Norm2 without previously calling DivideUsing(SV)");
      return DoNorm2();
    }
    ...
    inline RealType(T) Condition() const
    {
      ...
      TMV_Warning("Calling BandMatrix::Condition without previously calling DivideUsing(SV)");
      return DoCondition();
    }
...
#define XTEST_DEBUG
  ...
  template <class T> 
  class BandMatrixView<T,FortranStyle> : 
  ...
  public:
    ...
    inline const BandMatrixView<T,FortranStyle>& operator=(
        const BandMatrixView<T,CStyle>& m2) const
    { BandMatrixView<T,CStyle>::operator=(m2); return *this; }

    inline const BandMatrixView<T,FortranStyle>& operator=(
        const BandMatrixView<T,CStyle>& m2)
    { BandMatrixView<T,CStyle>::operator=(m2); return *this; }
...
#define NEW_SIZE(cs,rs,lo,hi) \
    ...
    { 
      TMVAssert(S == RowMajor || S == ColMajor || S == DiagMajor);
      Zero();
      m2.AssignToU(UpperTriMatrixView<T>(itsm,colsize(),stepi(),stepj(),
            NonUnitDiag,isdm()?NoMajor:stor(),ct() FIRSTLAST));
    }
    ...
    { 
      TMVAssert(S == RowMajor || S == ColMajor || S == DiagMajor);
      Zero();
      m2.AssignToL(LowerTriMatrixView<T>(itsm,colsize(),stepi(),stepj(),
            NonUnitDiag,isdm()?NoMajor:stor(),ct() FIRSTLAST));
    }
...
#undef NEW_SIZE
    ...
    template <IndexStyle I2>
    inline BandMatrix<T,S,I>& operator=(const BandMatrix<T,S,I2>& m2)
    {
      TMVAssert(colsize() == m2.colsize());
      TMVAssert(rowsize() == m2.rowsize());
      TMVAssert(nlo() >= m2.nlo());
    ...
    {
      return ConstVectorView<T,I>(itsm,
    ...
    inline ConstVectorView<T,I> diag(int i) const
    {
      ...
      if (i >= 0) {
        const size_t diagsize = MIN(colsize(),rowsize()-i);
        return ConstVectorView<T,I>(itsm+i*stepj(),
            diagsize,diagstep(),NonConj);
      } else {
        const size_t diagsize = MIN(colsize()+i,rowsize());
        return ConstVectorView<T,I>(itsm-i*stepi(),
            diagsize,diagstep(),NonConj);
      }
    }
    ...
    inline ConstVectorView<T,I> diag(int i, int j1, int j2) const
    {
      ...
      if (i >= 0) {
        TMVAssert(j2<=MIN(int(rowsize())-i,int(colsize())));
        return ConstVectorView<T,I>(itsm+i*stepj()+j1*diagstep(),
            j2-j1,diagstep(),NonConj);
      } else {
        TMVAssert(j2<=MIN(int(colsize())+i,int(rowsize())));
        return ConstVectorView<T,I>(itsm-i*stepi()+j1*diagstep(),
            j2-j1,diagstep(),NonConj);
      }
    }
    ...
    inline VectorView<T,I> diag(int i)
    {
      ...
      if (i >= 0) {
        const size_t diagsize = MIN(colsize(),rowsize()-i);
        return VectorView<T,I>(itsm+i*stepj(),
            diagsize,diagstep(),NonConj FIRSTLAST);
      } else {
        const size_t diagsize = MIN(colsize()+i,rowsize());
        return VectorView<T,I>(itsm-i*stepi(),
            diagsize,diagstep(),NonConj FIRSTLAST);
      }
    }
    ...
    inline VectorView<T,I> diag(int i, int j1, int j2)
    {
      ...
      if (i >= 0) {
        TMVAssert(j2<=MIN(int(rowsize())-i,int(colsize())));
        return VectorView<T,I>(itsm+i*stepj()+j1*diagstep(),
            j2-j1,diagstep(),NonConj FIRSTLAST);
      } else {
        TMVAssert(j2<=MIN(int(colsize())+i,int(rowsize())));
        return VectorView<T,I>(itsm-i*stepi()+j1*diagstep(),
            j2-j1,diagstep(),NonConj FIRSTLAST);
      }
    }
        ...
        int i, int j, int istep, int jstep, int size) const
    {
      ...
      return ConstVectorView<T,I>(itsm+i*stepi()+j*stepj(),size,
    ...
    {
      return ConstBandMatrixView<T>(itsm,
          MIN(colsize(),rowsize()),MIN(colsize()+nhi(),rowsize()),
          0,nhi(),stepi(),stepj(),diagstep(),stor(),ct());
    }
    ...
    {
      return ConstBandMatrixView<T>(itsm+stepj(),
          MIN(colsize(),rowsize()-1),MIN(colsize()+nhi(),rowsize()-1),
          0,nhi()-1,stepi(),stepj(),diagstep(),stor(),ct());
    }
    ...
    {
      return ConstBandMatrixView<T>(itsm+stepi(),
          MIN(colsize()-1,rowsize()+nlo()),MIN(colsize()-1,rowsize()),
          nlo()-1,0,stepi(),stepj(),diagstep(),stor(),ct());
    }
    ...
    inline ConstBandMatrixView<RealType(T),I> Real() const
    {
      return ConstBandMatrixView<RealType(T),I>(
          reinterpret_cast<const RealType(T)*>(itsm),
          colsize(),rowsize(),nlo(),nhi(),
          IsReal(T()) ? stepi() : 2*stepi(),
          IsReal(T()) ? stepj() : 2*stepj(),
          IsReal(T()) ? diagstep() : 2*diagstep(),
    ...
    inline ConstBandMatrixView<RealType(T),I> Imag() const
    {
      TMVAssert(IsComplex(T()));
      return ConstBandMatrixView<RealType(T),I>(
          reinterpret_cast<const RealType(T)*>(itsm)+1,
          colsize(),rowsize(),nlo(),nhi(),
          2*stepi(),2*stepj(),2*diagstep(),NoMajor,NonConj);
    }
        ...
        int istep, int jstep, int size)
    {
      TMVAssert(View().OKSubVector(i,j,istep,jstep,size));
      if (I == FortranStyle) { --i; --j; }
      return VectorView<T,I>(itsm+i*stepi()+j*stepj(),size,
    ...
    {
      return BandMatrixView<T,I>(itsm,
          MIN(colsize(),rowsize()),MIN(colsize()+nhi(),rowsize()),
          0,nhi(),stepi(),stepj(),diagstep(),stor(),ct() FIRSTLAST);
    }
    ...
    {
      return BandMatrixView<T,I>(itsm+stepj(),
          MIN(colsize(),rowsize()-1),MIN(colsize()+nhi(),rowsize()-1),
          0,nhi()-1,stepi(),stepj(),diagstep(),stor(),ct() FIRSTLAST);
    }
    ...
    {
      return BandMatrixView<T,I>(itsm+stepi(),
          MIN(colsize()-1,rowsize()+nlo()),MIN(colsize()-1,rowsize()),
          nlo()-1,0,stepi(),stepj(),diagstep(),stor(),ct() FIRSTLAST);
    }
    ...
    {
      return BandMatrixView<RealType(T),I>(
          reinterpret_cast<RealType(T)*>(itsm),
          colsize(),rowsize(),nlo(),nhi(),
          IsReal(T()) ? stepi() : 2*stepi(),
          IsReal(T()) ? stepj() : 2*stepj(),
          IsReal(T()) ? diagstep() : 2*diagstep(),
    ...
    inline BandMatrixView<RealType(T),I> Imag()
    {
      TMVAssert(IsComplex(T()));
      return BandMatrixView<RealType(T),I>(
          reinterpret_cast<RealType(T)*>(itsm)+1,
          colsize(),rowsize(),nlo(),nhi(),
          2*stepi(),2*stepj(),2*diagstep(),NoMajor,NonConj
#ifdef TMVFLDEBUG
          ,reinterpret_cast<const RealType(T)*>(first)+1
    ...
    { 
      return ConstBandMatrixView<T,I>(itsm,colsize(),rowsize(),nlo(),nhi(),
          stepi(),stepj(),diagstep(),S,NonConj,isdm()?0:linsize);
    }

    inline ConstBandMatrixView<T,I> Transpose() const
    { 
      return ConstBandMatrixView<T,I>(itsm,rowsize(),colsize(),nhi(),nlo(),
          stepj(),stepi(),diagstep(),TransOf(S),NonConj,isdm()?0:linsize);
    }

    inline ConstBandMatrixView<T,I> Conjugate() const
    { 
      return ConstBandMatrixView<T,I>(itsm,colsize(),rowsize(),nlo(),nhi(),
          stepi(),stepj(),diagstep(),S,ConjOf(T,NonConj),isdm()?0:linsize);
    }
    ...
    {
      return ConstVectorView<T>(itsm1.get(),linsize,1,NonConj);
    }

    inline BandMatrixView<T,I> View() 
    { 
      return BandMatrixView<T,I>(itsm,colsize(),rowsize(),nlo(),nhi(),
          stepi(),stepj(),diagstep(),S,NonConj,isdm()?0:linsize FIRSTLAST);
    }

    inline BandMatrixView<T,I> Transpose()
    { 
      return BandMatrixView<T,I>(itsm,rowsize(),colsize(),nhi(),nlo(),
          stepj(),stepi(),diagstep(),TransOf(S),NonConj,isdm()?0:linsize 
          FIRSTLAST);
    }
    ...
    { 
      return BandMatrixView<T,I>(itsm,rowsize(),colsize(),nhi(),nlo(),
          stepj(),stepi(),diagstep(),TransOf(S),ConjOf(T,NonConj),
          isdm()?0:linsize FIRSTLAST);
    }

    inline VectorView<T,I> LinearView()
    {
      return VectorView<T,I>(itsm1.get(),linsize,1,NonConj FIRSTLAST );
    }
  ...
  template <class T> 
  inline ConstBandMatrixView<T> Transpose(
      const GenBandMatrix<T>& m)
  { return m.Transpose(); }

  template <class T, IndexStyle I> 
  inline ConstBandMatrixView<T,I> Transpose(
  ...
  template <class T> 
  inline ConstBandMatrixView<T> Conjugate(
      const GenBandMatrix<T>& m)
  { return m.Conjugate(); }

  template <class T, IndexStyle I> 
  inline ConstBandMatrixView<T,I> Conjugate(
      const ConstBandMatrixView<T,I>& m)
  { return m.Conjugate(); }
  ...
  template <class T> 
  inline ConstBandMatrixView<T> Adjoint(
      const GenBandMatrix<T>& m)
  { return m.Adjoint(); }

  template <class T, IndexStyle I> 
  inline ConstBandMatrixView<T,I> Adjoint(
      const ConstBandMatrixView<T,I>& m)
  { return m.Adjoint(); }
