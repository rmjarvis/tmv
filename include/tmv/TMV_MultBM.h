...
namespace tmv {
    ...
    struct MultBM_Helper<21,cs,rs,xs,add,ix,T,M1,M2,M3>
    {
            ...
            const Scaling<ix,T>& x, const M1& m1, const M2& m2, M3& m3)
        {
            ...
            const int i1 = m1.nlo();
            const int i2 = TMV_MIN(M,K-m1.nhi());
            const int i3 = TMV_MIN(M,K+m1.nlo());
            int j1 = 0;
            int j2 = m1.nhi()+1;
    ...
    struct MultBM_Helper<41,cs,rs,xs,add,ix,T,M1,M2,M3>
    {
            ...
            const Scaling<ix,T>& x, const M1& m1, const M2& m2, M3& m3)
        {
            ...
            for(int k=m1.nlo();k;--k) {
                ...
                MultMD_Helper<-4,xx,xx,add,ix,T,M2rrt,M1dm,M3rrt>::call(
                    x,m2k,m1k,m3k);
                if (len < K) ++len;
            }
            TMVAssert(len == TMV_MIN(M,K));
            const int ds = IntTraits2<cs,xs>::min;
            M1dm m10 = DiagMatrixViewOf(m1.diag());
            M2rrt m20 = m2.cRowRange(0,len).transpose();
            M3rrt m30 = m3.cRowRange(0,len).transpose();
            MultMD_Helper<-4,ds,rs,add,ix,T,M2rrt,M1dm,M3rrt>::call(
                x,m20,m10,m30);
        ...
        const BaseMatrix_Rec<M2>& m2, BaseMatrix_Rec_Mutable<M3>& m3)
    {
        ...
        TMV_MAYBE_CREF(M2,M2v) m2v = m2.cView();
        TMV_MAYBE_REF(M3,M3v) m3v = m3.cView();
        MultBM_Helper<-2,cs,rs,xs,add,ix,T,M1v,M2v,M3v>::call(x,m1v,m2v,m3v);
    }
        ...
        const BaseMatrix_Rec<M2>& m2, BaseMatrix_Rec_Mutable<M3>& m3)
    {
        ...
        TMV_MAYBE_CREF(M2,M2v) m2v = m2.cView();
        TMV_MAYBE_REF(M3,M3v) m3v = m3.cView();
        MultBM_Helper<99,cs,rs,xs,add,ix,T,M1v,M2v,M3v>::call(x,m1v,m2v,m3v);
    }
