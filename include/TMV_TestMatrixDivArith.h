...
//#define XXDEBUG2
  ...
  double eps = EPS*Norm(m1)*std::max(m2.colsize(),m2.rowsize())
    *Norm(m2)*Norm(m2inv);
  ...
  if (CanLDivEq(m1,b)) {
    ...
    m3 = m1;
    m3 = b.Inverse()*m3;
    Assert(Norm(m3-m4) <= eps,label+" m=b^-1*m");
  }
  if (CanRDivEq(m1,b)) {
    tmv::Matrix<T> m3 = m1;
    tmv::Matrix<T> m4 = m1;
    ...
    m3 = m1;
    m3 = m3 * b.Inverse();
    Assert(Norm(m3-m4) <= eps,label+" m=m*b^-1");
  }
...
//#define XXDEBUG3
    ...
    std::cerr<<"v/m = "<<fracm<<std::endl;
    std::cerr<<"Norm(diff) = "<<Norm((v/b)-frac)<<std::endl;
    std::cerr<<"eps = "<<eps*normfrac<<std::endl;
#endif
...
//#define XXDEBUG4
    ...
    std::cerr<<"m = "<<tmv::Type(m)<<"  "<<m<<std::endl;
    std::cerr<<"v%b = "<<v%b<<std::endl;
    std::cerr<<"v%m = "<<frac<<std::endl;
    std::cerr<<"(v%b)*m = "<<(v%b)*m<<std::endl;
    std::cerr<<"(v%b)*b = "<<(v%b)*b<<std::endl;
    std::cerr<<"(v%m)*m = "<<frac*m<<std::endl;
    std::cerr<<"(v%m)*b = "<<frac*b<<std::endl;
    std::cerr<<"diff = "<<(v%b)-frac<<std::endl;
    std::cerr<<"Norm(diff) = "<<Norm((v%b)-frac)<<std::endl;
    std::cerr<<"eps = "<<eps*normfrac<<std::endl;
#endif
    Assert(Norm((v%b)-frac) <= eps*normfrac,label+" v%m");
...
//#define XXDEBUG5
  ...
  std::cout<<"eps*x = "<<eps*normfrac<<std::endl;
#endif
