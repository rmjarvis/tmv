...
#ifdef TMVDEBUG
#ifndef NWARN
	std::cout<<"Warning: calling Norm2 for a TriMatrix\n";
#endif
#endif
	return DoNorm2();
      }
...
#ifdef TMVDEBUG
#ifndef NWARN
	std::cout<<"Warning: calling Condition for a TriMatrix\n";
#endif
#endif
	return DoCondition();
      }
...
#define NEW_SIZE(s) \
	...
	inline UpperTriMatrix(size_t _size, const std::vector<T>& vv) :
	  ...
	  for(int i=itslen;i>0;--i,++vi,++vvi) *vi = *vvi;
	}
...
#undef NEW_SIZE
	...
	inline ConstVectorView<T,I> diag(int i, int j1, int j2) const
	{
	  ...
	  const int ds = stepi()+stepj();
	  return ConstVectorView<T,I>(cptr()+i*stepj()+j1*ds,
	      j2-j1,ds,NonConj);
	}
	...
	inline VectorView<T,I> diag(int i, int j1, int j2)
	{
	  ...
	  const int ds = stepi()+stepj();
	  return VectorView<T,I>(ptr()+i*stepj()+j1*ds,
	...
	{ memset(itsm.get(),0,itslen*sizeof(T)); return *this; }
	...
	inline ConstUpperTriMatrixView<T,I> SubTriMatrix(int i1, int i2) const
	{
	  ...
	  return ConstUpperTriMatrixView<T,I>(cptr()+i1*(stepi()+stepj()),
...
#undef NEW_SIZE
	...
	inline ConstVectorView<T,I> diag(int i, int j1, int j2) const
	{
	  ...
	  const int ds = stepi()+stepj();
	  return ConstVectorView<T,I>(cptr()-i*stepi()+j1*ds,
	      j2-j1,ds,NonConj);
	}
	...
	inline VectorView<T,I> diag(int i, int j1, int j2)
	{
	  ...
	  const int ds = stepi()+stepj();
	  return VectorView<T,I>(ptr()-i*stepi()+j1*ds,
	      j2-j1,ds,NonConj FIRSTLAST);
	}
	...
	inline ConstLowerTriMatrixView<T,I> SubTriMatrix(int i1, int i2) const
	{
	  TMVAssert(View().OKSubTriMatrix(i1,i2,1));
	  if (I==FortranStyle) { --i1; }
	  return ConstLowerTriMatrixView<T,I>(cptr()+i1*(stepi()+stepj()),
	      i2-i1,stepi(),stepj(),D,S,NonConj);
	}
